...
void GCPkLevelSend(int aIndex, BYTE pklevel)
{
	...
	ObjYola.PHeadSubSetB((LPBYTE)&pMsg, 0xF3, 0x08, sizeof(pMsg));
...
//	ObjYola.PHeadSetB(&pNotice->PacketHeader, 0x0D, wcslen((unsigned short *)pNotice->szNoticeMessage) * 2 + 0x12);
//#else
//	wsprintf(pNotice->message, szNoticeMsg);
//	ObjYola.PHeadSetB((LPBYTE)pNotice, 0x0D, strlen(pNotice->message) + sizeof(PMSG_NOTICE_SEND) - sizeof(pNotice->message) + 1);
//#endif
//}
