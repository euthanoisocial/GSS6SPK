...
void CQuest::MarlonTeleport(int aIndex)
{
	...
	if (this->TimeCount > gServerInfo.m_TimeTeleportMarlon) //Tiempo para el teleport, reducir para testear si funciona bien.
	{
		...
		int tableindex;
		CUSTOM_TELEPORT_MARLON* MarlonTeleport;
		while (true)
		{
			tableindex = rand() % gCustomMarlon.m_count;

			if (this->LastMapNumber != tableindex)
			{
				MarlonTeleport = gCustomMarlon.GetTeleportMarlon(tableindex);
				this->LastMapNumber = MarlonTeleport->m_Mapa;
				break;
			}
		}


		//gLog.Output(LOG_GENERAL,"[TeleporMarlon] - Mapa: %d PosicionX: %d PosicionY: %d", MarlonTeleport->m_Mapa, MarlonTeleport->m_CoordenadaX, MarlonTeleport->m_CoordenadaY);
		
		//LogAdd(LOG_BLUE,"[TeleporMarlon] - Mapa: %d PosicionX: %d PosicionY: %d", MarlonTeleport->m_Mapa,MarlonTeleport->m_CoordenadaX,MarlonTeleport->m_CoordenadaY);
		gObj[aIndex].X = MarlonTeleport->m_CoordenadaX;
		gObj[aIndex].Y = MarlonTeleport->m_CoordenadaY;
		...
		gObj[aIndex].Map = MarlonTeleport->m_Mapa;
		gObj[aIndex].Dir = MarlonTeleport->m_Dir;
