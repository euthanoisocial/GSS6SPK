...
void cSkyEvent::ProcState_EMPTY()
{
	if (this->Start == 0)
	{
		...
		{
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] Bắt Đầu");
...
void cSkyEvent::CheckSync()
{
	...
	//LogAdd(LOG_BLACK, "[SkyEvent] Thoi Gian Bat Dau Con Lai [%d] Phut", this->m_RemainTime / 60);
...
int cSkyEvent::MonsterDie(LPOBJ lpObj)	//OK
{
	...
	if (iLevel > -1)
	{
		...
		{
			LogAdd(LOG_EVENT, "error-L3 : [SkyEvent] (%d) Khong Phu Hop Voi Nguoi Choi (%d) %s",
...
void cSkyEvent::DropWinnerItem(int iLevel, int iUser, BYTE SocketBonus, LPBYTE SocketOptions)
{
	...
	{
		GDSetCoinSend(gObj[iIndex].Index, Coin1, Coin2, Coin3, 0, 0, "Lottery");
	}


	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "Nhân Vật: %s Chiến Thắng Sự Kiện", &gObj[iIndex].Name);

	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "Sự Kiện Thiên Giới Level [%d]", iLevel + 1);
}
		...
		while (true)
		{
			...
			for (int iStages = 1; iStages < (MAX_SKY_EVENT_STAGES + 1); iStages++)
			{
				...
				while (true)
				{
					...
					iLevel = static_cast<int>(TokenNumber);

					Token = GetToken();
					MobNum = static_cast<int>(TokenNumber);

					Token = GetToken();
					X = static_cast<int>(TokenNumber);

					Token = GetToken();
					Y = static_cast<int>(TokenNumber);

					Token = GetToken();
					Count = static_cast<int>(TokenNumber);
...
void cSkyEvent__InsideTrigger(void * lpParam)	//OK
{
	...
	for (int i = 5; i > 0; i--) //5
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] Sẽ Đóng Sau %d Phút", i);
		Sleep(60000);
	}

	gSkyEvent.NPCEnabled = FALSE;

	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] Đã Đóng Cứa");
	...
	for (int iStages = 0; iStages < MAX_SKY_EVENT_STAGES; iStages++)
	{
		...
		gSkyEvent.MsgStringSend("[Sự Kiện Thiên Giới] Stage Experience has changed!", 0x01);

		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] Bắt Đầu Đợt %d Thời Gian Còn: %d Phút!", iStages + 1, gSkyEvent.StageMinutes[iStages]);
	...
	LogAdd(LOG_EVENT, "[SkyEvent] Ket Thuc");

	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] Kết Thúc");

	for (int i = 2; i > 0; i--)
	{
		wsprintf(sBuf, "%d Giây Chuyển Về Devias", i * 10);
			...
			if (gObj[m_Index].Connected >= OBJECT_CONNECTED)
			{
				if (this->m_UserData[iLevel][iUser].DBNumber == gObj[m_Index].DBNumber)
				{
					if ((gObj[this->m_UserData[iLevel][iUser].m_Index].Map == MAP_ICARUS) && (gObj[this->m_UserData[iLevel][iUser].m_Index].X >= 130))
					{
						...
						{
							gNotice.NewNoticeSend(this->m_UserData[iLevel][iUser].m_Index, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] - Thêm Kinh Nghiệm: %d", this->ExtraExp[iLevel][0]);
						}
						if (this->CurrentStage == 2)
						{
							gNotice.NewNoticeSend(this->m_UserData[iLevel][iUser].m_Index, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] - Thêm Kinh Nghiệm: %d", this->ExtraExp[iLevel][1]);
						}
						if (this->CurrentStage == 3)
						{
							gNotice.NewNoticeSend(this->m_UserData[iLevel][iUser].m_Index, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] - Thêm Kinh Nghiệm: %d", this->ExtraExp[iLevel][2]);
						}
	...
	if ((this->Enabled == 1) && (this->Start == 1) && (this->NPCEnabled == 1))
	{
		...
		if (iLevel > -1)
		{
			if (this->PlayerCount[iLevel] < MAX_SKY_EVENT_SUB_LEVEL)
			{
				...
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn Cần Ngôi Sao %s + %d Để Tham Gia Sự Kiện Thiên Giới Level %d!", ItemAttribute[0x1C33].Name, iLevel + 1, iLevel + 1);
				}
			}
			else
			{
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] [%d] Room Full!", iLevel + 1);
...
void cSkyEvent::AddUser(BYTE iLevel, int iIndex, LPOBJ lpObj)	//OK
{
	...
	gNotice.NewNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, "[Sự Kiện Thiên Giới] Người Chơi %d", this->PlayerCount[iLevel]);

	LogAdd(LOG_BLACK, "[SkyEvent] [Level %d] - Nguoi Choi %d", iLevel + 1, this->PlayerCount[iLevel]);
}
