...
{	
	...
	#if(TIDUS_FIX_MT)
	this->m_AttackSuccessRate = ((lpObj->Level + lpObj->MasterLevel) * gServerInfo.m_DarkSpiritAttackSuccessRateConstA)+((lpItem->m_PetItemLevel*gServerInfo.m_DarkSpiritAttackSuccessRateConstB)/gServerInfo.m_DarkSpiritAttackSuccessRateConstC) + (Strength / gServerInfo.m_DarkSpiritAttackSuccessRateConstD) + ((lpObj->Leadership+lpObj->AddLeadership) / gServerInfo.m_DarkSpiritAttackSuccessRateConstE);
	#else
...
bool CDarkSpirit::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,BYTE action) // OK
{
	...
	__int64 defense = this->GetTargetDefense(lpObj,lpTarget,&effect);

	float damage = this->GetAttackDamage(lpObj,lpTarget,lpSkill,&effect,defense);
