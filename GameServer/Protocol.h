...
void GCDamageSend(int aIndex,int bIndex,BYTE flag,float damage,int type,float ShieldDamage);
...
void GCMonsterDieSend(int aIndex,int bIndex,int experience,float damage,BYTE flag);
...
void GCMonsterDamageSend(int aIndex,float damage,float ShieldDamage);
...
struct PMSG_SEND_TIENTE_BAUCUA
{
	PSBMSG_HEAD header;
	int eWCOINC;
	int eWCOINP;
...
void GetInfoRankTitle(LPOBJ lpObj);
#if(REGISTER_INGAME)
...
#endif
struct XULY_CGPACKET_CH
{
	PSBMSG_HEAD header; // C3:F3:03
	DWORD ThaoTac;
};
struct XULY_CGPACKET_DL
{
	PSBMSG_HEAD header; // C3:F3:03
	DWORD ThaoTac;
};
