...
void CNewUIMyInventory::GCPetItemChangeSend(int aIndex, BYTE slot) // OK
{
	...
	pMsg.header.set(0x4E, 0x0F, sizeof(pMsg));
...
void CNewUIMyInventory::CalcItemCommonOption(LPOBJ lpObj, bool flag) // OK
{
	for (int n = 0; n < EQUIP_INVENTORY_SIZE; n++)
	{
		...
		{
			if (flag != 0) { continue; }
			gCustomPet.CalcCustomPet2Option(lpObj, flag);//Option Real
...
void CNewUIMyInventory::CGItemSellRecv(PMSG_EQUIP_ITEM_SELL_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpItem->Sell > 0)
	{
		if (lpItem->Coin1 > 0)
		{
			GDSetCoinSend(lpObj->Index, value, 0, 0, 0, 0, "CommonShopSell");
		}
		else if (lpItem->Coin2 > 0)
		{
			GDSetCoinSend(lpObj->Index, 0, value, 0, 0, 0, "CommonShopSell");
		}
		else if (lpItem->Coin3 > 0)
		{
			GDSetCoinSend(lpObj->Index, 0, 0, value, 0, 0, "CommonShopSell");
		}
		else
		{
