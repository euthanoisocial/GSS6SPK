...
#if(SLOTPET2)
void CPersonalShop::CGPEquipShopSetItemPriceRecv(PMSG_PSHOP_SET_ITEM_PRICE_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpObj->EquipInventory[lpMsg->slot].IsItem() == 0)
	{
	...
	lpObj->EquipInventory[lpMsg->slot].m_PShopValue = price;
...
#if(BOT_STORE)
	if (gObjIsConnectedGP(bIndex) == 0 && gObj[bIndex].IsBot != 6)
	{
		return;
	}
#else
...
void CPersonalShop::GCPShopViewportSend(int aIndex) // OK
{
	...
	{
		if (gObj[lpObj->VpPlayer[n].index].IsBot != 6) //BOT_STORE
		{
	...
	{
#if(BOT_STORE)
		if ((gObjIsConnectedGP(lpObj->PShopDealerIndex) == 0 || gObj[lpObj->PShopDealerIndex].PShopOpen == 0) && gObj[lpObj->PShopDealerIndex].IsBot != 6)
		{
#else
...
void CPersonalShop::GCPShopBuyItemSend(int aIndex, int bIndex, int slot, BYTE result) // OK
{
	...
	if (aI->VaultTimeBuyShop > 0)
	{
		GCChatTargetNewSend(aI, bI->Index, "Thay Đổi Nhân Vật Để Mua Tiếp");
		return;
	}
	...
	if (aI->VaultTimeBuyShop == 0)
	{
		...
		gObj[aIndex].VaultTimeBuyShop = gServerInfo.VaultBuyShop; /// tim ca cai nay

		//LogAdd(LOG_RED, "TIME %d", gObj[aIndex].VaultTimeBuyShop);
