...
void CReconnect::SetReconnectInfo(LPOBJ lpObj) // OK
{
	...
	info.AutoResetEnable = ( gServerInfo.m_CommandResetAutoEnable[lpObj->AccountLevel] == 0 ) ? 0 : lpObj->AutoResetEnable;

	memcpy(info.AutoResetStats,lpObj->AutoResetStats,sizeof(info.AutoResetStats));
	#if(RESET_VIP_AUTO_ON)
	info.AutoResetVipEnable = lpObj->AutoResetVipEnable;
	#endif
...
void CReconnect::ResumeCommand(LPOBJ lpObj,RECONNECT_INFO* lpInfo) // OK
{
	...
	lpObj->AutoResetEnable = ( gServerInfo.m_CommandResetAutoEnable[lpObj->AccountLevel] == 0 ) ? 0 : lpInfo->AutoResetEnable;

	lpObj->AutoResetEnable = lpInfo->AutoResetEnable;
	#if(RESET_VIP_AUTO_ON)
	lpObj->AutoResetVipEnable = lpInfo->AutoResetVipEnable;
	#endif
