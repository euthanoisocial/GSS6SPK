...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int Bdamage,int count,bool combo) // OK
{
	...
	{
		if (!gBAntihack.CheckAttackBlock(lpObj->Index))
		{
			return 0;
		}		
	}
	...
	#if(CTCMINI)
	if (gCTCMini.CheckPlayerTarget(lpObj, lpTarget, 1))
	{
		return 0;
	}
	#endif
	...
	//---------------------------NEWWWWW----------------------------------------
	#if(Delay_Skill_Custom==1)
	if((GetTickCount()-((DWORD) lpObj->AttackCustomDelay )) >= (((((DWORD) lpObj->PhysiSpeed) * 10 /* 5 10 15 20 */ ) > 1500 ) ? 1 /* 1 */ : ( 1500 - (((DWORD) lpObj->PhysiSpeed) * 10 ))))  // so sanh time delay attack
	{
		if ((GetTickCount()-((DWORD)lpObj->AttackCustomDelay)) < (DWORD)gCustomAttack.m_CustomAttackDelay) // so sanh time delay attack
		{
			return 1; // => 1
		}

		lpObj->AttackCustomDelay = GetTickCount();
		// check ngan ngan thoi. thay doi cac con so gium t phat xem ntn 
		if(	   skill != SKILL_FLAME 
			&& skill != SKILL_TWISTER 
			&& skill != SKILL_EVIL_SPIRIT 
			&& skill != SKILL_HELL_FIRE 
			&& skill != SKILL_AQUA_BEAM 
			&& skill != SKILL_BLAST 
			&& skill != SKILL_INFERNO 
			&& skill != SKILL_TRIPLE_SHOT 
			&& skill != SKILL_IMPALE
			&& skill != SKILL_MONSTER_AREA_ATTACK 
			&& skill != SKILL_PENETRATION 
			&& skill != SKILL_FIRE_SLASH 
			&& skill != SKILL_FIRE_SCREAM)  // so sanh skill su dung cua user 
		{
			if(skill != SKILL_DARK_SIDE)
			{
				
				if(lpTarget->Type == OBJECT_MONSTER) // neu la quai vat thi bem delay 
				{
		...
		{
			// la nguoi thi bem binh thuong nhu cu 
			if(lpObj->Type == OBJECT_USER)
			{
	...
	if(damage == 0)
	{
		...
		#if(SLOTPET2)
		g_pNewMyInventory.EquipSprtite(lpTarget, damage);
		#else
		...
		if(skill == SKILL_FALLING_SLASH || skill == SKILL_LUNGE || skill == SKILL_UPPERCUT || skill == SKILL_CYCLONE || skill == SKILL_SLASH || skill == SKILL_TWISTING_SLASH || skill == SKILL_RAGEFUL_BLOW || skill == SKILL_DEATH_STAB || skill == SKILL_CRESCENT_MOON_SLASH || skill == SKILL_STAR_FALL || skill == SKILL_IMPALE || skill == SKILL_FIRE_BREATH || skill == SKILL_ICE_ARROW || skill == SKILL_PENETRATION || skill == SKILL_FIRE_SLASH || skill == SKILL_POWER_SLASH || skill == SKILL_SPIRAL_SLASH || skill == SKILL_FROZEN_STAB || skill == SKILL_SWORD_SLASH || skill == SKILL_CHARGE || skill == SKILL_BLOOD_STORM)
		{
			if(skill != SKILL_IMPALE || lpObj->Inventory[8].m_Index == GET_ITEM(13,2) || lpObj->Inventory[8].m_Index == GET_ITEM(13,3) || lpObj->Inventory[8].m_Index == GET_ITEM(13,37))
			{
				...
				{
					damage = (damage*lpObj->DKDamageMultiplierRate) / gServerInfo.DameSkillSocGioDK;
				}
			}
		}
		...
		{
			if(skill == SKILL_BLAST || skill == SKILL_ICE_STORM || skill == SKILL_DECAY || skill == SKILL_FLAME)
			{
				damage = (damage * lpObj->DWDamageMultiplierRate) / 100;
				//LogAdd(LOG_RED, "[1] damage: %d - skill : %d - Class : %d", damage,skill , lpObj->Class);
			}
		}
		if (lpObj->Class == CLASS_MG)
		{
			if (skill == SKILL_FIRE_SLASH || skill == SKILL_POWER_SLASH || skill == SKILL_SWORD_SLASH || skill == SKILL_SPIRAL_SLASH)
			{
				damage = (damage * lpObj->MGDamageMultiplierRatePhysic) / 100;	
				//LogAdd(LOG_RED, "[2] damage: %d - skill : %d - Class : %d", damage,skill , lpObj->Class);
			}
			else if (skill == SKILL_CYCLONE || skill == SKILL_SLASH || skill == SKILL_TWISTING_SLASH || skill == SKILL_CRESCENT_MOON_SLASH || skill == SKILL_IMPALE || skill == SKILL_FIRE_BREATH 
				|| skill == SKILL_BLOOD_STORM || skill == SKILL_BLAST || skill == SKILL_LIGHTNING_STORM || skill == SKILL_FLAME)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / 100;
				//LogAdd(LOG_RED, "[3] damage: %d - skill : %d - Class : %d", damage,skill , lpObj->Class);
			}
		}


		// DMG SKILL ELF
		if ( skill == SKILL_PENETRATION || skill == SKILL_FIVE_SHOT || skill == SKILL_STAR_FALL || skill == SKILL_ICE_ARROW )
		{
			if (skill == SKILL_ICE_ARROW || skill == SKILL_STAR_FALL)
			{
				damage = (damage * lpObj->ELFDamageMultiplierRate *2) / 100;
			}
			else
			{
				damage = (damage * lpObj->ELFDamageMultiplierRate) / 100;	
			}
		}
		...
		else if (skill == SKILL_CHAIN_LIGHTNING || skill == SKILL_DRAIN_LIFE || skill == SKILL_RED_STORM)
		{
			//damage = (damage*((count==2)?70:((count==3)?50:100)))/100;
			damage = (damage * lpObj->SUDamageMultiplierRate) / 100;
		}
		...
		if ((GetLargeRand() % gServerInfo.GioiHanTyLeX2SatThuong) < ((lpObj->DoubleDamageRate) - lpTarget->ResistDoubleDamageRate))
		{
			effect |= 0x100; // 0x40 se len 2 dong dmg
			damage += damage;
		}

		if ((GetLargeRand() % gServerInfo.GioiHanTyLeX3SatThuong) < lpObj->TripleDamageRate)
		{
			effect |= 0x100;
	...
	if(lpObj->Type == OBJECT_USER)
	{
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			else if(gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE && lpObj->Map == MAP_CASTLE_SIEGE && lpTarget->Map == MAP_CASTLE_SIEGE)
			{
				...
				{
					if(gServerInfo.m_CastleSiegeDamageRate2 == 0){
		...
		else
		{
			...
			damage = (damage * gServerInfo.m_GeneralDamageRatePvM) / gServerInfo.MoGioiHanDameQuai;

			damage = (damage * gServerInfo.m_DamageRatePvM[lpObj->Class]) / gServerInfo.MoGioiHanDameQuai;
			//#if(EXPDMG)
			//if (gServerInfo.m_GeneralDamageRatePvM > gServerInfo.MoGioiHanDameQuai)
			//{
			//	float gCalcDameRate = (float)(gServerInfo.m_GeneralDamageRatePvM / gServerInfo.MoGioiHanDameQuai);
			//	damage = damage*gCalcDameRate;
			//}
			//else 
			//{
			//	damage = (damage*gServerInfo.m_GeneralDamageRatePvM) / gServerInfo.MoGioiHanDameQuai;
			//}
			//
			//if (gServerInfo.m_DamageRatePvM[lpObj->Class] > gServerInfo.MoGioiHanDameQuai)
			//{
			//	float CalcDameRate = (float)(gServerInfo.m_DamageRatePvM[lpObj->Class] / gServerInfo.MoGioiHanDameQuai);
			//	damage = damage*CalcDameRate;
			//}
			//else 
			//{
			//	damage = (damage*gServerInfo.m_DamageRatePvM[lpObj->Class]) / gServerInfo.MoGioiHanDameQuai;
	...
	if(damage > 0)
	{
		if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER && lpObj->Index != lpTarget->Index)
		{
			...
			#if(THANMA)
			if (gThanMaChien.CheckPlayerTarget(lpObj) && gThanMaChien.CheckPlayerTarget(lpTarget))
			{
				CheckSelfDefense = 0;
			}
			#endif
			#if(EVENT_PK)
			if (gBsVEvent.CheckPlayerTarget(lpObj) && gBsVEvent.CheckPlayerTarget(lpTarget))
			{
				CheckSelfDefense = 0;
			}
			#endif
...
bool CAttack::DecreaseArrow(LPOBJ lpObj) // OK
{
	...
	if(lpObj->Inventory[0].m_Index >= GET_ITEM(4,0) && lpObj->Inventory[0].m_Index < GET_ITEM(5,0) && lpObj->Inventory[0].m_Index != GET_ITEM(4,15) && lpObj->Inventory[0].m_Slot == 0)
	{
		...
		{
			#if(ELF_NO_ARROW)
			if (gServerInfo.ElfNoBoltArrow == 1)
			{
				gItemManager.DecreaseItemDur(lpObj, 0, 0);
			}
			else if (gServerInfo.ElfNoBoltArrow == 0)
			{
...
void CAttack::WingSprite(LPOBJ lpObj,LPOBJ lpTarget,float* damage) // OK
{
...
void CAttack::HelperSprite(LPOBJ lpObj,LPOBJ lpTarget,float* damage) // OK
{
	if(lpObj != 0 && lpObj->Type == OBJECT_USER)
	{
		...
		if(lpItem->IsItem() != 0 && lpItem->m_Durability > 0)
		{
			...
			{
				if ((lpItem->m_NewOption & 4) != 0)
				{
					(*damage) = ((*damage)*(100 + gServerInfo.m_GodenFenrirIncDamageConstA)) / 100;
				}
	...
	if (lpObj != 0 && lpObj->Type == OBJECT_USER)
	{
		CItem* lpItem = &lpObj->EquipInventory[12];
...
//======================Dame Mới Pet================
		else if(lpItem->m_Index == GET_ITEM(13,64) && lpItem->m_Level >= 0) // Đại Quỷ
			{
				
				if(lpItem->m_Level == 0)
				{
				(*damage) = ((*damage)*(110+lpItem->m_Level))/100;
				lpObj->ExperienceRate +=	   52;
				lpObj->MasterExperienceRate += 52;
				}
				if(lpItem->m_Level >= 1)
				{
				
				(*damage) = ((*damage)*(110+lpItem->m_Level*2))/100;

				lpObj->ExperienceRate +=	   50+lpItem->m_Level*16;
				lpObj->MasterExperienceRate += 50+lpItem->m_Level*16;
	...
	if(lpTarget != 0 && lpTarget->Type == OBJECT_USER)
	{
		...
		if(lpItem->IsItem() != 0 && lpItem->m_Durability > 0)
		{
			...
			else if (lpItem->m_Index == GET_ITEM(13, 37)) // Fenrir
			{
				if ((lpItem->m_NewOption & 4) != 0)
				{
					(*damage) = ((*damage)*(100 - gServerInfo.m_GodenFenrirDecDamageConstA)) / 100;
				}
...
void CAttack::DamageSprite(LPOBJ lpObj,float damage) // OK
{
	if(lpObj->Type != OBJECT_USER)
	{
		return;
	...
	{
		if(this->MuunPet(lpObj, damage) == 0)
		{
			return;
		}
	}
...
bool CAttack::MuunPet(LPOBJ lpObj,__int64 damage) // OK
{
	CItem* lpItem = &lpObj->Inventory[8];
	...
	int MaxSmallDur = (200 * gServerInfo.m_MuunDurabilityRate) / 100;
...
bool CAttack::DarkHorseSprite(LPOBJ lpObj,float damage) // OK
{
	CItem* lpItem = &lpObj->Inventory[8];
...
bool CAttack::FenrirSprite(LPOBJ lpObj,float damage) // OK
{
	CItem* lpItem = &lpObj->Inventory[8];
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	//PK Guild
	if(lpObj->Map == 88 || lpObj->Map == 6 && lpObj->X > 54 && lpObj->Y > 138 || lpObj->Map == MAP_THANMACHIEN || lpObj->Map == MAP_CHIENTRUONGCO || lpObj->Map == MAP_DUEL_ARENA || lpObj->Map == MAP_CHAOS_CASTLE1 || lpObj->Map == MAP_CHAOS_CASTLE2|| lpObj->Map == MAP_CHAOS_CASTLE3|| lpObj->Map == MAP_CHAOS_CASTLE4|| lpObj->Map == MAP_CHAOS_CASTLE5|| lpObj->Map == MAP_CHAOS_CASTLE6|| lpObj->Map == MAP_CHAOS_CASTLE7 )
	{
		//PK nếu ở map đấu trường và Chaos thì cho phép.
	}
	else
	{
		if (gObjGetRelationShip(lpObj, lpTarget) == 1) 
		{ 
			return 0; // PK Lien Minh
		}

		if(lpObj->Guild != 0 && lpTarget->Guild != 0)
		{		
			if(lpObj->Guild->Number == lpTarget->Guild->Number && gServerInfo.m_GuildPK != 0 && gDuel.CheckDuel(lpObj,lpTarget) == 0)
			{
				return 0; // PK Cung Gui
			}
		}
	}
	if(gObjGetRelationShip(lpObj,lpTarget) == 2 && gMapManager.GetMapNonPK(lpTarget->Map,lpTarget,lpObj) == 0)
	{
		return 1;
	}	
	...
	if(gObjTargetGuildWarCheck(lpObj,lpTarget) == 0)
	{
		...
		{
			if(lpTarget->Guild->WarType == 1 && lpTarget->Map != MAP_ARENA && gMapManager.GetMapNonPK(lpTarget->Map,lpTarget,lpObj) == 0)
			{
				return 1;
			}
	...
	{
		return ((gMapManager.GetMapNonPK(lpTarget->Map,lpTarget,lpObj)==0)?gGensSystem.CheckGens(lpObj,lpTarget):0);
	}
	...
	if (gServerInfo.m_DevilSquarePK == 0)
	...
	if (gServerInfo.m_BloodCastlePK == 0)
	...
	if(lpObj->PKCount >= gServerInfo.m_PKLimitCount){
		return 0;
	}

	#if(THANMA)
	if (gThanMaChien.CheckPlayerJoined(lpObj, lpObj) && !gThanMaChien.CheckPlayerJoined(lpTarget, lpTarget))
	{
		return 0;
	}

	if (gThanMaChien.CheckPlayerJoined(lpTarget, lpTarget) && !gThanMaChien.CheckPlayerJoined(lpObj, lpObj))
	{
		return 0;
	}

	if (gThanMaChien.CheckSelfTeam(lpObj, lpTarget))
	{
		return 0;
	}
	#endif
	...
	#if(EVENT_PK)
	if (gBsVEvent.CheckPlayerJoined(lpObj,lpObj) && !gBsVEvent.CheckPlayerJoined(lpTarget,lpTarget))
	{
		return 0;
	}
	
	if (gBsVEvent.CheckPlayerJoined(lpTarget,lpTarget) && !gBsVEvent.CheckPlayerJoined(lpObj,lpObj))
	{
		return 0;
	}
	
	if (gBsVEvent.CheckSelfTeam(lpObj, lpTarget))
	{
		return 0; 
	}
	#endif
...
bool CAttack::ApplySkillEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,__int64 damage) // OK
{
	...
	switch(lpSkill->m_skill)
	{
		...
		case SKILL_SLASH:
			...
			break;
		case SKILL_DECAY:
			gEffectManager.AddEffect(lpTarget, 0, gSkillManager.GetSkillEffect(lpSkill->m_index), gServerInfo.ThoiGianDameDocDW, lpObj->Index, 2, 3, 0);
			break;
...
float CAttack::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	int DamageMin = 0;
	int DamageMax = 0;
	float SkillDamageMin = 0;
	float SkillDamageMax = 0;
	...
	else
	{
		...
		if ((GetLargeRand() % gServerInfo.GioiHanTyLeSatThuongChiMang) < ((lpObj->CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate) - lpTarget->ResistCriticalDamageRate))
		{
			(*effect) = 3;
		...
		if ((GetLargeRand() % gServerInfo.GioiHanTyLeSatThuongHoanHao) < ((lpObj->ExcellentDamageRate + lpObj->EffectOption.AddExcellentDamageRate) - lpTarget->ResistExcellentDamageRate))
		{
			(*effect) = 2;
...
float CAttack::GetAttackDamageWizard(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	CItem* Right = &lpObj->Inventory[0];
	CItem* Left = &lpObj->Inventory[1];
	...
	float damage = DamageMin+(GetLargeRand()%range);

	if ((GetLargeRand() % gServerInfo.GioiHanTyLeX2SatThuong) < ((lpObj->DoubleDamageRate) - lpTarget->ResistDoubleDamageRate))
	{
		(*effect) = 6;
		damage = DamageMax;
		damage += lpObj->DoubleDamageRate;

		//LogAdd(LOG_RED, "[TimLoi] DoubleDamageRate Monster");
	}
	if ((GetLargeRand() % gServerInfo.GioiHanTyLeSatThuongChiMang) < ((lpObj->CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate) - lpTarget->ResistCriticalDamageRate))
	{
		(*effect) = 3;
...
float CAttack::GetAttackDamageCursed(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	CItem* Right = &lpObj->Inventory[0];
	CItem* Left = &lpObj->Inventory[1];
	...
	if(Left->IsItem() != 0 && Left->m_IsValidItem != 0 && Left->m_Index >= GET_ITEM(5,21) && Left->m_Index <= GET_ITEM(5,29))
	{
...
float CAttack::GetAttackDamageFenrir(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	float BaseDamage = 0;
	...
	float damage = (BaseDamage+lpSkill->m_DamageMin)+(GetLargeRand()%range);
...
float CAttack::GetAttackDamageElemental(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int AttackDamage,int TargetDefense) // OK
{
	#if(GAMESERVER_UPDATE>=701)
...
float CAttack::GetShieldDamage(LPOBJ lpObj,LPOBJ lpTarget,float damage) // OK
{
	...
	float SDDamage = (damage*rate)/100;
	float HPDamage = damage-SDDamage;
...
void CAttack::GetPreviewDamageMultiplier(LPOBJ lpObj,DWORD* DamageMultiplier,DWORD* RFDamageMultiplierA,DWORD* RFDamageMultiplierB,DWORD* RFDamageMultiplierC) // OK
{
	switch(lpObj->Class)
	{
		...
		case CLASS_FE:
			...
			break;
		case CLASS_MG:
			(*DamageMultiplier) = lpObj->MGDamageMultiplierRate;//200;
			(*RFDamageMultiplierA) = 100;
			(*RFDamageMultiplierB) = 100;
			(*RFDamageMultiplierC) = 100;
			break;
