...
#endif
	#if(THANMA)
    gThanMaChien.MainProc();
#endif
	#if(CB_EventArenaPvP)
	gCBArenaPVP.MainProc();
#endif
}
#if(TEST_GHRS)
void gObjGHRSRunProc() // OK
{
	gServerInfo.ThayDoiGioiHanRS();
}
#endif
...
*/
#if(MAX_LEVEL)
void gObjSetExperienceTable() // OK  //code exp 2k level
{
	gLevelExperience[0] = 0;
...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	#if(SAU_ANTIHACK)
	gBAntihack.SetUserZeroCache(lpObj->Index);
	#endif
	memset(lpObj->DayReg,0,sizeof(lpObj->DayReg));
	lpObj->Level = 0;
	...
	lpObj->ChaosLock = 0;
#if(SLOTPET2)
	lpObj->EquipInventoryStatus[0] = 0;
	lpObj->EquipInventoryStatus[1] = 0;
	lpObj->EquipInventoryStatus[3] = 0;
	lpObj->EquipInventoryStatus[4] = 0;
	lpObj->LoadEquipItem = 0;
#else
	...
	{
		lpObj->CH_IndexItem[ch] = -1;
		lpObj->CH_InfoItem[ch].Clear();
	}
	#endif
#if (CUSTOM_HARMONY)
	lpObj->HM_IndexRandom = -1;
	lpObj->HM_StatusMix = 0;
	lpObj->HM_ItemChuaEp.Clear();
	lpObj->HM_ItemDaEp.Clear();
#endif
	#if (SPK_DUNGLUYEN)
	lpObj->DL_StatusMix = 0;
	lpObj->DL_IndexMix = -1;
	...
	lpObj->BuyShopTickCount = 0;
	lpObj->BuyXShopTickCount = 0;
...
//	lpObj->AttackCustomOfflineMoneyDelay = 0;
	...
	#if (CB_EventArenaPvP)
	lpObj->ArenaEventStatus = 0; //trang thai antihack khong check, dung de set neu co chuc nang event hoaac trang thai dac biet
#endif	
...
void gObjClearPlayerOption(LPOBJ lpObj) // OK
{
	...
	for (int n = 0; n < EQUIP_INVENTORY_SIZE; n++)
	{
		...
		lpObj->EquipInventory1[n].Clear();

		lpObj->EquipInventory2[n].Clear();
	}
#else
	#if(GAMESERVER_UPDATE>=603)
...
#if(SLOTPET2)
	memset(lpObj->EquipInventoryMap, 0xFF, EQUIP_INVENTORY_SIZE);
	memset(lpObj->EquipInventoryMap1, 0xFF, EQUIP_INVENTORY_SIZE);
	memset(lpObj->EquipInventoryMap2, 0xFF, EQUIP_INVENTORY_SIZE);
#else
	#if(GAMESERVER_UPDATE>=602)
...
#if(SLOTPET2)
bool gObjFixEquipInventoryPointer(int aIndex) // OK
{
	...
	if (lpObj->EquipInventory == lpObj->EquipInventory1)
	{
		return 1;
	}

	if (lpObj->EquipInventory == lpObj->EquipInventory2)
	{
...
void gObjSetEquipInventory1Pointer(LPOBJ lpObj) // OK
{
	lpObj->EquipInventory = lpObj->EquipInventory1;
	lpObj->EquipInventoryMap = lpObj->EquipInventoryMap1;
}

void gObjSetEquipInventory2Pointer(LPOBJ lpObj) // OK
{
	lpObj->EquipInventory = lpObj->EquipInventory2;
	lpObj->EquipInventoryMap = lpObj->EquipInventoryMap2;
}
#endif
...
short gObjAdd(SOCKET socket,char* IpAddress,int aIndex) // OK
{
	...
	LogAddConnect(LOG_BLUE, "Đang login - IP: %s",lpObj->IpAddr);
...
short gObjDel(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		if(lpObj->Account[0] != 0){LogAddConnect(LOG_ALERT,"Tài khoản [%s] đã thoát!",lpObj->Account);}

		LogAddConnect(LOG_ALERT,"Xóa IP: %s",lpObj->IpAddr);
...
bool gObjInventoryTransaction(int aIndex) // OK
{
	...
	for (int n = 0; n < EQUIP_INVENTORY_SIZE; n++)
	{
		lpObj->EquipInventory2[n] = lpObj->EquipInventory1[n];
	}
#endif

	memcpy(lpObj->InventoryMap2,lpObj->InventoryMap1,INVENTORY_SIZE);
#if(SLOTPET2)
	memcpy(lpObj->EquipInventory2, lpObj->EquipInventory1, EQUIP_INVENTORY_SIZE);
#endif
...
bool gObjInventoryCommit(int aIndex) // OK
{
	...
	for (int n = 0; n < EQUIP_INVENTORY_SIZE; n++)
	{
		lpObj->EquipInventory1[n] = lpObj->EquipInventory2[n];
	}
#endif
...
#if(SLOTPET2)
	memcpy(lpObj->EquipInventory1, lpObj->EquipInventory2, EQUIP_INVENTORY_SIZE);
#endif
...
#if(BOT_STORE)
	if (lpObj->Type == OBJECT_USER || lpObj->IsBot >= 1)
#else
	if(lpObj->Type == OBJECT_USER)
#endif
	{
	...
	if(lpObj->Interface.type == INTERFACE_TRADE)
	{
		if(lpObj->Interface.state == 0)
		{
			if(OBJECT_RANGE(lpObj->TargetNumber) != 0)
			{
				...
				gObj[lpObj->TargetNumber].TradeOkTime = 0;
				gObj[lpObj->TargetNumber].TradeMoney = 0;
	...
	if(lpObj->Interface.type == INTERFACE_PARTY)
	{
		if(lpObj->Interface.state == 0)
		{
			if(OBJECT_RANGE(lpObj->TargetNumber) != 0)
			{
				...
				gObj[lpObj->TargetNumber].Interface.type = INTERFACE_NONE;
				gObj[lpObj->TargetNumber].Interface.state = 0;
				gObj[lpObj->TargetNumber].InterfaceTime = 0;
				gObj[lpObj->TargetNumber].TargetNumber = -1;
...
void gObjSkillNovaCheckTime(LPOBJ lpObj) // OK
{
	...
	if((GetTickCount()-lpObj->SkillNovaTime) < 5000)
	{
		return;
	}
...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		gCBArenaPVP.UserDieProc(lpObj, lpTarget);
		return;
	}
	#endif
	...
	#if(THANMA)
	if (gThanMaChien.CheckPlayerTarget(lpObj))
	{
		gThanMaChien.UserDieProc(lpObj, lpTarget);
	}
	#endif
	#if(EVENT_PK)
	if (gBsVEvent.CheckPlayerTarget(lpObj))
	{
		gBsVEvent.UserDieProc(lpObj,lpTarget);
	}
	#endif
	...
	#if(NOTICE_PKSYSTEM)
	//Die System User
	if (gServerInfo.m_DieUserSwitch != 0 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		if (gServerInfo.m_DieUserSwitch == 2)
		{
			DWORD PVPColor = 0x0;
			sscanf_s(gServerInfo.PK_ColorMessPVP, "%x", &PVPColor);
			gNotice.GCNoticeSendToAll(eMessagePK, 0, 0, gServerInfo.PK_DelayMess, PVPColor, 0, gMessage.GetMessage(515), lpTarget->Name, lpObj->Name); //
		}
		else
		{
	...
	//Die System Monster
	if (gServerInfo.m_DieMonsterSwitch != 0 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_MONSTER)
	{
		//
		if (gServerInfo.m_DieMonsterSwitch == 2)
		{
			DWORD PVMColor = 0x0;
			sscanf_s(gServerInfo.PK_ColorMessPVM, "%x", &PVMColor);
			gNotice.GCNoticeSendToAll(eMessagePK, 0, 0, gServerInfo.PK_DelayMess, PVMColor, 0, gMessage.GetMessage(516), lpTarget->Name, lpObj->Name); //
		}
		else
		{
...
void gObjPlayerKiller(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	#if(CB_EventArenaPvP)
	if (gCBArenaPVP.CheckEnteredUser(lpObj->Index) && gCBArenaPVP.CheckEnteredUser(lpTarget->Index))
	{
		return;
	}
	#endif
	...
	#if(CTCMINI)
	if (gCTCMini.CheckPlayerTarget(lpObj, lpTarget, 0) != 0)
	{
		return;
	}
	#endif
...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Có lỗi xẩy ra.");
			gPersonalShop.CGPShopCloseRecv(lpObj->Index);
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Hãy thử lại lúc khác!");
			return false;
		}
	}
...
#if (SWAMP_EVENT == 1)
	if (gSwampEvent.GateMove(aIndex, gate) == false){
		goto ERROR_JUMP;
	}
#endif
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_USER)
			{
				...
				#if(DUCTUONG)
				gDucTuong.TuongSend(lpObj->Index);
				#endif				
				...
				if (lpObj->LuckySpinTime > 0 && GetTickCount() - lpObj->LuckySpinTime > 5000)
				{
					...
					gLuckySpin.MakeItem(lpObj->Index, lpObj);
					GC_LuckySpinUpdateData pMsg;
					pMsg.header.set(0xFB, 0x31, sizeof(pMsg));
				...
				if(lpObj->Map == MAP_ARENA && BattleGround == 1)
				{
					...
					#if(CB_EventArenaPvP)
					if(BattleGround != 0 && !gCBArenaPVP.CheckEnteredUser(lpObj->Index))  // CB_EventArenaPvP
					#else
					if (BattleGround != 0)
					#endif
					{
		...
		{
			gScanItemDupe.QuetDupe(lpObj->Index);

			#if(HT_THUE_FLAG)
			if (lpObj->CheckFlag == 1)
			{
				ThueFlag.MainProc(lpObj->Index);
			}
			#endif
		...
		if(lpObj->Connected == OBJECT_CONNECTED ||	lpObj->Connected == OBJECT_LOGGED || lpObj->Connected == OBJECT_ONLINE)
		{
			if(lpObj->Type == OBJECT_USER)
			{
				...
				{
					if(GetTickCount() - lpObj->ConnectTickCount > 180000)
					{
						CloseClient(n);
...
void gObjCustomLogPlusChaosMix(LPOBJ lpObj,int type,int index)
{
	...
	if(gServerInfo.m_AnnounceChaosMix != 0 && type ==0)
	{
		ITEM_INFO ItemInfo;
		...
		if(gItemManager.GetInfo(index,&ItemInfo) != 0)
		{
			...
			if(gServerInfo.m_AnnounceChaosMix == 1)
			{
				...
				wsprintf(buff,gServerInfo.m_AnnounceChaosMix10,lpObj->Name,ItemInfo.Name);
		...
		if(gServerInfo.m_AnnounceChaosMix != 0 && type ==1)
	{
		ITEM_INFO ItemInfo;
		...
		if(gItemManager.GetInfo(index,&ItemInfo) != 0)
		{
			...
			if(gServerInfo.m_AnnounceChaosMix == 1)
			{
				...
				wsprintf(buff,gServerInfo.m_AnnounceChaosMix11,lpObj->Name,ItemInfo.Name);
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,buff);
			}
			else
			{
		...
		if(gServerInfo.m_AnnounceChaosMix != 0 && type ==2)
	{
		ITEM_INFO ItemInfo;
		...
		if(gItemManager.GetInfo(index,&ItemInfo) != 0)
		{
			...
			if(gServerInfo.m_AnnounceChaosMix == 1)
			{
				...
				wsprintf(buff,gServerInfo.m_AnnounceChaosMix12,lpObj->Name,ItemInfo.Name);
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,buff);
			}
			else
			{
		...
		if(gServerInfo.m_AnnounceChaosMix != 0 && type ==3)
	{
		ITEM_INFO ItemInfo;
		...
		if(gItemManager.GetInfo(index,&ItemInfo) != 0)
		{
			...
			if(gServerInfo.m_AnnounceChaosMix == 1)
			{
				...
				wsprintf(buff,gServerInfo.m_AnnounceChaosMix13,lpObj->Name,ItemInfo.Name);
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,buff);
			}
			else
			{
		...
		if(gServerInfo.m_AnnounceChaosMix != 0 && type ==4)
	{
		ITEM_INFO ItemInfo;
		...
		if(gItemManager.GetInfo(index,&ItemInfo) != 0)
		{
			...
			if(gServerInfo.m_AnnounceChaosMix == 1)
			{
				...
				wsprintf(buff,gServerInfo.m_AnnounceChaosMix14,lpObj->Name,ItemInfo.Name);
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,buff);
			}
			else
			{
		...
		if(gServerInfo.m_AnnounceChaosMix != 0 && type ==5)
	{
		ITEM_INFO ItemInfo;
		...
		if(gItemManager.GetInfo(index,&ItemInfo) != 0)
		{
			...
			if(gServerInfo.m_AnnounceChaosMix == 1)
			{
				...
				wsprintf(buff,gServerInfo.m_AnnounceChaosMix15,lpObj->Name,ItemInfo.Name);
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,buff);
			}
			else
			{
...
bool isRecconect(char* address, char* account){
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObj[n].Connected >= OBJECT_CONNECTED && strcmp(gObj[n].Account,account) == 0 && strcmp(gObj[n].IpAddr,address) == 0)
		{
				return true;
		}
	}
