...
enum eCommandNumber
{
	...
	COMMAND_QUYETDAU		= 83,
	COMMAND_QUYETDAUOK		= 84,
	COMMAND_RESET_VIP		= 86,
	#if(PARTYSETPASS)
	COMMAND_SETPASS			= 87,
	#endif
	COMMAND_SETLEVEL		= 88,
...
public:
	...
	bool CommandMakeDur(LPOBJ lpObj,char* arg);
	...
	bool CommandQuyetDau(LPOBJ lpObj,char* arg);// search dual
	bool CommandQuyetDauOK(LPOBJ lpObj,char* arg);// Accept Dual

#if(PARTYSETPASS)
	bool CommandSetPass(LPOBJ lpObj, char* arg);
#endif

	bool CommandSetLevel(LPOBJ lpObj, char* arg);	
	bool CommandSetReset(LPOBJ lpObj, char* arg);
	bool CommandStartBsV(LPOBJ lpObj,char* arg);
