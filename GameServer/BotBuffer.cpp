	...
	try
	{
		while(true)
		{
			...
			while(true)
			{
				...
				this->bot[BotNum].TienTe = lpMemScript->GetAsNumber();
...
BOOL ObjBotBuffer::TradeOpen(int index, int nindex)
{
	...
	if(this->bot[number].TienTe > gObj[index].Coin1)
	{
		gNotice.NewMessageDevTeam(gObj[index].Index,gMessage.GetMessage(524),this->bot[number].Name,this->bot[number].TienTe);
		return 1;		
	}
	...
	for(int x = 0; x < this->bot[number].SkillCount; x ++)
	{
		...
		//gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"%s nhận được thời gian buff là %d phút",lpObj->Name,this->bot[number].skill[x].time);
	}
	...
	if(this->bot[number].Zen > 0)
	{
		...
		//gNotice.NewNoticeSend(gObj[index].Index,0,0,0,0,0,"%s đã trả phí %d Wcoin cho Buff",lpObj->Name,this->bot[number].Zen);
	}
	
	if(this->bot[number].TienTe > 0)
	{
		GDSetCoinSend(lpObj->Index, -(this->bot[number].TienTe), 0, 0,0, 0, "BuffBot");
		gNotice.NewNoticeSend(gObj[index].Index,0,0,0,0,0,"%s đã trả phí %d Wcoin cho Buff",lpObj->Name,this->bot[number].TienTe);
