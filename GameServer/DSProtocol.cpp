...
void DataServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
    ...
    switch(head)
    {
        ...
        case 0x04:
            ...
            break;
        case 0x78:
            DGGlobalItemPostRecv((SDHP_GLOBAL_ITEM_POST_RECV*)lpMsg);
            break;

        case 0x7A:
        switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
        {
            case 0x01:	gRanking.RecvDS((DGCharTop*)lpMsg);					break;
            case 0x02:	gRanking.RecvDSTP((DGCharTopTP*)lpMsg);				break;
            case 0x03:	gRanking.RecvDSET((DGCharTopET*)lpMsg);				break;
            case 0x04:	gRanking.RecvDSTG((DGCharTopTG*)lpMsg);				break;
            case 0x05:	gRanking.RecvDSPK((DGCharTopPK*)lpMsg);				break;
            case 0x06:	gRanking.RecvDSMT((DGCharTopMT*)lpMsg);				break;
        }
        break;
        ...
        {
			#if(HT_THUE_FLAG)
			case 0x05:  ThueFlag.RecvGDGetListBuy((DATAFLAG_LOAD_COUNT*)lpMsg); break;
			#endif
        }
        break;
        ...
        case 0x05:
            switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
            {
                ...
                case 0x71:
                    ...
                    break;
                case 0x76:
                    gWarehouse.DGWarehouseGuildOpenRecv((SDHP_WAREHOUSE_GUILD_STATUS_RECV*)lpMsg);
                    break;
            }
            break;
        case 0x07:
        ...
        case 0x10:
            switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
            {
              ...
              #if CHANGE_PASS
                case 0x01:  DSSENDGS_DOIMK_KQ((DSGS_DOIMK_KQ*)lpMsg);                                   break;
                #endif
            }
            break;
        case 0x11:
        ...
        case 0x27:
            ...
            {
                #if(SLOTPET2)
                case 0x01: g_pNewMyInventory.DGPetInventoryRecv((SDHP_PET_INVENTORY_RECV*)lpMsg);        break;
                #endif
                case 0x00:
        ...
        switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
        {
            case 0x00:	gCustomNpcQuest.DGCustomNpcQuestRecv((SDHP_CUSTOMNPCQUEST_RECV*)lpMsg);				break;
            #if(NGAN_HANG_NGOC)
            case 0x05:	gCustomJewelBank.CustomJewelBankInfoRecv((SDHP_CUSTOM_JEWELBANK_INFO_RECV*)lpMsg);	break;
            #endif
        }
        break;
        ...
        switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
        {
            case 0x40:	gRanking.RecvInfoCharTop((DATA_VIEWTOPRANKING*)lpMsg);	break;
        }
        break;
        //tue add
#if(CHIEN_TRUONG_CO)
        case 0xFC:
            gChienTruongCo.CapNhatTimeCTC((CAPNHAT_CTC_0H*)lpMsg);
            break;
#endif
            //---
    }
...
//----------------------
        ...
        info.pDanhHieu      = lpInfo->pDanhHieu;
        info.pQuanHam       = lpInfo->pQuanHam;
        info.pTuLuyen       = lpInfo->pTuLuyen;
        info.pHonHoan       = lpInfo->pHonHoan;
...
//-------------------------- cosas de pet
        ...
        //-------------------
        else if (TempInventory[8] >= 451 && TempInventory[8] <= 511) // xemu
        {
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
    ...
    //Check nếu thoát ra vào lại thì đá về Fate yêu cầu! SPKCheckMap
    LPOBJ lpObj = &gObj[lpMsg->index];

    if((lpObj->Map == 88 || lpObj->Map == 98)) 
    {
        gObjMoveGate(lpObj->Index, 17); // Gate Loren
    }

    #if(THANMA)
    if (lpObj->Map == gThanMaChien.EventMap) 
    {
    ...
    #if(SLOTPET2)
    g_pNewMyInventory.GDPetInventorySend(lpObj->Index);
    #else
    #if(GAMESERVER_UPDATE>=603)
    ...
    gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, "Giới hạn reset hôm nay là %d lần nha %s!",gServerInfo.GioiHanRS, lpObj->Name);
    ...
    CItem* lpItem2 = &lpObj->Inventory[1];
    ...
    {
        if (GET_ITEM(it->ItemType, it->ItemIndex) == lpItem->m_Index || GET_ITEM(it->ItemType, it->ItemIndex) == lpItem2->m_Index)
        {
            gEffectManager.AddEffect(lpObj, 0, it->EffectBuff, (int)(time(0) + it->TimeBuff), it->DameRate, 0, 0, 0);
    ...
    if(lpObj->CloseCount <= 0)
    {
        ...
        if (gServerInfo.AccountTypeVIP == 1 && lpObj->AccountLevel == 0)
        {
            lpObj->CloseCount = 6;
            lpObj->CloseType = 4;
            return;
    ...
    //tue add
    //==============================================================================================================
    LogAddConnect(LOG_BLUE, "[Obj] [%d] Name (%s)", lpObj->Index, lpObj->Name);
    //======================================================================================
#if(CHIEN_TRUONG_CO)
    gChienTruongCo.SendInfoCTC(lpObj->Index, 0);
#endif
    //----
    ...
    if (info != 0)
    {
        ...
        lpObj->ObtainRepairOn = 1; //TU Sua do
        ...
        LogAdd(LOG_RED, "[FAKE ONLINE][Obj][%d] AddCharacterInfo (%s) OK", lpObj->Index, lpObj->Name);
    }
#endif
#if(BUNDLE_ITEM)
    BundleItem.OnCharacterLogin(lpObj->Index);
#endif
#if(HT_THUE_FLAG)
	ThueFlag.GDGetListBuy(lpObj->Index);
#endif
}
...
void GDSaveTheGiftData(int aIndex)
{
    ...
    pRequest.TheGift = lpObj->ItemStart;
...
void DGCreateItemRecv(SDHP_CREATE_ITEM_RECV* lpMsg) // OK
{
    ...
    else
    {
        ...
        if(gMap[lpMsg->Map].MonsterItemDrop(lpMsg->ItemIndex,lpMsg->Level,lpMsg->Dur,lpMsg->X,lpMsg->Y,lpMsg->Option1,lpMsg->Option2,lpMsg->Option3,lpMsg->NewOption,lpMsg->SetOption,lpMsg->LootIndex,lpMsg->Serial,lpMsg->JewelOfHarmonyOption,lpMsg->ItemOptionEx,lpMsg->SocketOption,lpMsg->SocketOptionBonus,lpMsg->Duration) != 0)
        {
            ...
            #if BAG_EVENT_DROP
            gLog.Output(LOG_ITEMBAGDROP, "%s: (%s: %d/%d) Drop: %s, Serial: %08X, %d %d %d %d %d %d %d %d %d,%d,%d,%d,%d", 
            lpObj->Name, 
            MapSPKDS[lpMsg->Map], 
            lpMsg->X, lpMsg->Y, 
            ...
            lpMsg->SocketOption[5]);
        }
        else
        {
            gLog.Output(LOG_ITEMBAGDROP, "Error:\n%s Map %d: %d/%d NoDrop %s, Serial: %08X, level: %d, Skill: %d, Luck %d, Option3: %d, NewOption: %d, SetOption: %d, JOH: %d, OptionEx: %d, SocketOption:%d,%d,%d,%d,%d \n===============================", lpObj->Name, MapSPKDS[lpMsg->Map], lpMsg->X, lpMsg->Y, gItemManager.GetItemName(lpMsg->ItemIndex), lpMsg->Serial, lpMsg->Level, lpMsg->Option1, lpMsg->Option2,
                lpMsg->Option3, lpMsg->NewOption, lpMsg->SetOption, lpMsg->JewelOfHarmonyOption, lpMsg->ItemOptionEx, lpMsg->SocketOption[1], lpMsg->SocketOption[2], lpMsg->SocketOption[3], lpMsg->SocketOption[4], lpMsg->SocketOption[5]);
        #endif
        }
    }
}
...
void GDCreateItemSend(int aIndex, BYTE map,BYTE x,BYTE y,int index,BYTE level,BYTE dur,BYTE Option1,BYTE Option2,BYTE Option3,int LootIndex,BYTE NewOption,BYTE SetOption,BYTE JewelOfHarmonyOption,BYTE ItemOptionEx,BYTE SocketOption[MAX_SOCKET_OPTION],BYTE SocketOptionBonus,DWORD duration) // OK
{
    ...
    pMsg.Option1 = ((Option1 !=0 )?gItemManager.CheckItemSkill(index):Option1);
...
void GDGlobalItemPostSend(int MapServerGroup, char* name, char* message, char* item_data) // OK
{
    ...
    memcpy(pMsg.item_data, item_data, sizeof(pMsg.item_data));
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
    ...
    pMsg.Money = lpObj->Money; //tue fix
    ...
    #endif
    #if(NEW_VIP_NEW)
    pMsg.rNewVip = lpObj->rNewVip;
    #endif
    #if(B_HON_HOAN)
    pMsg.m_RankTitle5 = lpObj->m_RankTitle5;
    #endif

    #if(HT_THUE_FLAG)
	pMsg.m_RankTitle6 = lpObj->m_RankTitle6;
	pMsg.CheckFlag = 0;
    #endif
    #if RELIFE
    pMsg.rResetLife = lpObj->rResetLife;
    #endif
    #if(B_MOCNAP)
    pMsg.NhanMocNap = lpObj->NhanMocNap;
    #endif
    #if LUC_CHIEN
    pMsg.mLucChien = (DWORD)lpObj->mLucChien;
    #endif
    gDataServerConnection.DataSend((BYTE*)&pMsg,sizeof(pMsg));
    ...
    #endif
#else
    g_pNewMyInventory.GDPetInventorySaveSend(aIndex);
    #endif
}
...
void DGGlobalItemPostRecv(SDHP_GLOBAL_ITEM_POST_RECV* lpMsg) // OK
{
    ...
    memcpy(pMsg.chatid, lpMsg->name, sizeof(pMsg.chatid));

    memcpy(pMsg.chatmsg, lpMsg->message, sizeof(pMsg.chatmsg));
...
void GDSetCoinSend(int aIndex, int value1, int value2, int value3, int value4, int value5, char * LogName) // OK
{
    LPOBJ lpObj = &gObj[aIndex];
    ...
    {
        lpObj->Coin1 += value1;
        gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gSPKMessage.GetMessage(0),value1);
    }
    if (value2 > 0)
    {
        lpObj->Coin2 += value2;
        gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gSPKMessage.GetMessage(1),value2);
    }
    if (value3 > 0)
    {
        lpObj->Coin3 += value3;
        gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gSPKMessage.GetMessage(2),value3);
    }
    if (value4 > 0)
    {
        lpObj->Ruud += value4;
       gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gSPKMessage.GetMessage(3), value4);
    }
    if (value5 > 0)
    {
        lpObj->CoinAtm += value5;
        gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gSPKMessage.GetMessage(4), value5);
    }

    if (value1 < 0)
    {
        lpObj->Coin1 += value1;
        gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gSPKMessage.GetMessage(5),value1);
    }
    if (value2 < 0)
    {
        lpObj->Coin2 += value2;
        gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gSPKMessage.GetMessage(6),value2);
    }
    if (value3 < 0)
    {
        lpObj->Coin3 += value3;
        gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gSPKMessage.GetMessage(7),value3);
    }
    if (value4 < 0)
    {
        lpObj->Ruud += value4;
        gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gSPKMessage.GetMessage(8), value4);
    }
    if (value5 < 0)
    {
        lpObj->CoinAtm += value5;
        gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gSPKMessage.GetMessage(9), value5);
    }
    gCashShop.CGCashShopPointRecv(lpObj->Index);

    if(value1)
    {
        LogAdd(LOG_BLUE,"%s - %s/%s : %d WcoinC",LogName,lpObj->Account,lpObj->Name,value1);
    }
    if(value2)
    {
        LogAdd(LOG_BLUE,"%s - %s/%s : %d WcoinP",LogName,lpObj->Account,lpObj->Name,value2);
    }
    if(value3)
    {
        LogAdd(LOG_BLUE,"%s - %s/%s : %d WcoinG",LogName,lpObj->Account,lpObj->Name,value3);
    }
    if (value4)
    {
        LogAdd(LOG_BLUE, "%s - %s/%s : %d Ruud", LogName, lpObj->Account, lpObj->Name, value4);
    }
    if (value5)
    {
        LogAdd(LOG_BLUE, "%s - %s/%s : %d CoinAtm", LogName, lpObj->Account, lpObj->Name, value5);
    }
}
...
#endif
#if(EVENT_PK)
void GDRankingBsVEventSaveSend(int aIndex, DWORD KillCount, DWORD DeathCount)
{
    LPOBJ lpObj = &gObj[aIndex];

    SDHP_RANKING_BSV_EVENT_SAVE_SEND pMsg;
