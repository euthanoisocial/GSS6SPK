...
void CMonter::CalcUserRank() // OK
{
	...
	for (int n = 0; n < TOPDAME; n++)
	{
		...
		for (int i = 0; i < TOPDAME; i++)
		{
			...
			if (this->Player[n].Dmg > this->Player[i].Dmg)
			{
				rank--;
				continue;
			}
			if (this->Player[n].Dmg == this->Player[i].Dmg && this->Player[n].Regen < this->Player[i].Regen)
			{
				rank--;
				continue;
			}
			if (this->Player[n].Dmg == this->Player[i].Dmg && n < i)
			{
				rank--;
				continue;
			}
...
bool CMonter::DelMember(int aIndex)
{
	for (int i = 0; i < TOPDAME; i++)
	{
...
bool CMonter::AddMember(int aIndex)
{
	...
	if (this->m_TotalPlayer >= TOPDAME)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1069));
		//
		return false;
	}
...
void CMonter::SendBXH(LPOBJ lpObj, int aIndex)
{
	this->CalcUserRank();
	...
	info.monsterid = lpObj->Class;
	info.Level = lpObj->Level;
	info.MinLife = lpObj->Life;
	info.MaxLife = lpObj->MaxLife + lpObj->AddLife;
	info.Rate = (BYTE)((lpObj->Life * 100) / (lpObj->MaxLife + lpObj->AddLife));
	...
	for (int n = 0; n < 10; n++)
	{
		...
		if ((this->Player[this->mIndexRank[n]].IndexQuai) != lpObj->Index)
		{
			continue;
		}
		if (this->Player[this->mIndexRank[n]].Rank > 0 && this->Player[this->mIndexRank[n]].Rank <= 10)
		{
			...
			info.Kill = (QWORD)this->Player[this->mIndexRank[n]].Dmg;
			pMsg.count++;
...
void CMonter::DrawClient(int aIndex)
{
	XULY_CGPACKET pMsg;
	pMsg.header.set(0xA7, 0x16, sizeof(pMsg));
	pMsg.ThaoTac = 1; //1 Open Window , 2 Show Img Clean Wave
	DataSend(aIndex, (BYTE*)& pMsg, pMsg.header.size);
}

void CMonter::AddPlayerPoint(int aIndex, float damage, int Quai)
{
	for (int i = 0; i < TOPDAME; i++)
	{
		if (this->Player[i].Index == aIndex)
		{
			if (gObjIsConnectedGP(this->Player[i].Index) != 0)
				this->Player[i].Dmg = damage;
				this->Player[i].IndexQuai = Quai;
			this->DrawClient(aIndex);
		}
	}	
}

bool CMonter::DelAllMembers()
{
	for (int i = 0; i < TOPDAME; i++)
	{
		this->Player[i].Reset();
...
{
	gCustomMonster.RewardKill(lpObj, lpTarget);
	...
	#if(CTCMINI)
	if (gCTCMini.MonsterDie(lpObj->Index, lpTarget->Index))
	{
		return;
	}
	#endif
	...
	if (gPremiumDrop.DropPremium(lpObj, lpTarget) != 0)
	{
		return;
	}

#if(SWAMP_EVENT == 1) // TEST NUEVO
	gSwampEvent.MonsterDie(lpObj, lpTarget->Index); //FIX Moset Count Swamp Event
#endif
	...
	//tue add
#if(CHIEN_TRUONG_CO)		
	gChienTruongCo.MonsterDie(lpTarget); //<<<<<<<<<<<
#endif

	//-----
	#if BXHDAME
	gMonter.DelMember(lpTarget->Index);
	#endif
	...
	else if(lpObj->Money > 0 && (GetLargeRand()%((lpObj->MoneyRate==0)?1:lpObj->MoneyRate)) < 10)
	{
		...
		if (gZenDrop.IsZenDropActive(lpTarget->Map) == true)
			{
				DWORD dwZenDropMoney = gZenDrop.GetZenAmount(lpTarget->Map, lpTarget->Level);

				if (dwZenDropMoney > 0)
				{
					money = dwZenDropMoney;
					//LogAdd(LOG_RED, "Packet Unk from Client(%d): ZENNNNN: [%d]",lpObj->Index, dwZenDropMoney);
				}
			}
			else
			{
				money = (lpObj->Level*gServerInfo.m_MoneyValue[lpTarget->AccountLevel]) - (rand() % 100);
			}
...
bool gObjSetMonster(int aIndex,int MonsterClass) // OK
{
	...
	if(lpObj->AttackType != 0)
	{
		...
		if(MonsterAttackSkill){
			...
			gSkillManager.AddSkill(lpObj,MonsterAttackSkill->m_Skill2,0);
		}
...
bool gObjSetMonsterBot(int aIndex, int MonsterClass) // OK
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
void gObjMonsterInitHitDamage(LPOBJ lpObj) // OK
{
	for(int n=0;n < MAX_HIT_DAMAGE;n++)
	{
		...
		#if BXHDAME
		lpObj->HitDamage[n].indexquai = -1;
		gMonter.DelAllMembers();
		#endif
	}
...
void gObjMonsterSetHitDamage(LPOBJ lpObj,int aIndex,float damage) // OK
{
	...
	for(int n=0;n < MAX_HIT_DAMAGE;n++)
	{
		...
		{
			lpObj->HitDamage[n].damage = (((lpObj->HitDamage[n].damage + damage) > lpObj->MaxLife) ? (float)lpObj->MaxLife : (lpObj->HitDamage[n].damage + damage));
			lpObj->HitDamage[n].time = GetTickCount();
			#if BXHDAME
			lpObj->HitDamage[n].indexquai = lpObj->Index;
			if(gCustomMonster.GetCustomMonsterDrawBXH(lpObj->Class,lpObj->Map,&CustomMonsterInfo) == 1)
			{
				gMonter.AddMember(aIndex);
				gMonter.AddPlayerPoint(aIndex,lpObj->HitDamage[n].damage, lpObj->HitDamage[n].indexquai);
				gMonter.SendBXH(lpObj,aIndex);
			}
			#endif
			return;
		}
...
int gObjMonsterGetTopHitDamageUser(LPOBJ lpObj) // OK
{
	...
	float TopHitDamage = 0;
	int PartyTopHitDamageUser = -1;
	float PartyTopHitDamage = 0;
