...
struct ITEM_INFO
{
	...
	int HaveOption;
	int DropItem;
	char Name[100]; // 32 // Thay Doi Do Dai Item
	int Level;
	int DamageMin;
	int DamageMax;
	int MagicDamageRate;
...
public:
	...
	#if(BUNDLE_ITEM)
	void GCItemModifySend2(int aIndex,BYTE slot,BYTE mode = 0);
	#endif
	...
	bool CheckItemMoveToEquip(LPOBJ lpObj, CItem* lpItem, int slot);
	int RepairItemEquip(LPOBJ lpObj, CItem* lpItem, int slot, int type);
	void CGItemEquipRepairRecv(PMSG_ITEM_REPAIR_RECV* lpMsg, int aIndex);
	BYTE MoveItemToEquipFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToInventoryFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToEquipFromPersonalShop(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToPersonalShopFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToTradeFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToEquipFromTrade(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToEquipFromChaosBox(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToChaosBoxFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	//
	BYTE MoveItemToEquipFromWarehouse(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToWarehouseFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	//--
#endif
	...
	#if NEWXSHOP
	bool CItemManager::CheckItemRequireClassUnick2(WORD Class, int index);
	#endif
	#if LUC_CHIEN
	int GetItemLevel(int index);
	#endif
};
