...
bool CCustomStore::OnPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg,int aIndex) // OK
{
	...
	if (tick >= lpObj->TimeDelayStore + gServerInfo.m_ThoiGianMuaHang)
	{
		lpObj->TimeDelayStore = GetTickCount();
		int bIndex = MAKE_NUMBERW(lpMsg->index[0],lpMsg->index[1]);
		#if(BOT_STORE)
		if (gObjIsConnectedGP(bIndex) == 0 && gObj[bIndex].IsBot != 6)//BOT_STORE
		{
			return 1;
		}
		#else
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, "Bạn đang thao tác quá nhanh !");
	}
	return 1;
}
...
void CCustomStore::OnPShopBuyItemCallbackRecv(LPOBJ lpObj,LPOBJ lpTarget,DWORD slot,DWORD WCoinC,DWORD WCoinP,DWORD GoblinPoint) // OK
{
	...
	if (GetTickCount() - lpObj->BuyShopTickCount < gServerInfo.m_DelayBuyShop) {
		return;
	}
	lpObj->BuyShopTickCount = GetTickCount();
	...
	//gCashShop.GDCashShopSubPointSaveSend(lpObj->Index,0,PShopWCCValue,PShopWCPValue,PShopWGPValue,0);
	GDSetCoinSend(lpObj->Index, -(PShopWCCValue), -(PShopWCPValue), -(PShopWGPValue), 0, 0, "SellPesonalShopItem");
	...
	gCashShop.GDCashShopAddPointSaveSend(lpTarget->Index,0,PShopWCCValue,PShopWCPValue,PShopWGPValue,0, 0);
