...
int CCustomJewel::GetCustomJewelNewOption(CItem* lpItem, int value) // OK
{
	...
	*/
	...
	if (lpItem->m_Index == GET_ITEM(13, 37)) // Fenrir
	{
		...
		if (lpItem->m_Index == GET_ITEM(13, 200)) // Pét Muun
			if (lpItem->m_Index == GET_ITEM(13, 201)) // Pét Muun
				if (lpItem->m_Index == GET_ITEM(13, 202)) // Pét Muun
					if (lpItem->m_Index == GET_ITEM(13, 203)) // Pét Muun
						if (lpItem->m_Index == GET_ITEM(13, 204)) // Pét Muun
							if (lpItem->m_Index == GET_ITEM(13, 205)) // Pét Muun
								if (lpItem->m_Index == GET_ITEM(13, 206)) // Pét Muun
									if (lpItem->m_Index == GET_ITEM(13, 207)) // Pét Muun
										if (lpItem->m_Index == GET_ITEM(13, 208)) // Pét Muun
											if (lpItem->m_Index == GET_ITEM(13, 209)) // Pét Muun
												if (lpItem->m_Index == GET_ITEM(13, 210)) // Pét Muun
													if (lpItem->m_Index == GET_ITEM(13, 211)) // Pét Muun
														if (lpItem->m_Index == GET_ITEM(13, 212)) // Pét Muun
															if (lpItem->m_Index == GET_ITEM(13, 213)) // Pét Muun
																if (lpItem->m_Index == GET_ITEM(13, 214)) // Pét Muun
																	if (lpItem->m_Index == GET_ITEM(13, 215)) // Pét Muun
																		if (lpItem->m_Index == GET_ITEM(13, 216)) // Pét Muun
																			if (lpItem->m_Index == GET_ITEM(13, 217)) // Pét Muun
																				if (lpItem->m_Index == GET_ITEM(13, 218)) // Pét Muun
																					if (lpItem->m_Index == GET_ITEM(13, 229)) // Pét Muun
																						if (lpItem->m_Index == GET_ITEM(13, 220)) // Pét Muun

																							if (lpItem->m_Index == GET_ITEM(13, 255)) // Sói
																								if (lpItem->m_Index == GET_ITEM(13, 256)) // Sói
																									if (lpItem->m_Index == GET_ITEM(13, 257)) // Sói
																										if (lpItem->m_Index == GET_ITEM(13, 258)) // Sói
																											if (lpItem->m_Index == GET_ITEM(13, 259)) // Sói
																												if (lpItem->m_Index == GET_ITEM(13, 260)) // Sói
																													if (lpItem->m_Index == GET_ITEM(13, 261)) // Sói
																														if (lpItem->m_Index == GET_ITEM(13, 262)) // Sói
																															if (lpItem->m_Index == GET_ITEM(13, 263)) // Sói

																																if (lpItem->m_Index == GET_ITEM(13, 264)) //Ngựa
																																	if (lpItem->m_Index == GET_ITEM(13, 265)) //Ngựa
																																		if (lpItem->m_Index == GET_ITEM(13, 266)) //Ngựa
																																			if (lpItem->m_Index == GET_ITEM(13, 267)) //Ngựa
																																				if (lpItem->m_Index == GET_ITEM(13, 268)) //Ngựa


																																					if (lpItem->m_Index == GET_ITEM(13, 269)) //Ngựa
																																						if (lpItem->m_Index == GET_ITEM(13, 270)) //Ngựa

																																							if (lpItem->m_Index == GET_ITEM(13, 271)) //Ngựa
																																								if (lpItem->m_Index == GET_ITEM(13, 272)) //Ngựa
		...
		for(std::vector<CUSTOM_JEWEL_UPGRADE_INFO>::iterator it=this->m_UpgradeInfo.begin();it != this->m_UpgradeInfo.end();it++)
		{
			if(it->Index == lpInfo->Index)
			{
				if(lpItem->m_Index == it->ItemOld)
				{
					...
					LogAdd(LOG_RED,"[Upgrade] [%s][%s] - (Old: %s, New: %s)",lpObj->Account, lpObj->Name, gItemManager.GetItemName(it->ItemOld), gItemManager.GetItemName(it->ItemNew));
					//gLog.Output(LOG_DISMEMAY, "[Upgrade][%s][%s] - (Old: %s, New: %s)", lpObj->Account, lpObj->Name, gItemManager.GetItemName(it->ItemOld), gItemManager.GetItemName(it->ItemNew));	
				}
			}
