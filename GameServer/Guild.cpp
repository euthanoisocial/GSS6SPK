...
void CGuild::CGGuildListRecv(int aIndex) // OK
{
	...
	if(pMsg.count > 0)
	{
		...
		GuildUpgrade.SendMeInfo(aIndex);
#endif
	}
}
...
void CGuild::CGGuildDeleteRecv(PMSG_GUILD_DELETE_RECV* lpMsg,int aIndex) // OK
{
	...
	if(strcmp(lpObj->Guild->Names[0],lpObj->Name) == 0)
	{
		...
		GuildUpgrade.RemoveDevoteEff(lpObj);
		#endif
		LogAdd(LOG_BLACK," Account:[%s] Name:[%s] Guild Request - Dismiss All : %s",lpObj->Account,lpObj->Name,lpObj->Guild->Name);
	}
	else
	{
		if(strcmp(lpObj->Name,name) != 0)
		...
		GuildUpgrade.RemoveDevoteEff(lpObj);
		#endif
		LogAdd(LOG_BLACK," Account:[%s] Name:[%s] Guild Request - Leave : %s",lpObj->Account,lpObj->Name,lpObj->Guild->Name);
	}
}
