...
void CViewport::GCViewportNewPetSend(int aIndex) // OK
{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		if (gObj[lpObj->VpPlayer[n].index].IsBot == 0) //BOT_STORE
		{
...
{
	#if(CB_EventArenaPvP)
	if (gCBArenaPVP.CheckPlayerJoined(lpObj, lpTarget) == 1)
	{
		return 1;
	}
	#endif
	...
	#if(THANMA)
	if (gThanMaChien.CheckPlayerTarget(lpObj)==1)
	{
		return 1;
	}
	#endif
	#if(EVENT_PK)
	if (gBsVEvent.CheckPlayerTarget(lpObj)==1)
	{
		return 1;
	}
	#endif
	return 0;
	#if(CTCMINI)
	if (gCTCMini.CheckPlayerTarget(lpObj, lpTarget, 0));
	{
		return 1;
	}
	#endif
