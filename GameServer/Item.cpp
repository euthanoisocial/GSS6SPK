...
{
	if ((this->m_Index >= GET_ITEM(12, 36) && this->m_Index <= GET_ITEM(12, 40)) || (this->m_Index == GET_ITEM(12, 43)) || (this->m_Index == GET_ITEM(12, 50)))
	{
		return 0;
	}
...
{
	if ((this->m_Index >= GET_ITEM(12, 36) && this->m_Index <= GET_ITEM(12, 40)) || (this->m_Index == GET_ITEM(12, 43)) || (this->m_Index == GET_ITEM(12, 50)) // Third Wings Prevent Wings Level 2 Attributes
#if (CRYSTAL_EDITION == 1)
		|| (this->m_Index >= GET_ITEM(12, 200) && this->m_Index <= GET_ITEM(12, 254))
#endif
		)
	{
		return 0;
...
bool CItem::IsSocketItem(int type) // OK
{
	if (gServerInfo.m_SocketHarmony == 1 && type == 1)
	{
		return 0;
	}
...
bool CItem::IsJewel()
{
	if (this->m_Index == GET_ITEM(12, 15) || this->m_Index == GET_ITEM(14, 13) || this->m_Index == GET_ITEM(14, 14) || this->m_Index == GET_ITEM(14, 16) ||
		this->m_Index == GET_ITEM(14, 22) || this->m_Index == GET_ITEM(14, 31) || this->m_Index == GET_ITEM(14, 41) || this->m_Index == GET_ITEM(14, 42) ||
		this->m_Index == GET_ITEM(12, 30) || this->m_Index == GET_ITEM(12, 31) || this->m_Index == GET_ITEM(12, 136) || this->m_Index == GET_ITEM(12, 137) ||
		this->m_Index == GET_ITEM(12, 138) || this->m_Index == GET_ITEM(12, 139) || this->m_Index == GET_ITEM(12, 140) || this->m_Index == GET_ITEM(12, 141) ||
		gCustomJewel.CheckCustomJewelByItem(this->m_Index))
	{
		return 1;
	}
...
void CItem::Convert(int index, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NewOption, BYTE SetOption, BYTE JewelOfHarmonyOption, BYTE ItemOptionEx, BYTE SocketOption[MAX_SOCKET_OPTION], BYTE SocketOptionBonus) // OK
{
	...
	if (ItemInfo.RequireLevel != 0)
	{
		...
		//Muun
		else if (gCustomPet.CheckCustomPetByItem(this->m_Index) != 0)
		{
	...
	if (this->m_Index == GET_ITEM(13, 20)) // Transformation Ring
	{
		if (ItemLevel >= 1 && ItemLevel <= 2 )
		{
			this->m_AttackSpeed = 0;
			this->m_DamageMax = 0;
			this->m_MagicDamageRate = 0;
		}
	}
...
#if(HT_THUE_FLAG)
//===Info Buy Flag
CInfoFlagBuy::CInfoFlagBuy() // OK
{
	this->Clear();
}
int CInfoFlagBuy::IsBuy(int FlagID) // OK
{
	if (this->FlagID == FlagID && (this->StatusTime == 0 || this->StatusTime > time(0)))
	{
		return this->StatusTime;
	}
	return -1;
}
void CInfoFlagBuy::Clear() // OK
{
	this->FlagID = -1;
	this->StatusTime = -1;
	this->CountFlagBuy = 0;
...
void CInfoFlagBuy::AddListBuyFlag(int FlagID, int Time) // OK
{
	if (!FlagID) { return; }
	this->FlagID = FlagID;
