...
void CBloodCastle::GiveUserRewardItem(BLOOD_CASTLE_LEVEL* lpLevel,int aIndex) // OK
{
	...
	{
		//Reward
		GDSetCoinSend(lpUser->Index, this->m_BloodCastleRewardValue[lpLevel->Level][0], this->m_BloodCastleRewardValue[lpLevel->Level][1], this->m_BloodCastleRewardValue[lpLevel->Level][2], 0, 0,"BC");
...
void CBloodCastle::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	//if(BC_LEVEL_RANGE(level) == 0) =)) copy tu chung ? 100% :))) laij char k chua tra phi cho tao 

		lpUser->Score += lpObj->Level*(level+1);

		if (gServerInfo.m_BatTatBloodVIP == 1)
		{
			//lpLevel->Map = ((n<7)?(MAP_BLOOD_CASTLE1+n):MAP_BLOOD_CASTLE8);
			//int n = gServerInfo.m_LevelBloodVIP;
			//if(lpObj->Map == MAP_BLOOD_CASTLE1 + n){
			if(lpObj->Map == MAP_BLOOD_CASTLE8){
				lpTarget->ScoreKill++;
				gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(957), lpTarget->ScoreKill,gServerInfo.m_MocKillQuaiBloodVIP);
				if (lpTarget->ScoreKill == gServerInfo.m_MocKillQuaiBloodVIP)
				{
					/*int Blood_Index = 0;
					unsigned char ItemsIndex[5] = {13,14,16,22,41};
					Blood_Index = ItemsIndex[rand()%5];*/
					//GDCreateItemSend(lpTarget->Index, lpObj->Map, lpObj->X, lpObj->Y, GET_ITEM(14, Blood_Index), 0, 0, 0, 0, 0, lpTarget->Index, 0, 0, 0, 0, 0, 0xFF, 0);
					gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_VIP,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
					
				//	gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_VIP,&gObj[lpUser->Index],lpObj->Map, lpObj->X, lpObj->Y);
					this->NoticeSendToAll(&this->m_BloodCastleLevel[level],0,gMessage.GetMessage(958),lpTarget->Name, lpTarget->ScoreKill);
					return;
				}
				if(lpTarget->ScoreKill > gServerInfo.m_MocKillQuaiBloodVIP)
				{lpTarget->ScoreKill = 0; Sleep(500);}
			}
		}
		//======Blood VIP danh quai roi ngoc
}
