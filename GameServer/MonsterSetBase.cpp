...
#if(THANMA)
void CMonsterSetBase::LoadAditionalMonster()
{
	//==== NPC Event Than Ma Chien
	if (gThanMaChien.m_ThanMaChienEnable)
	{
		//LogAdd(LOG_RED, "[SinhTon]Set NPC OK!");
		MONSTER_SET_BASE_INFO info;
		memset(&info, 0, sizeof(info));

		info.Type = 0;
		info.MonsterClass = gThanMaChien.m_ThanMaChienNPC;
		info.Map = gThanMaChien.m_ThanMaChienNPCMap;
		info.Dis = 0;
		info.Dir = gThanMaChien.m_ThanMaChienNPCDir;
		info.X = gThanMaChien.m_ThanMaChienNPCX;
		info.Y = gThanMaChien.m_ThanMaChienNPCY;
