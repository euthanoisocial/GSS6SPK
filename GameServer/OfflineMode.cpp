...
void OfflineMode::ReadOffLine(char* section,char* path)
{
	...
	this->m_MUOffHelperPickUpEnabled[0] = GetPrivateProfileInt(section, "MuOffHelperPickUpEnabled_AL0", 0, path);
	this->m_MUOffHelperPickUpEnabled[1] = GetPrivateProfileInt(section, "MuOffHelperPickUpEnabled_AL1", 0, path);
	this->m_MUOffHelperPickUpEnabled[2] = GetPrivateProfileInt(section, "MuOffHelperPickUpEnabled_AL2", 0, path);
	this->m_MUOffHelperPickUpEnabled[3] = GetPrivateProfileInt(section, "MuOffHelperPickUpEnabled_AL3", 0, path);
...
void OfflineMode::Start(CG_OFFMODE_RESULT* aRecv, int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"[OffHelper] Đã Tắt");
		return;
	}
	...
	case MAP_INDEX_CASTLESIEGE:
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "[OfflineMode] Admin Đã Tắt Treo Off Máp Này");
		return;
	...
	{
		if (this->bOnlyLorenMarket != 0 && lpObj->Map != 79)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "[OffTrade] OffTrade work only Loren Market");
			return;
		}
		else
		{
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[OfflineTrade][%s]", lpObj->Name);
		}
	}*/
	//===========
...
{	
	if(g_OfflineMode.m_MUOffHelperPickUpEnabled[0] == 0 || g_OfflineMode.m_MUOffHelperPickUpEnabled[1] == 0 || g_OfflineMode.m_MUOffHelperPickUpEnabled[2] == 0 || g_OfflineMode.m_MUOffHelperPickUpEnabled[3] == 0)
	{
		return 0;
	}
...
{	
	...
	if(g_OfflineMode.m_MUOffHelperPickUpEnabled[lpObj->AccountLevel] == 0)
	{
		return 0;
	}
