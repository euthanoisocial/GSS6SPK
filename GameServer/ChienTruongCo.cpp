...
void cChienTruongCo::MonsterDie(LPOBJ Player)
{
	...
	if (Player->Map == 97 && Player->CTCTime > 0) {
		...
		if (this->PointKillMonter > 0)
		{
			if (this->LoaiTien == 0)
			{
				gCashShop.GDCashShopAddPointSaveSend(Player->Index, 0, this->PointKillMonter, 0, 0, 0,0);
				//-- WCoinC Update
				gNotice.GCNoticeSend(Player->Index, 1, 0, 0, 0, 0, 0, this->MsgTPoint, this->PointKillMonter);
			}
			if (this->LoaiTien == 1)
			{
				gCashShop.GDCashShopAddPointSaveSend(Player->Index, 0, 0, this->PointKillMonter, 0, 0,0);
				//-- WCoinP Update
				gNotice.GCNoticeSend(Player->Index, 1, 0, 0, 0, 0, 0, this->MsgTPoint, this->PointKillMonter);
			}
			if (this->LoaiTien == 2)
			{
				gCashShop.GDCashShopAddPointSaveSend(Player->Index, 0, 0, 0, this->PointKillMonter, 0,0);
				//-- GobilCoin Update
				gNotice.GCNoticeSend(Player->Index, 1, 0, 0, 0, 0, 0, this->MsgTPoint, this->PointKillMonter);
			...
			if (this->LoaiTien == 3)
			{
				gCashShop.GDCashShopAddPointSaveSend(Player->Index, 0, 0, 0, 0, this->PointKillMonter,0);
				//-- Ruud Update
				gNotice.GCNoticeSend(Player->Index, 1, 0, 0, 0, 0, 0, this->MsgTPoint, this->PointKillMonter);
		...
		if ((OBJECT_RANGE(Player->PartyNumber) != 0) && this->PartyKillPoint > 0)
		{
			...
			for (int n = 0; n < MAX_PARTY_USER; n++)
			{
				if (OBJECT_RANGE(lpParty->Index[n]) != 0 && Player->Index != lpParty->Index[n])
				{
					...
					{
						gCashShop.GDCashShopSubPointSaveSend(lpParty->Index[n], 0, 0, 0, 0, this->PartyKillPoint,0);
						//-- Ruud Update
...
void cChienTruongCo::CGPacketCTC(XULY_CGPACKET* aRecv, int aIndex)
{
	...
	if ((GetTickCount() - lpObj->ClickClientSend) < 2000) {
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn thao tác qua nhanh!");
		return;
	}
	...
	{
		gObjMoveGate(lpObj->Index, 850); //sua gate nay
	...
	else if (ThaoTac == 2)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Vượt số lần mua", this->MaxBuyDay);
			return;
		}
		if (lpObj->Coin1 < this->WCoinBuy) {
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Không đủ WcoinC", this->WCoinBuy);
			return;
		}
		GDSetCoinSend(lpObj->Index, -(this->WCoinBuy), 0, 0,0,0, "Đã trừ WcoinC");
		lpObj->CTCTime += (this->TimeBuyMin * 60);
		lpObj->CTCRegDay++;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Mua thời gian thành công", this->TimeBuyMin);
