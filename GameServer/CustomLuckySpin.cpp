...
{
	this->Init();
	this->Read(gPath.GetFullPath("SPK\\LuckySpin.txt"));
}
...
void CLuckySpin::StartRoll(LPOBJ lpObj)
{
	...
	{
		int CurrentWC = this->TicketWcoinC - lpObj->Coin1;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn còn thiếu %d WC nữa để quay!", CurrentWC);
		return;
	}
	this->DeleteItemCount(lpObj->Index, this->TicketCount);
	GDSetCoinSend(lpObj->Index, -(this->TicketWcoinC), 0, 0, 0, 0, "LuckSpin");
...
void CLuckySpin::MakeItem(int aIndex, LPOBJ lpObj)
{
	...
	int randomNumber = rand() % totalRate;
	...
	WORD ItemIndex = 0;
	ItemIndex = GET_ITEM(this->itemdata[number].ItemType, this->itemdata[number].ItemIndex, this->itemdata[number].ItemLevel);
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(2002), lpObj->Name, gItemManager.GetItemName(ItemIndex));

	char szTemp[256];
	wsprintf(szTemp, "[%s] Nhận được [%s] ", lpObj->Name, gItemManager.GetItemName(ItemIndex));
	GDGlobalPostSend(gMapServerManager.GetMapServerGroup(), 1, "VQMM", szTemp);
	gLog.Output(LOG_TRADE, "[VQMM] Tài Khoản:[%s] Nhân Vật:[%s] - Tiền: [%d] - Phần Phưởng: [%s]", lpObj->Account, lpObj->Name, this->TicketWcoinC, gItemManager.GetItemName(ItemIndex));
}
#endif