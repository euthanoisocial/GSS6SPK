		...
		if (lpInfo->State == DA_STATE_EMPTY)
		{
			...
			if (gServerInfo.m_TienTrinhQuyetDau == 1)
			{
				if (gServerInfo.m_DuelArenaAnnounceSwitch != 0){
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(939), lpObj->Name, lpTarget->Name, gServerInfo.m_TienCuocQuyetDau);
				}
				gServerInfo.m_TienTrinhQuyetDau = 2; //gan tien trinh quyet dau

			}
			else if (gServerInfo.m_DuelArenaAnnounceSwitch != 0){
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(174), lpObj->Name, lpTarget->Name);
				gServerInfo.m_TienTrinhQuyetDau = 3; //gan tien trinh quyet dau
			}
...
void CDuel::CheckDuelScore(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	if (gServerInfo.m_TienTrinhQuyetDau == 2)
	{
		if (lpObj->DuelScore >= gServerInfo.m_SoTranChamChien) // 
		{
			...
			if (lpInfo != 0)
			{
				...
				int tienbitru = ((float)gServerInfo.m_Tylenhantien / 100) * gServerInfo.m_TienCuocQuyetDau;

				if (gServerInfo.m_LoaiTienNhan == 1){
					GDSetCoinSend(lpObj->Index, gServerInfo.m_TienCuocQuyetDau - tienbitru, 0, 0, 0, 0, "QuyetDau"); //
					GDSetCoinSend(lpTarget->Index, -gServerInfo.m_TienCuocQuyetDau, -0, -0, 0, 0, "QuyetDau"); // 
					gServerInfo.m_TienTrinhQuyetDau = 0; //
					if (gServerInfo.m_DuelArenaAnnounceSwitch != 0){
						gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(940), lpObj->Name, lpTarget->Name, gServerInfo.m_TienCuocQuyetDau - tienbitru);
					}
				}
				else if (gServerInfo.m_LoaiTienNhan == 2)
				{
					GDSetCoinSend(lpObj->Index, 0, gServerInfo.m_TienCuocQuyetDau - tienbitru, 0, 0, 0, "QuyetDau"); //
					GDSetCoinSend(lpTarget->Index, -0, -gServerInfo.m_TienCuocQuyetDau, -0, 0, 0, "QuyetDau"); // 
					gServerInfo.m_TienTrinhQuyetDau = 0; //
					if (gServerInfo.m_DuelArenaAnnounceSwitch != 0){
						gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(951), lpObj->Name, lpTarget->Name, gServerInfo.m_TienCuocQuyetDau - tienbitru);
					}
				}
				else if (gServerInfo.m_LoaiTienNhan == 3)
				{
					GDSetCoinSend(lpObj->Index, 0, 0, gServerInfo.m_TienCuocQuyetDau - tienbitru, 0, 0, "QuyetDau"); //
					GDSetCoinSend(lpTarget->Index, 0, -0, -gServerInfo.m_TienCuocQuyetDau, 0, 0, "QuyetDau"); // 
					gServerInfo.m_TienTrinhQuyetDau = 0; //
					if (gServerInfo.m_DuelArenaAnnounceSwitch != 0){
						gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(952), lpObj->Name, lpTarget->Name, gServerInfo.m_TienCuocQuyetDau - tienbitru);
					}
				}
				else {
	...
	else if (gServerInfo.m_TienTrinhQuyetDau == 3)
	{
...
void CDuel::CGDuelOkRecv(PMSG_DUEL_OK_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpMsg->flag == 0 || lpObj->Interface.use != 0 || lpTarget->Interface.use != 0) // fix dupe 
	{
