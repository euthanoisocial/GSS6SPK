		...
		while (true)
		{
			...
			while (true)
			{
				...
				else if (section == 1)
				{
					...
					//Igk Dev - Alarm Time <- Agregar las 2 siguientes lineas
...
void CInvasionManager::ProcState_EMPTY(INVASION_INFO* lpInfo) // OK
{
	...
	if (lpInfo->RemainTime > 0 && lpInfo->RemainTime <= (lpInfo->AlarmTime * 60))
	{
		if ((lpInfo->AlarmMinSave = (((lpInfo->RemainTime % 60) == 0) ? ((lpInfo->RemainTime / 60) - 1) : (lpInfo->RemainTime / 60))) != lpInfo->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(960), lpInfo->AlertMessage, (lpInfo->AlarmMinLeft + 1));
	...
	for (int n = 0; n < MAX_INVASION_RESPAWN_GROUP; n++)
	{
		if (lpInfo->RespawnInfo[n].empty() == 0)
		{
			...
			for (std::vector<INVASION_MONSTER_INFO>::iterator it = lpInfo->MonsterInfo.begin(); it != lpInfo->MonsterInfo.end(); it++)
			{
				...
				{
					#if SO_LUONG_BOSS
					BossInfo& bossInfo = classBossInfo[it->MonsterClass];
					bossInfo.bossindex = it->MonsterClass;
					#endif
...
void CInvasionManager::ClearMonster(INVASION_INFO* lpInfo) // OK
{
	for (int n = 0; n < MAX_INVASION_MONSTER; n++)
	{
		if (OBJECT_RANGE(lpInfo->MonsterIndex[n]) != 0)
		{
			...
			gActiveInvasions.monster_del(lpObj->Class);
			BossInfo& bossInfo = classBossInfo[lpObj->Class];
			bossInfo.BossRegen = false;
			bossInfo.BossMap = -1;
			bossInfo.BossCount = 0;
...
void CInvasionManager::SetMonster(INVASION_INFO* lpInfo, INVASION_RESPWAN_INFO* lpRespawnInfo, INVASION_MONSTER_INFO* lpMonsterInfo) // OK
{
	for (int n = 0; n < gMonsterSetBase.m_count; n++)
	{
		...
		#if SO_LUONG_BOSS
		if (lpObj->Class == lpMonsterInfo->MonsterClass && lpMonsterInfo->RegenType == 0)
		{
			gActiveInvasions.monster_add(lpObj->Class);
		}
		else if (lpObj->Class == lpMonsterInfo->MonsterClass && lpMonsterInfo->RegenType != 0)
		{
			this->CheckBossCount(lpObj->Class, lpObj->Map);
		}
		#endif
		...
		if (lpObj->Class == lpInfo->BossIndex)
		{
			...
			#if REMOVE_DRAGONFLY
			if (gServerInfo.m_FlyingDragonsSwitch == 1 && gServerInfo.m_FlyingDragonsOnlyBossMapSwitch == 1) 
			{
		...
		if (gServerInfo.m_FlyingDragonsSwitch == 1 && gServerInfo.m_FlyingDragonsOnlyBossMapSwitch == 0) 
		{
			if (lpInfo->Index == 1 || lpInfo->Index == 2) {
				gDragonMaps.AddFlyingDragons(lpObj->Map, lpInfo->InvasionTime, ((lpInfo->Index * 2) - 1));
			}
...
#if SO_LUONG_BOSS
void CInvasionManager::CheckBossCount(int classValue, int mapNumber)
{
	INVASION_MONSTER_INFO* lpMonsterInfo;
	BossInfo& bossInfo = classBossInfo[classValue];

	bossInfo.BossRegen = true;
	bossInfo.BossMap = mapNumber;
	bossInfo.BossCount++;

	if (bossInfo.BossRegen == false || bossInfo.bossindex != classValue)
	{
		return;
	}
	if (bossInfo.BossMap != mapNumber)
	{
		return;
	}
	#if REMOVE_DRAGONFLY
	if (bossInfo.BossCount > 0)
	{
		GCMapEventStateSend(bossInfo.BossMap, 1, 3);
		gActiveInvasions.monster_add(classValue, true);
	}
	else
	{
		GCMapEventStateSend(bossInfo.BossMap, 0, 3);
	}
	#else
	if (bossInfo.BossCount > 0)
	{
