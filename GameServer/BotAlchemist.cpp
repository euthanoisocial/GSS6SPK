...
BYTE ObjBotAlchemist::Alchemy(int aIndex, int BotNum)
{
	...
	{
		wsprintf(sbuf, "Bạn cần đủ %d Zen!", this->bot[BotNum].Zen);
	...
	{
		if (lpObj->Coin2 < this->bot[BotNum].mWcoinP)
		{
			int ThieuWP = this->bot[BotNum].mWcoinP - lpObj->Coin2;
			if(ThieuWP)
			{
				goto Cancel;
			}
	...
	{
		if (lpObj->Coin1 < this->bot[BotNum].mWcoinC)
		{
			int ThieuWC = this->bot[BotNum].mWcoinC - lpObj->Coin1;
			if(ThieuWC)
			{
				goto Cancel;
			}
	...
	if (count != 2)
	{
		ChatSend(&gObj[this->bot[BotNum].index], "Yêu cầu 2 món cùng loại.");
		goto Cancel;
	}
	if ((lpObj->Trade[fitem].m_Index >= 6144) || (lpObj->Trade[sitem].m_Index >= 6144))
	{
		ChatSend(&gObj[this->bot[BotNum].index], "Chỉ yêu cầu vũ khí");
		goto Cancel;
	}

	if (fitem == -1 || sitem == -1)
	{
		ChatSend(&gObj[this->bot[BotNum].index], "Chưa đặt đúng vị trí");
		goto Cancel;
	}
	...
	if (this->bot[BotNum].OnlySameType == 1)
	{
		...
		if (fType != sType)
		{
			ChatSend(&gObj[this->bot[BotNum].index], "Yêu cầu item cùng loại.");
			goto Cancel;
		}
	}
	...
	if (this->bot[BotNum].OnlyLowerIndex == 1)
	{
		...
		if (fLevel > sLevel)
		{
			if (fLevel > (sLevel + 10))
			{
				ChatSend(&gObj[this->bot[BotNum].index], "Yêu cầu item cùng cấp!");
	...
	if (this->bot[BotNum].OnlySameType == 1)
	{
		...
		if (fType != sType)
		{
			ChatSend(&gObj[this->bot[BotNum].index], "Không thể cộng đồ này.");
			goto Cancel;
		}
	}
	...
	{
		ChatSend(&gObj[this->bot[BotNum].index], "Không thể cộng đồ thần.");
		goto Cancel;
	}
	...
	if (a == 255 || b == 255)
	{
		ChatSend(&gObj[this->bot[BotNum].index], "Thùng đồ đã đầy");
		goto Cancel;
	}
...
BOOL ObjBotAlchemist::TradeOpen(int index, int nindex)
{
	...
	{
		ChatSend(&gObj[this->bot[number].index], "Bạn không phải là VIP");
		return 0;
	}
	if (this->Enabled == TRUE)
	{
		...
		else
		{
			...
			char wbuf[1024] = { 0 };
			wsprintf(wbuf, "(%s)(%s) đã mở Cộng Hưởng", gObj[index].Account, gObj[index].Name);
			LogAdd(LOG_RED, wbuf);
			ChatSend(&gObj[this->bot[number].index], "Tôi đã sẵn sàng!");

			if (this->bot[number].mWcoinP > 0 && lpObj->Coin2 < this->bot[number].mWcoinP)
			{
				this->IsCoinP = this->bot[number].mWcoinP - lpObj->Coin2;
				if(this->IsCoinP)
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Cần %d WP mới đủ %d WP cho giao dịch!", 
					this->IsCoinP,
					this->bot[number].mWcoinP);
				}				
			}

			if (this->bot[number].mWcoinC > 0 && lpObj->Coin1 < this->bot[number].mWcoinC)
			{
				this->IsCoinC = this->bot[number].mWcoinC - lpObj->Coin1;
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Cần %d WC mới đủ %d WC cho giao dịch!", 
				this->IsCoinC,
			...
			if (this->bot[number].Zen > 0 && lpObj->Money < this->bot[number].Zen)
			{
				...
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Cần %d Zen mới đủ %d Zen cho giao dịch!", 
				this->IsMoney,
				this->bot[number].Zen);
			}
		}
