...
int CItemManager::GetItemDurability(int index,int level,int NewOption,int SetOption) // OK
{
	...
	#if(BUNDLE_ITEM)
	if (BundleItem.IsBundleItem(index, level))
	{
		return 1;
	}
	#endif
	...
	if(index != GET_ITEM(0,19) && ItemInfo.Slot != 7) // Sword of Archangel,Scepter of Archangel,Crossbow of Archangel,Staff of Archangel,Stick of Archangel
	{
		if(SetOption != 0)
		{
			dur += 20;
...
int CItemManager::GetInventoryItemCount(LPOBJ lpObj,int index,int level) // OK
{
	...
	for(int n=0;n < MaxValue;n++)
	{
		if(lpObj->Inventory[n].IsItem() != 0)
		{
			...
			{
				#if(BUNDLE_ITEM)
				if(BundleItem.IsBundleItem(lpObj->Inventory[n].m_Index, level) == 0)
				#else
...
bool CItemManager::CheckItemMoveToChaos(LPOBJ lpObj,CItem* lpItem,BYTE TargetFlag) // OK
{
	...
	if (BundleItem.IsBundleItem(lpItem->m_Index, lpItem->m_Level) && lpItem->m_Durability >= 2)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 2, 0, "hãy tách ItemStack ra trước khi bỏ vào máy chaos");
		return 0;
	}
		...
		if(lpObj->Inventory[n].IsItem() != 0)
		{
			if(lpObj->Inventory[n].m_Index == lpItem->m_Index && lpObj->Inventory[n].m_Level == lpItem->m_Level && lpObj->Inventory[n].m_SocketOptionBonus == lpItem->m_SocketOptionBonus)
			{
				if(lpObj->Inventory[n].m_Durability < MaxStack)
				{
					...
					{
						#if(BUNDLE_ITEM)
						std::map<int,ITEM_STACK_INFO>::iterator it = gItemStack.m_ItemStackInfo.find(lpItem->m_Index);
						if(it->second.ItemLevel != -1)
						{
							this->InventoryDelItem(lpObj->Index,n);
							this->GCItemDeleteSend(lpObj->Index,n,1);
							GDCreateItemSend(lpObj->Index,0xEB,0,0,CreateItemIndex,it->second.ItemLevel,1,0,0,0,lpObj->Index,0,0,0,0,0,lpItem->m_SocketOptionBonus,0);
...
bool CItemManager::ConvertItemByte(CItem* lpItem,BYTE* lpMsg) // OK
{
	...
	if (lpItem->m_JewelOfHarmonyOption == 0xFF)
	{
...
void CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	...
	#if(BUNDLE_ITEM)
	if (BundleItem.InventoryInsertItemStack(lpObj, &item) != 0)
	{
		gMap[lpObj->Map].ItemGive(aIndex, index);
		pMsg.result = 0xFD;
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg,int aIndex) // OK
{
	...
	if( DropItemIndex != 0 )
	{
		...
		{
			#if(BUNDLE_ITEM)
			int DropStack = BundleItem.IsBundleItemSlot(lpObj->Index, lpMsg->slot);
			if (DropStack == 0)
			{
			...
			{
				this->DecreaseItemDur(lpObj, lpMsg->slot, DropStack);
				gItemManager.GCItemModifySend(lpObj->Index, lpMsg->slot);
				return;
			}
			#else
			this->InventoryDelItem(aIndex,lpMsg->slot);
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 0) // Inventory -> Inventory
	{
		...
		if ((pMsg.result = this->MoveItemToInventoryFromInventory(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			...
			CustomStartItemDame_INFO* lpInfo = &gCustomStartItemDame.m_CustomStartItemDameInfo[lpObj->Class];

			for (std::vector<CUSTOM_START_ITEM_DAME_INFO>::iterator it = lpInfo->ItemDameStart.begin(); it != lpInfo->ItemDameStart.end(); it++)
			{
				if (GET_ITEM(it->ItemType, it->ItemIndex) == lpItem->m_Index || GET_ITEM(it->ItemType, it->ItemIndex) == lpItem1->m_Index)
				{
					gEffectManager.AddEffect(lpObj, 0, it->EffectBuff, (int)(time(0) + it->TimeBuff), it->DameRate, it->DupeDame, it->TripDame, 0);
					lpObj->TrashCodeItemId = GET_ITEM(it->ItemType, it->ItemIndex);
				}
				else
				{
					if (lpMsg->SourceSlot == 0 && (lpObj->TrashCodeItemId == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemId == -1))
					{
						gEffectManager.DelEffect(lpObj, it->EffectBuff);			
					}
					lpObj->TrashCodeItemId = -1;
				}
				regalo++;
			}
			...
			for (std::vector<CustomStartSetItemDame_Data>::iterator it = gCustomStartSetItemDame.itemdata.begin(); it != gCustomStartSetItemDame.itemdata.end(); it++)
			{
				...
				if (lpItemHelm->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->Defense, it->IncResistCriticalDamage, it->IncResisteExcellentDamage);
						lpObj->TrashCodeItemSet2 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				else
				{
					if (lpMsg->SourceSlot == 2 && (lpObj->TrashCodeItemSet2 == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemSet2 == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemSet2 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					lpObj->TrashCodeItemId = -1;
				}
				...
				if (lpItemArmor->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->Defense, it->IncResistCriticalDamage, it->IncResisteExcellentDamage);
						lpObj->TrashCodeItemSet3 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				else
				{
					if (lpMsg->SourceSlot == 3 && (lpObj->TrashCodeItemSet3 == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemSet3 == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemSet3 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
				...
				if (lpItemPant->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->Defense, it->IncResistCriticalDamage, it->IncResisteExcellentDamage);
						lpObj->TrashCodeItemSet4 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				else
				{
					if (lpMsg->SourceSlot == 4 && (lpObj->TrashCodeItemSet4 == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemSet4 == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemSet4 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
				...
				if (lpItemGloves->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->Defense, it->IncResistCriticalDamage, it->IncResisteExcellentDamage);
						lpObj->TrashCodeItemSet5 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				else
				{
					if (lpMsg->SourceSlot == 5 && (lpObj->TrashCodeItemSet5 == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemSet5 == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemSet5 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
				...
				if (lpItemBoot->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->Defense, it->IncResistCriticalDamage, it->IncResisteExcellentDamage);
						lpObj->TrashCodeItemSet6 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				else
				{
					if (lpMsg->SourceSlot == 6 && (lpObj->TrashCodeItemSet6 == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemSet6 == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemSet6 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
				...
				if (lpItemWing->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->Dame, it->IncLife, it->ExcellentDamageRate, it->CriticalDamageRate);
						lpObj->TrashCodeItemWing = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				else
				{
					if (lpMsg->SourceSlot == 7 && (lpObj->TrashCodeItemWing == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemWing == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemWing = GET_ITEM(it->ItemType, it->ItemIndex);
					}
				...
				if (lpItemKhien->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->IncResistIgnoreDefense, it->IncReflectRate, it->IncBlockStuck);
						lpObj->TrashCodeItemKhien = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				else
				{
					if (lpMsg->SourceSlot == 1 && (lpObj->TrashCodeItemKhien == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemKhien == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemKhien = GET_ITEM(it->ItemType, it->ItemIndex);
					}
				...
				if (lpItemPen->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->Dame, it->Defense, it->IncLife, 0);
						lpObj->TrashCodeItemPen = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				...
				{
					if (lpMsg->SourceSlot == 9 && (lpObj->TrashCodeItemPen == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemPen == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemPen = GET_ITEM(it->ItemType, it->ItemIndex);
					}
				...
				if (lpItemRing1->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->Dame, it->Defense, it->IncLife, 0);
						lpObj->TrashCodeItemRing1 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				else
				{
					if (lpMsg->SourceSlot == 10 && (lpObj->TrashCodeItemRing1 == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemRing1 == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemRing1 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
				...
				if (lpItemRing2->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
				{
					...
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->Dame, it->Defense, it->IncLife, 0);
						lpObj->TrashCodeItemRing2 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
					regalo++;
				}
				else
				{
					if (lpMsg->SourceSlot == 11 && (lpObj->TrashCodeItemRing2 == GET_ITEM(it->ItemType, it->ItemIndex) || lpObj->TrashCodeItemRing2 == -1))
					{
						gEffectManager.DelEffect(lpObj, it->Effect);
						lpObj->TrashCodeItemRing2 = GET_ITEM(it->ItemType, it->ItemIndex);
					}
	...
	else if((lpMsg->SourceFlag == 3 || (lpMsg->SourceFlag >= 5 && lpMsg->SourceFlag <= 20)) && lpMsg->TargetFlag == 0) // Chaos Box -> Inventory
	{
		...
		{
			this->ItemByteConvert(pMsg.ItemInfo,lpObj->Inventory[lpMsg->TargetSlot]);
			gLog.Output(LOG_CHAOS_MIX," Serial: %08X",lpObj->Inventory[lpMsg->TargetSlot].m_Serial);
		}
	}
	...
	#else
	else if (lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 40) // Inventory -> Inventory Equip
	{
		if ((pMsg.result = this->MoveItemToEquipFromInventory(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->EquipInventory[lpMsg->TargetSlot]);
		}
	}
	else if (lpMsg->SourceFlag == 1 && lpMsg->TargetFlag == 40) // Trade -> Inventory Equip
	{
		if ((pMsg.result = this->MoveItemToEquipFromTrade(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->EquipInventory[lpMsg->TargetSlot]);
		}
	}
	else if (lpMsg->SourceFlag == 4 && lpMsg->TargetFlag == 40) // Personal Shop -> Inventory Equip
	{
		if ((pMsg.result = this->MoveItemToEquipFromPersonalShop(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->EquipInventory[lpMsg->TargetSlot]);
		}
	}
	else if ((lpMsg->SourceFlag == 3 || (lpMsg->SourceFlag >= 5 && lpMsg->SourceFlag <= 20)) && lpMsg->TargetFlag == 40) // Chaos Box -> Inventory Equip
	{
		if ((pMsg.result = this->MoveItemToEquipFromChaosBox(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->EquipInventory[lpMsg->TargetSlot]);
		}
	}
	else if (lpMsg->SourceFlag == 40 && lpMsg->TargetFlag == 0) // Inventory Equip -> Inventory
	{
		if ((pMsg.result = this->MoveItemToInventoryFromEquip(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->Inventory[lpMsg->TargetSlot]);
		}
	}
	else if (lpMsg->SourceFlag == 40 && lpMsg->TargetFlag == 1) // Muun Inventory -> Trade
	{
		if ((pMsg.result = this->MoveItemToTradeFromEquip(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->Trade[lpMsg->TargetSlot]);
		}
	}
	else if (lpMsg->SourceFlag == 40 && lpMsg->TargetFlag == 4) // Inventory Equip -> Personal Shop
	{
		if ((pMsg.result = this->MoveItemToPersonalShopFromEquip(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->Inventory[lpMsg->TargetSlot]);
		}
	}
	else if (lpMsg->SourceFlag == 40 && (lpMsg->TargetFlag == 3 || (lpMsg->TargetFlag >= 5 && lpMsg->TargetFlag <= 20))) // Inventory Equip-> Chaos Box
	{
		if ((pMsg.result = this->MoveItemToChaosBoxFromEquip(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->ChaosBox[lpMsg->TargetSlot]);
		}
	}
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg,int aIndex) // OK
{
	...
	#if(BUNDLE_ITEM)
	if (BundleItem.CGStackItem(aIndex, lpMsg->SourceSlot, lpMsg->TargetSlot))
	{
		return;
	}
	#endif
	...
	else if(lpItem->m_Index == GET_ITEM(13,66)) // Vé San Ta
	{
	...
	else if(lpItem->m_Index == GET_ITEM(14,151)) // Vé sóc đĩa
	{
		int ChanLe = 0;
		...
		if(gServerInfo.m_LoaiTienMoBat == 1)
		{

				if(lpObj->Coin1  > gServerInfo.m_PhiMoBat)
				{
					GDSetCoinSend(lpObj->Index, -gServerInfo.m_PhiMoBat, 0, 0, 0, 0, "");
				}
				else
				{
					gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(870),lpObj->Coin1,gServerInfo.m_PhiMoBat);
					return;
				}
				
		}
		else if(gServerInfo.m_LoaiTienMoBat == 2)
		{

				if(lpObj->Coin2  > gServerInfo.m_PhiMoBat)
				{
					GDSetCoinSend(lpObj->Index,0,-gServerInfo.m_PhiMoBat, 0, 0, 0, "");
				}
				else
				{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(871),lpObj->Coin2,gServerInfo.m_PhiMoBat);
				return;
				}
				
		}
		else if(gServerInfo.m_LoaiTienMoBat == 3)
		{

				if(lpObj->Coin3  > gServerInfo.m_PhiMoBat)
				{
					GDSetCoinSend(lpObj->Index,0, 0,-gServerInfo.m_PhiMoBat, 0, 0, "");
				}
				else
				{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(872),lpObj->Coin3,gServerInfo.m_PhiMoBat);
				return;
				}
				
		}

		if (gServerInfo.m_KieuMoBat == 0 )
			{
				gNotice.GCNoticeSendToAll(0, 3, 0, 5000, 0, 0, gMessage.GetMessage(865),lpObj->Name, RandomNumber);
			}

		if (gServerInfo.m_KieuMoBat == 1 )
		{
			if (RandomNumber %2 == 0)
				{
					gNotice.GCNoticeSendToAll(0, 3, 0, 5000, 0, 0, gMessage.GetMessage(866),lpObj->Name);
				}
			else 
				{
					gNotice.GCNoticeSendToAll(0, 3, 0, 5000, 0, 0, gMessage.GetMessage(867),lpObj->Name);
				}
		}
		if (gServerInfo.m_KieuMoBat == 2 )
		{
			if (RandomNumber %2 == 0)
				{
					gNotice.GCNoticeSendToAll(0, 3, 0, 5000, 0, 0, gMessage.GetMessage(868),lpObj->Name, RandomNumber);
				}
			else 
				{
					gNotice.GCNoticeSendToAll(0, 3, 0, 5000, 0, 0, gMessage.GetMessage(869),lpObj->Name, RandomNumber);
				}
		}
		if (gServerInfo.m_MoveMapMoBat == 1 )
		{
			gObjTeleport(aIndex,gServerInfo.m_MapMoBat,gServerInfo.m_XMoBat,gServerInfo.m_YMoBat);
		}
		this->DecreaseItemDur(lpObj,lpMsg->SourceSlot,1);
	}
	...
	#if(BUNDLE_ITEM)
	else if(lpItem->m_Index == GET_ITEM(14,244)		// Jewel of Level
		||  lpItem->m_Index == GET_ITEM(14,245)		// Jewel of Skill
		||  lpItem->m_Index == GET_ITEM(14,246)		// Jewel of Luck
		||  lpItem->m_Index == GET_ITEM(14,247)		// Jewel of Option
		||  lpItem->m_Index == GET_ITEM(14,248)		// Jewel of Exllent
		||  lpItem->m_Index == GET_ITEM(14,249)		// Jewel of Socket
		||  lpItem->m_Index == GET_ITEM(14,250)		// Jewel of Cent 1
		||  lpItem->m_Index == GET_ITEM(14,251)		// Jewel of Cent 2
		||  lpItem->m_Index == GET_ITEM(14,252)		// Jewel of Full
		||  lpItem->m_Index == GET_ITEM(14,260))	// Jewel of Upgrade
	{
		if(gObjectManager.CharacterUseCustomJewel(lpObj,lpMsg->SourceSlot,lpMsg->TargetSlot) != 0)
		{
			if (BundleItem.IsBundleItemSlot(lpObj->Index, lpMsg->SourceSlot)) //check bundle
			{
				if (BundleItem.UseStack(lpObj->Index, lpMsg->SourceSlot, 1))
				{
					this->GCItemModifySend2(aIndex, lpMsg->TargetSlot, 1);
				}
				else
				{
					return;
	...
	{
		if(gCustomJewel.CharacterUseCustomJewel(lpObj,lpMsg->SourceSlot,lpMsg->TargetSlot) != 0)
		{
			this->InventoryDelItem(aIndex,lpMsg->SourceSlot);
			this->GCItemDeleteSend(aIndex,lpMsg->SourceSlot,1);
			this->GCItemModifySend(aIndex,lpMsg->TargetSlot);
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg,int aIndex,int ok) // OK
{
	...
	if(item.m_Index == GET_ITEM(13,4) || item.m_Index == GET_ITEM(13,5)) //Dark Horse,Dark Reaven
	{
		...
		{
			GDSetCoinSend(lpObj->Index, -(item.Coin1), -(item.Coin2), -(item.Coin3), 0, 0, "CommonShop");
		}
		else
		{
	...
	if(this->InventoryInsertItemStack(lpObj,&item) != 0)
	{
		if (item.Coin1 > 0 || item.Coin2 > 0 || item.Coin3 > 0)
		{
			GDSetCoinSend(lpObj->Index, -(item.Coin1*valuex), -(item.Coin2*valuex), -(item.Coin3*valuex), 0, 0, "CommonShopBuy");
		}
		else
		{
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpItem->Sell > 0)
	{
		if (lpItem->Coin1 > 0)
		{
			GDSetCoinSend(lpObj->Index, value, 0, 0, 0, 0, "Shop WC:");
		}
		else if (lpItem->Coin2 > 0)
		{
			GDSetCoinSend(lpObj->Index, 0, value, 0, 0, 0, "Shop WP");
		}
		else if (lpItem->Coin3 > 0)
		{
			GDSetCoinSend(lpObj->Index, 0, 0, value, 0, 0, "Shop WG");
		}
		else
		{
			...
			else
			{
				...
				{
					gLog.Output(LOG_SHOP," %s | %s |Phi Shop: %s + %d, Skill: %d, Luck: %d, Tính Năng: %d, Hoàn Hảo: %d,Thần: %d, Hamorny: %03d, Item 380: %03d, Socket: %03d, %03d, %03d, %03d, %03d, Serial: %08X)"
					, lpObj->Account
					, lpObj->Name
...
void CItemManager::CGItemRepairRecv(PMSG_ITEM_REPAIR_RECV* lpMsg,int aIndex) // OK
{
	...
	#if(BUNDLE_ITEM)
	if (BundleItem.CGBundleItemUnmix(aIndex, lpMsg->slot))
	{
		return;
	}
	#endif
...
//}
#if(BUNDLE_ITEM)
void CItemManager::GCItemModifySend2(int aIndex, BYTE slot, BYTE mode) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
bool CItemManager::CheckItemMoveToEquip(LPOBJ lpObj, CItem* lpItem, int slot)
{
	if (slot == EQUIPMENT_WEAPON_LEFT)
	{
		if (lpObj->Inventory[slot].IsItem())
		{
			if (lpObj->Inventory[slot].m_Index == GET_ITEM(13, 5))
			{
				return false;
			}
		}
...
int CItemManager::RepairItemEquip(LPOBJ lpObj, CItem* lpItem, int slot, int type)
{
	if (lpItem->m_Slot == 0xFF || EQUIP_INVENTORY_RANGE(lpItem->m_Slot) == 0)
	{
		return 0;
	}
	...
	if (EQUIP_INVENTORY_RANGE(slot) != 0 && lpItem->CheckDurabilityState() != 0)
	{
		gObjectManager.CharacterCalcAttribute(lpObj->Index);
	}

	g_pNewMyInventory.GCPetItemDurSend(lpObj->Index, slot, (BYTE)lpItem->m_Durability);

	return lpObj->Money;
}

void CItemManager::CGItemEquipRepairRecv(PMSG_ITEM_REPAIR_RECV* lpMsg, int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	if (lpMsg->slot != 0xFF)
	{
		if (lpObj->EquipInventory[lpMsg->slot].m_Index != GET_ITEM(13, 4)
			&& lpObj->EquipInventory[lpMsg->slot].m_Index != GET_ITEM(13, 5))
		{
			if (lpObj->Interface.use == INTERFACE_TRADE)
			{
				return;
	...
	if (lpMsg->slot == 0xFF)
	{
		for (int n = 0; n < EQUIP_INVENTORY_SIZE; n++)
		{
			...
			pMsg.money = this->RepairItemEquip(lpObj, &lpObj->EquipInventory[n], n, lpMsg->type);
	...
	pMsg.money = this->RepairItemEquip(lpObj, &lpObj->EquipInventory[lpMsg->slot], lpMsg->slot, lpMsg->type);
...
BYTE CItemManager::MoveItemToEquipFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (INVENTORY_FULL_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	...
	if (this->CheckItemMoveToEquip(lpObj, &lpObj->Inventory[SourceSlot], TargetSlot) == 0)
	{
		return 0xFF;
	}

	if (g_pNewMyInventory.PetInventoryAddItem(lpObj->Index, lpObj->Inventory[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
	...
	g_pNewMyInventory.PetInventoryItemSet(lpObj->Index, TargetSlot, 1);

	g_pNewMyInventory.GCPetItemChangeSend(lpObj->Index, TargetSlot);
...
BYTE CItemManager::MoveItemToInventoryFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (EQUIP_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	}
	...
	if (gObjInventorySearchSerialNumber(lpObj, lpObj->EquipInventory[SourceSlot].m_Serial) == 0)
	{
		return 0xFF;
	}

	if (gObjWarehouseSearchSerialNumber(lpObj, lpObj->EquipInventory[SourceSlot].m_Serial) == 0)
	{
		return 0xFF;
	}

	if (this->CheckItemMoveToInventory(lpObj, &lpObj->EquipInventory[SourceSlot], TargetSlot) == 0)
	{
		return 0xFF;
	}

	if (this->InventoryAddItem(lpObj->Index, lpObj->EquipInventory[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}

	lpObj->EquipInventoryStatus[SourceSlot] = 0;

	g_pNewMyInventory.PetInventoryDelItem(lpObj, SourceSlot);

	g_pNewMyInventory.GCPetItemChangeSend(lpObj->Index, SourceSlot);
...
BYTE CItemManager::MoveItemToChaosBoxFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (EQUIP_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	...
	if (this->CheckItemMoveToChaos(lpObj, &lpObj->EquipInventory[SourceSlot], TargetFlag) == 0)
	{
		return 0xFF;
	}
	...
	if (this->ChaosBoxAddItem(lpObj->Index, lpObj->EquipInventory[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
...
BYTE CItemManager::MoveItemToEquipFromChaosBox(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (EQUIP_INVENTORY_RANGE(TargetFlag) == 0)
	{
		return 0xFF;
	}
	...
	if (this->CheckItemMoveToEquip(lpObj, &lpObj->ChaosBox[SourceSlot], TargetSlot) == 0)
	{
		return 0xFF;
	}

	if (g_pNewMyInventory.PetInventoryAddItem(lpObj->Index, lpObj->ChaosBox[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
...
BYTE CItemManager::MoveItemToPersonalShopFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (EQUIP_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	...
	if (this->CheckItemMoveToTrade(lpObj, &lpObj->EquipInventory[SourceSlot], TargetFlag) == 0)
	{
		return 0xFF;
	}
...
BYTE CItemManager::MoveItemToEquipFromPersonalShop(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (INVENTORY_SHOP_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
...
BYTE CItemManager::MoveItemToEquipFromTrade(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (TRADE_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	...
	if (EQUIP_INVENTORY_RANGE(TargetSlot) == 0)
	{
		return 0xFF;
	}
	...
	if (this->CheckItemMoveToEquip(lpObj, &lpObj->Trade[SourceSlot], TargetSlot) == 0)
	{
		return 0xFF;
	}

	if (g_pNewMyInventory.PetInventoryAddItem(lpObj->Index, lpObj->Trade[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
...
BYTE CItemManager::MoveItemToTradeFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (EQUIP_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	...
	if (this->TradeAddItem(lpObj->Index, lpObj->EquipInventory[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
...
BYTE CItemManager::MoveItemToEquipFromWarehouse(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (WAREHOUSE_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
...
BYTE CItemManager::MoveItemToWarehouseFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if (EQUIP_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	...
	if (this->CheckItemMoveToVault(lpObj, &lpObj->EquipInventory[SourceSlot], TargetFlag) == 0)
	{
		return 0xFF;
	}
	...
	if (this->WarehouseAddItem(lpObj->Index, lpObj->EquipInventory[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
...
#endif
#if NEWXSHOP
bool CItemManager::CheckItemRequireClassUnick2(WORD Class, int index) // OK
{
	ITEM_INFO ItemInfo;
	int isClass = 0;
	int classes = 0;
	if (this->GetInfo(index, &ItemInfo) == 0 || CHECK_RANGE(Class, MAX_CLASS) == 0)
	{
		return false;
	}
...
#endif
#if LUC_CHIEN
int CItemManager::GetItemLevel(int index) // OK
{
	ITEM_INFO ItemInfo;
	if (this->GetInfo(index, &ItemInfo) == 0)
	{
