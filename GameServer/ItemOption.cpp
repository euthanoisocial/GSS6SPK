	...
	for(int n=0; n < MAX_SPECIAL; n++)
	{
		...
		switch(index)
		{
			...
			case ITEM_OPTION_MUL_DEFENSE_SUCCESS_RATE:
				...
				lpObj->DefenseSuccessRate += (int)(lpObj->DefenseSuccessRate*((lpItem->m_Level*20)/100))/100;
				break;
			case ITEM_OPTION_ADD_DAMAGE_REFLECT:
				lpObj->DamageReflect += value;
				lpObj->DamageReflect += (int)(lpObj->DamageReflect*((lpItem->m_Level*20)/100))/100;
				break;
			...
			case ITEM_OPTION_MUL_MP:
				...
				lpObj->AddMana += (int)(lpObj->MaxMana*((lpItem->m_Level*20)/100))/100;
				break;
			case ITEM_OPTION_MUL_HP:
				lpObj->AddLife += (int)((lpObj->MaxLife*value)/100);
				lpObj->AddLife += (int)(lpObj->MaxLife*((lpItem->m_Level*20)/100))/100;
				break;
			...
			case ITEM_OPTION_ADD_SPEED:
				...
				lpObj->PhysiSpeed += (int)(lpObj->PhysiSpeed*((lpItem->m_Level*20)/100))/100;
				lpObj->MagicSpeed += (int)(lpObj->MagicSpeed*((lpItem->m_Level*20)/100))/100;
				break;
...
void CItemOption::CBInsertOption(LPOBJ lpObj, int SpecialIndex, int SpecialValue) // OK
{
	//for (int n = 0; n < MAX_SPECIAL; n++)
	{
		int index = SpecialIndex;

		int value = SpecialValue;
		...
		case ITEM_OPTION_ADD_HUNT_MP:
			...
			break;
		case ITEM_OPTION_ADD_WING_HP:
			lpObj->AddLife += 50 + (value * 10);
			break;
		case ITEM_OPTION_ADD_WING_MP:
			lpObj->AddMana += 50 + (value * 10);
			break;
		case ITEM_OPTION_ADD_IGNORE_DEFENSE_RATE:
			lpObj->IgnoreDefenseRate += value;
			break;
		...
		case ITEM_OPTION_MUL_BP:
			...
			break;
		case ITEM_OPTION_ADD_WING_LEADERSHIP:
			lpObj->AddLeadership += 10 + (value * 10);
			break;
		case ITEM_OPTION_ADD_FENRIR1:
			break;
		case ITEM_OPTION_ADD_FENRIR2:
