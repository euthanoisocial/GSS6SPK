...
#	if(GAMESERVER_TYPE==0 && SKY_EVENT == 1)
	this->ReadSkyEvent();
	#endif
...
void CServerInfo::ReadCommandInfo() // OK
{
	...
	this->ReadCommandInfo("GameServerInfo",".\\Data\\GameServerInfo - Command.ini");
...
void CServerInfo::ReadCommonInfo() // OK
{
	...
	gSPKMessage.Load(gPath.GetFullPath("SPK\\SPK_Message.txt"));
...
{	
	#if(ADD_POINT)
	this->ReadClientInterfaceInfo();
	#endif
	#if(HT_THUE_FLAG)
	ThueFlag.Read(gPath.GetFullPath("Custom\\ThueFlag.txt"));
#	endif
	
	#if(BUNDLE_ITEM)
	BundleItem.Read(gPath.GetFullPath("SPK\\BundleItem.txt"));
	#endif
	#if USE_FAKE_ONLINE == TRUE
	s_FakeOnline.LoadFakeData(".\\AutoTrain.xml");
	#endif

	this->ReadCustomInfo("GameServerInfo",".\\Data\\GameServerInfo - Custom.ini");

	gCustomAttack.ReadCustomAttackInfo("GameServerInfo",".\\Data\\GameServerInfo - Custom.ini");
	#if EVENT_DROP
	gCustomEventDrop.ReadCustomEventDropInfo("GameServerInfo",".\\Data\\GameServerInfo - Custom.ini");
	gCustomEventDrop.Load(gPath.GetFullPath("Custom\\CustomEventDrop.txt"));
	#endif
	gCustomOnlineLottery.ReadCustomOnlineLotteryInfo("GameServerInfo",".\\Data\\GameServerInfo - Custom.ini");
	...
	gPremiumDrop.Load(gPath.GetFullPath("Custom\\PremiumDrop.xml"));
#if(BUFF_VUKHI_ICON)
	gCustomStartItemDame.LoadItemDame(gPath.GetFullPath("Custom\\CustomStartItemDame.txt"));
#endif

#if(BUFF_SET_ICON)
	gCustomStartSetItemDame.Load();
#endif
#if(VONGQUAY_NEW)
	gLuckySpin.Load();
#endif
	...
	#if(PARTYSEARCH==1)
	    gPartySearch.Load("Config", "..\\Data\\Custom\\PartySearch\\PartySearch.txt");
    #endif
	gShopPointEx.Load(gPath.GetFullPath("Custom\\ShopPoint.txt"));

#if (CUSTOM_GUILD_UPGRADE == TRUE)
	GuildUpgrade.Load(gPath.GetFullPath("Custom\\GuildUpgrade.txt"));
#endif

	gZenDrop.Load(gPath.GetFullPath("Custom\\ZenDrop.xml"));
...
void CServerInfo::ReadEventInfo() // OK
{
	...
	#if(CB_EventArenaPvP)
	gCBArenaPVP.Load(gPath.GetFullPath("Event\\EventArenaPvP.xml"));
	#endif
	...
	#if(CTCMINI)
	gCTCMini.Load(gPath.GetFullPath("Event\\CTCMini\\CongThanhChien.xml"));
	gCTCMini.Init();
	#endif
	
#if(CHIEN_TRUONG_CO)
	gChienTruongCo.LoadData(gPath.GetFullPath("Event\\ChienTruongCo\\ChienTruongCo.xml"));
#endif
	
#if(EVENT_PK)
	gBsVEvent.Load(gPath.GetFullPath("Event\\BsVEvent.xml"));
#endif
	#if(HAPPY_HOUR == 1)
	ObjHappy.Load(gPath.GetFullPath("Event\\HappyHours.xml"));
	#endif
	...
	if(gCastleSiege.Ready(gMapServerManager.GetMapServerGroup()) != 0)
	{
		...
		{
			gCastleSiege.LoadPreFixData(".\\Data\\GameServerInfo - Event.ini");
	...
	#if(THANMA)
	gThanMaChien.Load(gPath.GetFullPath("Event\\ThanMaChien.xml"));
	#endif
...
void CServerInfo::ReadHackInfo() // OK
{
	...
	#if(SAU_ANTIHACK)
	gBAntihack.LoadFileXML(gPath.GetFullPath("Hack\\SauAntiGS.xml"));
	#endif
...
void CServerInfo::ReadSwampEvent()
{
	#if(SWAMP_EVENT == 1)
	gSwampEvent.LoadDataConfig(gPath.GetFullPath("Event\\SwampEvent\\SwampEvent.dat"));
	gSwampEvent.ReadMonsters(gPath.GetFullPath("Event\\SwampEvent\\SwampMonsterGroup.dat"));
	#endif
}

void CServerInfo::ReadSkyEvent(){
	#if(SKY_EVENT == 1)
	gSkyEvent.LoadData(gPath.GetFullPath("Event\\SkyEvent\\SkyEvent.dat"));
	gSkyEvent.ReadMonsters(gPath.GetFullPath("Event\\SkyEvent\\SkyMonsterGroup.dat"));
	LogAdd(LOG_BLACK, "[ServerInfo] SkyEvent Event loaded successfully");
	#endif
}
...
void CServerInfo::ReadMonsterInfo() // OK
{
	...
	#if(SPK_ITEMTRADER)
	gGC_ItemTrader.SetNPC();
	#endif
	#if(SPK_DUNGLUYEN)
	gBDungLuyenItem.SetNPC();
	#endif
...
void CServerInfo::ReadSPKInfo()
{
	gAddBuffer.Read(gPath.GetFullPath("SPK\\AddBuff.txt"));
	#if(B_MOCNAP)
	gCBMocNap.LoadFileXML(gPath.GetFullPath("SPK\\CustomMocNap.xml"));
	#endif
	#if (SPK_DUNGLUYEN)
	gBDungLuyenItem.LoadConfig(gPath.GetFullPath("SPK\\CustomDungLuyenItem.xml"));
	#endif
	#if BAUCUA
	gCustomBauCua.Load();
	#endif
	#if LUC_CHIEN
	gCustomLucChien.LoadFileXML(gPath.GetFullPath("SPK\\CustomLucChien.xml"));
	#endif
	#if NEWXSHOP
	gNewCashShop.LoadFileXML(gPath.GetFullPath("SPK\\CustomShop.xml"));
	#endif
	#if (SPK_ITEMTRADER)
	gGC_ItemTrader.LoadConfig(gPath.GetFullPath("SPK\\ChangeItem.xml"));
	#endif
	#if (CongHuongV2)
	gCBCongHuong.CH_LoadConfigXML(gPath.GetFullPath("SPK\\CustomCongHuong.xml"));
	#endif
	#if ACTIVE_VKR
	GITEM00.Load(gPath.GetFullPath("SPK\\CustomItemRong\\00.VuKhiRong.xml"));
	GITEM06.Load(gPath.GetFullPath("SPK\\CustomItemRong\\06.KhienRong.xml"));
	GITEM07.Load(gPath.GetFullPath("SPK\\CustomItemRong\\07.MuRong.xml"));
	GITEM08.Load(gPath.GetFullPath("SPK\\CustomItemRong\\08.AoRong.xml"));
	GITEM09.Load(gPath.GetFullPath("SPK\\CustomItemRong\\09.QuanRong.xml"));
	GITEM10.Load(gPath.GetFullPath("SPK\\CustomItemRong\\10.GangRong.xml"));
	GITEM11.Load(gPath.GetFullPath("SPK\\CustomItemRong\\11.GiayRong.xml"));
	GITEM12.Load(gPath.GetFullPath("SPK\\CustomItemRong\\12.WingRong.xml"));
	GITEM13.Load(gPath.GetFullPath("SPK\\CustomItemRong\\13.RingRong.xml"));
	#endif

	gSPKMessage.Load(gPath.GetFullPath("SPK\\SPK_Message.txt"));

	#if(NGAN_HANG_NGOC)
	gCustomJewelBank.LoadConfig(gPath.GetFullPath("SPK\\CustomJewelBank.xml"));
	#endif
	#if(BUNDLE_ITEM)
	BundleItem.Read(gPath.GetFullPath("SPK\\BundleItem.txt"));
	#endif
	#if VONGQUAY_NEW
	gLuckySpin.Read(gPath.GetFullPath("SPK\\LuckySpin.txt"));
	#endif
	#if(BAN_RS)
		gResetChange.Load(gPath.GetFullPath("SPK\\ResetChange.txt"));
	#endif
	#if(DUCTUONG == 0)
	gDucTuong.LoadConfig();
	gDucTuong.MakeBot();
	#endif
	#if(B_HON_HOAN)	
	gBHonHoan.LoadConfig(gPath.GetFullPath("SPK\\HonHoan.xml"));
	#endif
	#if(DANH_HIEU_NEW)	
	gSPKDanhHieu.LoadConfig(gPath.GetFullPath("SPK\\DanhHieu.xml"));
	#endif
	#if(QUAN_HAM_NEW)	
	gSPKQuanHam.LoadConfig(gPath.GetFullPath("SPK\\QuanHam.xml"));
	#endif
	#if(TU_LUYEN_NEW)	
	gSPKTuLuyen.LoadConfig(gPath.GetFullPath("SPK\\TuChan.xml"));
	#endif
	#if(NEW_VIP_NEW)	
	gSPKNewVip.LoadConfig(gPath.GetFullPath("SPK\\NewVip.xml"));
	#endif
	#if RELIFE
	gTaiSinh.Load(gPath.GetFullPath("SPK\\CustomTaiSinhTrungSinh.xml"));
	#endif
	LogAdd(LOG_BLUE,"Tải cấu hình SPK thành công!");
}
void CServerInfo::ReadUtilInfo() // OK
{
	...
	#if(RESET_VIP_AUTO_ON)
	gResetTableVip.Load(gPath.GetFullPath("Util\\ResetTableVIP.txt"));
	#endif
	gRewardSystem.Load(gPath.GetFullPath("Util\\RewardTable.txt"));
...
#if (BOTHUMUADORAC == 1)
	BotThuMua.Read(gPath.GetFullPath("Custom\\BotSystem\\ThuMuaDoExc.txt"));
	BotThuMua.MakeBot();
#endif
#if(BOTCONGHUONG==1)
	BotAlchemist.Read(gPath.GetFullPath("Custom\\BotSystem\\CongHuong.txt"));
...
void CServerInfo::ReadChaosMixInfo(char* section,char* path) // OK
{
	...
	//Sói vàng
	this->m_HornOfFenrirGoldMixRate[0] = GetPrivateProfileInt(section,"HornOfFenrirGoldMixRate_AL0",0,path);

	this->m_HornOfFenrirGoldMixRate[1] = GetPrivateProfileInt(section,"HornOfFenrirGoldMixRate_AL1",0,path);

	this->m_HornOfFenrirGoldMixRate[2] = GetPrivateProfileInt(section,"HornOfFenrirGoldMixRate_AL2",0,path);

	this->m_HornOfFenrirGoldMixRate[3] = GetPrivateProfileInt(section,"HornOfFenrirGoldMixRate_AL3",0,path);
...
void CServerInfo::ReadCharacterInfo(char* section,char* path) // OK
{
	...
	#endif
	#if COMBO_SKILL
	this->m_TyLeRaComboDW = GetPrivateProfileInt(section, "TyLeRaComboDW", 0, path);
	this->m_TyLeRaComboDK = GetPrivateProfileInt(section, "TyLeRaComboDK", 0, path);
	this->m_TyLeRaComboEF = GetPrivateProfileInt(section, "TyLeRaComboEF", 0, path);
	this->m_TyLeRaComboMG = GetPrivateProfileInt(section, "TyLeRaComboMG", 0, path);
	this->m_TyLeRaComboDL = GetPrivateProfileInt(section, "TyLeRaComboDL", 0, path);
	this->m_TyLeRaComboSM = GetPrivateProfileInt(section, "TyLeRaComboSM", 0, path);
	this->m_TyLeRaComboRF = GetPrivateProfileInt(section, "TyLeRaComboRF", 0, path);
	#endif
	#if(EXPDMG)
	this->MoGioiHanDameQuai = GetPrivateProfileInt(section, "MoGioiHanDameQuai", 0, path);
	#endif
	...
	this->m_MGDamageMultiplierConstA = GetPrivateProfileInt(section, "MGDamageMultiplierConstA", 0, path);
	...
	this->m_MGDamageMultiplierRatePhysic = GetPrivateProfileInt(section, "MGDamageMultiplierRatePhysic", 0, path);

	this->m_MGDamageMultiplierMaxRate = GetPrivateProfileInt(section, "MGDamageMultiplierMaxRate", 0, path);

	this->m_ELFDamageMultiplierMaxRate = GetPrivateProfileInt(section, "ELFDamageMultiplierMaxRate", 0, path);

	this->m_SUDamageMultiplierMaxRate = GetPrivateProfileInt(section, "SUDamageMultiplierMaxRate", 0, path);

	this->m_DWDamageMultiplierMaxRate = GetPrivateProfileInt(section, "DWDamageMultiplierMaxRate", 0, path);
	...
	#if(TIDUS_FIX_MT)
	this->m_DarkSpiritAttackSuccessRateConstD = GetPrivateProfileInt(section,"DarkSpiritAttackSuccessRateConstD",0,path);
	this->m_DarkSpiritAttackSuccessRateConstE = GetPrivateProfileInt(section,"DarkSpiritAttackSuccessRateConstE",0,path);
	#endif
	...
	this->m_GodenFenrirIncDamageConstA = GetPrivateProfileInt(section,"GodenFenrirIncDamageConstA",0,path);
	...
	this->m_GodenFenrirDecDamageConstA = GetPrivateProfileInt(section,"GodenFenrirDecDamageConstA",0,path);
	...
	//--- Skill All
	this->GioiHanTyLeX2SatThuong = GetPrivateProfileInt(section, "GioiHanTyLeX2SatThuong", 0, path);
	this->GioiHanTyLeX3SatThuong = GetPrivateProfileInt(section, "GioiHanTyLeX3SatThuong", 0, path);
	this->GioiHanTyLeSatThuongChiMang = GetPrivateProfileInt(section, "GioiHanTyLeSatThuongChiMang", 0, path);
	this->GioiHanTyLeSatThuongHoanHao = GetPrivateProfileInt(section, "GioiHanTyLeSatThuongHoanHao", 0, path);
...
void CServerInfo::ReadCommandInfo(char* section,char* path) // OK
{
	...
	this->m_DisableResetAutoOutGame = GetPrivateProfileInt(section,"DisableResetAutoOutGame",0,path);
	...
	// Auto reset vip
	#if(RESET_VIP_AUTO_ON)
	this->m_CommandResetVipAutoEnable[0] = GetPrivateProfileInt(section,"CommandResetVipAutoEnable_AL0",0,path);

	this->m_CommandResetVipAutoEnable[1] = GetPrivateProfileInt(section,"CommandResetVipAutoEnable_AL1",0,path);

	this->m_CommandResetVipAutoEnable[2] = GetPrivateProfileInt(section,"CommandResetVipAutoEnable_AL2",0,path);

	this->m_CommandResetVipAutoEnable[3] = GetPrivateProfileInt(section,"CommandResetVipAutoEnable_AL3",0,path);
	#endif
	...
	#if(RESET_WCOIN)
	this->m_CommandResetPayment[0] = GetPrivateProfileInt(section,"CommandResetPayment_AL0",0,path);
	this->m_CommandResetPayment[1] = GetPrivateProfileInt(section,"CommandResetPayment_AL1",0,path);
	this->m_CommandResetPayment[2] = GetPrivateProfileInt(section,"CommandResetPayment_AL2",0,path);
	this->m_CommandResetPayment[3] = GetPrivateProfileInt(section,"CommandResetPayment_AL3",0,path);
	#endif
	...
	this->m_CommandResetAddReward = GetPrivateProfileInt(section,"CommandResetAddReward",0,path);

	this->m_CommandResetAddWCoinC[0] = GetPrivateProfileInt(section,"CommandResetAddWCoinC_AL0",0,path);

	this->m_CommandResetAddWCoinC[1] = GetPrivateProfileInt(section,"CommandResetAddWCoinC_AL1",0,path);

	this->m_CommandResetAddWCoinC[2] = GetPrivateProfileInt(section,"CommandResetAddWCoinC_AL2",0,path);

	this->m_CommandResetAddWCoinC[3] = GetPrivateProfileInt(section,"CommandResetAddWCoinC_AL3",0,path);


	this->m_CommandResetAddWCoinP[0] = GetPrivateProfileInt(section,"CommandResetAddWCoinP_AL0",0,path);

	this->m_CommandResetAddWCoinP[1] = GetPrivateProfileInt(section,"CommandResetAddWCoinP_AL1",0,path);

	this->m_CommandResetAddWCoinP[2] = GetPrivateProfileInt(section,"CommandResetAddWCoinP_AL2",0,path);

	this->m_CommandResetAddWCoinP[3] = GetPrivateProfileInt(section,"CommandResetAddWCoinP_AL3",0,path);


	this->m_CommandResetAddGPoin[0] = GetPrivateProfileInt(section,"CommandResetAddGPoin_AL0",0,path);
	
	this->m_CommandResetAddGPoin[0] = GetPrivateProfileInt(section,"CommandResetAddGPoin_AL0",0,path);

	this->m_CommandResetAddGPoin[1] = GetPrivateProfileInt(section,"CommandResetAddGPoin_AL1",0,path);

	this->m_CommandResetAddGPoin[2] = GetPrivateProfileInt(section,"CommandResetAddGPoin_AL2",0,path);

	this->m_CommandResetAddGPoin[3] = GetPrivateProfileInt(section,"CommandResetAddGPoin_AL3",0,path);
	...
	this->m_ClearMasterChangeClass = GetPrivateProfileInt(section,"ClearMasterChangeClass",0,path);
	
	this->m_DeleteQuestChangeClass = GetPrivateProfileInt(section,"DeleteQuestChangeClass",0,path);
...
void CServerInfo::ReadResetInfo()
{
	this->GioiHanRS = GetPrivateProfileInt("CONFIG", "GioiHanRS", 0, "..//Data//SPK//GHRS.ini");
	this->ResetNgay = GetPrivateProfileInt("CONFIG", "ResetNgay", 0, "..//Data//SPK//RESET.ini");
	this->ResetGio  = GetPrivateProfileInt("CONFIG", "ResetGio",  0, "..//Data//SPK//RESET.ini");
	this->ResetPhut = GetPrivateProfileInt("CONFIG", "ResetPhut", 0, "..//Data//SPK//RESET.ini");
	this->ResetGiay = GetPrivateProfileInt("CONFIG", "ResetGiay", 0, "..//Data//SPK//RESET.ini");
	LogAdd(LOG_DARKGREEN, "Giới Hạn Reset: %d %d:%d:%d +%d", this->GioiHanRS, this->ResetGio, this->ResetPhut, this->ResetGiay, this->ResetNgay);
}
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	GetPrivateProfileString(section, "DayConfig", "15/03/2023", this->DayConfig, sizeof(this->DayConfig), path);

	this->m_CheckDaySub		= GetPrivateProfileInt(section,"DaoNguocSub",0,path);
	...
	this->AccountTypeVIP = GetPrivateProfileInt(section,"AccountTypeVIP",0,path);
	...
	//antiflood
	this->m_IpLimitConnectionTime = GetPrivateProfileInt(section,"IpLimitConnectionTime",1000,path);
	this->m_IpConnectionBlockedTime = GetPrivateProfileInt(section, "IpConnectionBlockedTime", 300000, path);
	this->m_MaxHwidConnection[0] = GetPrivateProfileInt(section, "MaxHwidConnection_AL0", 0, path);

	this->m_MaxHwidConnection[1] = GetPrivateProfileInt(section, "MaxHwidConnection_AL1", 0, path);

	this->m_MaxHwidConnection[2] = GetPrivateProfileInt(section, "MaxHwidConnection_AL2", 0, path);

	this->m_MaxHwidConnection[3] = GetPrivateProfileInt(section, "MaxHwidConnection_AL3", 0, path);

	

	this->m_GameGuardCRC = GetPrivateProfileInt(section, "GameGuardCRC", 0, path);

	this->m_VerifyCRC = GetPrivateProfileInt(section, "GameGuardVerifyCRC", 0, path);

	this->m_MainDLLCRC = GetPrivateProfileInt(section, "MainDLLCRC", 0, path);

	this->m_MainEXECRC = GetPrivateProfileInt(section, "MainEXECRC", 0, path);


	this->m_TimeOutGame = GetPrivateProfileInt(section, "TimeOutGame", 6, path);
	...
	this->m_ElfBufferTime[0] = GetPrivateProfileInt(section, "ElfBufferTime_AL0", 0, path);

	this->m_ElfBufferTime[1] = GetPrivateProfileInt(section, "ElfBufferTime_AL1", 0, path);

	this->m_ElfBufferTime[2] = GetPrivateProfileInt(section, "ElfBufferTime_AL2", 0, path);

	this->m_ElfBufferTime[3] = GetPrivateProfileInt(section, "ElfBufferTime_AL3", 0, path);
	...
	this->VaultBuyShop = GetPrivateProfileInt(section, "ThoiGianMuaDo", 0, path);
	this->m_ThoiGianMuaHang = GetPrivateProfileInt(section, "ThoiGianMuaHang", 0, path);
	this->m_DelayBuyShop = GetPrivateProfileInt(section, "DelayBuyShop", 1000, path);
	this->m_DelayBuyXShop = GetPrivateProfileInt(section, "DelayBuyXShop", 1000, path);
	this->m_ThoiGianQH = GetPrivateProfileInt(section, "ThoiGianQH", 1000, path);
	this->m_GioiHanUpGuild = GetPrivateProfileInt(section, "GioiHanUpGuild", 1000, path);
	this->DeleteDupeItem = GetPrivateProfileInt(section, "DeleteDupeItem", 0, path);
	this->DisconectDupeItem = GetPrivateProfileInt(section, "DisconectDupeItem", 0, path);
	//#if(GAMESERVER_UPDATE>=803)
	
	this->m_MaxEventLine = GetPrivateProfileInt(section, "TongSoSuKien", 15, path);
	...
	this->m_BatTatBloodVIP	= GetPrivateProfileInt(section,"BatTatBloodVIP",0,path);
	this->m_MocKillQuaiBloodVIP	= GetPrivateProfileInt(section,"MocKillQuaiBloodVIP",0,path);
	this->m_LevelBloodVIP	= GetPrivateProfileInt(section,"LevelBloodVIP",0,path);

	this->m_BatTatDevilVIP	= GetPrivateProfileInt(section,"BatTatDevilVIP",0,path);
	this->m_MocKillQuaiDevilVIP	= GetPrivateProfileInt(section,"MocKillQuaiDevilVIP",0,path);
	this->m_LevelDevilVIP	= GetPrivateProfileInt(section,"LevelDevilVIP",0,path);
	...
	#if(NOTICE_PKSYSTEM);
	GetPrivateProfileString(section, "PK_ColorMessPVP", "FFFFFFFF", this->PK_ColorMessPVP, sizeof(this->PK_ColorMessPVP), path);
	GetPrivateProfileString(section, "PK_ColorMessPVM", "FFFFFFFF", this->PK_ColorMessPVM, sizeof(this->PK_ColorMessPVM), path);
	this->PK_DelayMess = GetPrivateProfileInt(section, "PK_DelayMess", 10, path);
	#endif
...
void CServerInfo::ReadCustomInfo(char* section,char* path) // OK
{
	char GameServerInfo[100] = {0};
	//---
	this->m_DoiClass_WCoiC = GetPrivateProfileInt(section, "DoiClass_WCoiC", 0, path);
	this->m_KieuMoBat	 = GetPrivateProfileInt(section,"KieuMoBat",0,path);
	this->m_MoveMapMoBat = GetPrivateProfileInt(section,"MoveMapMoBat",0,path);
	this->m_MapMoBat	 = GetPrivateProfileInt(section,"MapMoBat",0,path);
	this->m_XMoBat		 = GetPrivateProfileInt(section,"XMoBat",0,path);
	this->m_YMoBat		 = GetPrivateProfileInt(section,"YMoBat",0,path);	
	this->m_LoaiTienMoBat		 = GetPrivateProfileInt(section,"LoaiTienMoBat",0,path);
	this->m_PhiMoBat		 = GetPrivateProfileInt(section,"PhiMoBat",0,path);

#if(DROP_ITEMBAG)
	this->m_BatTatDropItemBag = GetPrivateProfileInt(section, "CheckOptionDrop", 0, path);

	this->m_DropRateItemBagVK1 = GetPrivateProfileInt(section, "VKExcOption1", 0, path);
	this->m_DropRateItemBagVK2 = GetPrivateProfileInt(section, "VKExcOption2", 0, path);
	this->m_DropRateItemBagVK4 = GetPrivateProfileInt(section, "VKExcOption3", 0, path);
	this->m_DropRateItemBagVK8 = GetPrivateProfileInt(section, "VKExcOption4", 0, path);
	this->m_DropRateItemBagVK16 = GetPrivateProfileInt(section, "VKExcOption5", 0, path);
	this->m_DropRateItemBagVK32 = GetPrivateProfileInt(section, "VKExcOption6", 0, path);

	this->m_DropRateItemBagSet1 = GetPrivateProfileInt(section, "TrangBiExcOption1", 0, path);
	this->m_DropRateItemBagSet2 = GetPrivateProfileInt(section, "TrangBiExcOption2", 0, path);
	this->m_DropRateItemBagSet4 = GetPrivateProfileInt(section, "TrangBiExcOption3", 0, path);
	this->m_DropRateItemBagSet8 = GetPrivateProfileInt(section, "TrangBiExcOption4", 0, path);
	this->m_DropRateItemBagSet16 = GetPrivateProfileInt(section, "TrangBiExcOption5", 0, path);
	this->m_DropRateItemBagSet32 = GetPrivateProfileInt(section, "TrangBiExcOption6", 0, path);
#endif
	//; Cusstom quyet dau
	this->m_SoTranChamChien = GetPrivateProfileInt(section,"SoTranChamChien",0,path);
	this->m_SoPhongDauCaCuoc = GetPrivateProfileInt(section,"SoPhongDauCaCuoc",0,path);
	this->m_Tylenhantien = GetPrivateProfileInt(section,"TyleNhanTien",0,path);
	this->m_LoaiTienNhan = GetPrivateProfileInt(section,"LoaiTienNhan",0,path);
	this->m_QuyetDau = GetPrivateProfileInt(section,"QuyetDau",0,path);
	this->m_GioiHanTienCuocQuyetDau = GetPrivateProfileInt(section,"GioiHanTienCuocQuyetDau",0,path);
	//;==================================================
	
	this->m_GuildPK = GetPrivateProfileInt(section,"TatCungGuildPK",0,path);
	
	GetPrivateProfileString(section,"MessageMemberOnline","",this->m_MemberOnline,sizeof(this->m_MemberOnline),path);

	GetPrivateProfileString(section,"MessagePK","",this->m_MessagePK,sizeof(this->m_MessagePK),path);
	...
	GetPrivateProfileString(section,"AnnounceChaosMix15","",this->m_AnnounceChaosMix15,sizeof(this->m_AnnounceChaosMix15),path);
	GetPrivateProfileString(section,"AnnounceChaosMix14","",this->m_AnnounceChaosMix14,sizeof(this->m_AnnounceChaosMix14),path);
	GetPrivateProfileString(section,"AnnounceChaosMix13","",this->m_AnnounceChaosMix13,sizeof(this->m_AnnounceChaosMix13),path);
	GetPrivateProfileString(section,"AnnounceChaosMix12","",this->m_AnnounceChaosMix12,sizeof(this->m_AnnounceChaosMix12),path);
	GetPrivateProfileString(section,"AnnounceChaosMix11","",this->m_AnnounceChaosMix11,sizeof(this->m_AnnounceChaosMix11),path);
	GetPrivateProfileString(section,"AnnounceChaosMix10","",this->m_AnnounceChaosMix10,sizeof(this->m_AnnounceChaosMix10),path);
	GetPrivateProfileString(section,"AnnounceChaosMixFailed","",this->m_AnnounceChaosMixFailed,sizeof(this->m_AnnounceChaosMixFailed),path);
...
void CServerInfo::ReadEventInfo(char* section,char* path) // OK
{
	...
	this->m_FlyingDragonsOnlyBossMapSwitch = GetPrivateProfileInt(section, "FlyingDragonsOnlyBossMapSwitch", 0, path); //Dragones Only Boss Map Switch
	...
	this->m_EventHideAndSeekAutoReward1		= GetPrivateProfileInt(section,"EventHideAndSeekAutoRewardWC",0,path);
	this->m_EventHideAndSeekAutoReward2		= GetPrivateProfileInt(section,"EventHideAndSeekAutoRewardWP",0,path);
	this->m_EventHideAndSeekAutoReward3		= GetPrivateProfileInt(section,"EventHideAndSeekAutoRewardGC",0,path);
...
void CServerInfo::ReadHackInfo(char* section,char* path) // OK
{
	...
	this->m_XdameHackSkillEnable = GetPrivateProfileInt(section, "BatKiemTraTocDo"		, 0, path);
	this->m_XdameHackSkillPenalty = GetPrivateProfileInt(section, "ThoiGianTrungPhat"	, 0, path);
	this->m_XdameHackSkillDebug = GetPrivateProfileInt(section, "AdminTest", 0, path);
	this->m_XdameHackSkillType = GetPrivateProfileInt(section, "TrungPhat", 0, path);
	this->m_XdameHackSkillDialog = GetPrivateProfileInt(section, "ThongBao", 0, path);
	this->m_timedongbang = GetPrivateProfileInt(section, "GayChoangSkill", 0, path);

	this->ChangePassCoin = GetPrivateProfileInt(section, "ChangePassCoin", 0, path);
...
void CServerInfo::ReadSkillInfo(char* section,char* path) // OK
{
	...
	this->ElfNoBoltArrow = GetPrivateProfileInt(section, "ElfNoBoltArrow", 0, path);

	this->TyLeRaChoangSkillMasterNguTien = GetPrivateProfileInt(section, "TyLeRaChoangSkillMasterNguTien", 0, path);

	this->ThoiGianChoangSkillMasterNguTien = GetPrivateProfileInt(section, "ThoiGianChoangSkillMasterNguTien", 0, path);
	...
	//--- Skill DW
	this->SkillDocDW		= GetPrivateProfileInt(section, "SkillDocDW", 0, path);
	this->ThoiGianDameDocDW = GetPrivateProfileInt(section, "ThoiGianDameDocDW", 0, path);

	//--- Skill DK
	this->DameSkillSocGioDK = GetPrivateProfileInt(section, "DameSkillSocGioDK", 0, path);
	this->SkillDamGioLan	= GetPrivateProfileInt(section, "SkillDamGioLan", 0, path);
	...
	this->m_GreaterDefenseTimeConstB = GetPrivateProfileInt(section,"GreaterDefenseTimeConstB",0,path);
	...
	this->m_GreaterDamageTimeConstB = GetPrivateProfileInt(section,"GreaterDamageTimeConstB",0,path);
	...
	this->m_GreaterIgnoreDefenseMaxRate = GetPrivateProfileInt(section, "GreaterIgnoreDefenseMaxRate", 0, path);
...
#if(ADD_POINT)
void CServerInfo::ReadClientInterfaceInfo() // OK
{
	this->m_QuickAddButton = GetPrivateProfileInt("Interface", "QuickAddButton", 0, "..\\Data\\SPK\\ClientInterface.ini");
	this->m_ResetStatButton = GetPrivateProfileInt("Interface", "ResetPointButton", 0, "..\\Data\\SPK\\ClientInterface.ini");
}
#endif
...
#if(TEST_GHRS)
void CServerInfo::ThayDoiGioiHanRS()
{
	tm * today;
	time_t ltime;
	time(&ltime);
	...
	{
		if (today->tm_min == this->ResetPhut)
		{
			if (today->tm_hour == this->ResetGio)
			{
				GioiHanRS += this->ResetNgay;
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(922), this->ResetNgay, this->GioiHanRS);
				LogAdd(LOG_DARKGREEN, "Giới Hạn Reset: %d %d:%d:%d +%d", this->GioiHanRS, this->ResetGio, this->ResetPhut, this->ResetGiay, this->ResetNgay);
				char	Stringg[20] = { 0 };
				wsprintf(Stringg, "GioiHanRS = %d", this->GioiHanRS);

				ofstream myfile;
				myfile.open("..//Data//SPK//GHRS.ini");
				myfile << "[CONFIG]\n";
				myfile << Stringg;
				myfile.close();
