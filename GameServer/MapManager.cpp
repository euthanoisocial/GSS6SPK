...
void CMapManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.CommandHelp = lpMemScript->GetAsNumber();

			info.CommandOffPvP = lpMemScript->GetAsNumber();
...
int CMapManager::GetMapCommandHelp(int index) // OK
{
	...
	{
		return ((it->second.CommandHelp == -1) ? 0 : it->second.CommandHelp);
	}
}
...
int CMapManager::GetMapCommandOffPvP(int index) // OK
{
	...
	{
		return ((it->second.CommandOffPvP == -1) ? 0 : it->second.CommandOffPvP);
	}
}
