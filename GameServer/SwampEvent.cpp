...
void cSwampEvent::CheckSync() // OK
{
	...
	//LogAdd(LOG_BLACK, "[Swamp Event] Thoi Gian Bat Dau Con [%d] Phut", this->m_RemainTime / 60);
}
...
void cSwampEvent::ReadMonsters(char * FilePath)
{
	...
	if (SMDFile == NULL)
	{
		ErrorMessageBox("Swamp Event Loi Tai Du lieu %s", FilePath);
		return;
	}
	...
	{
		//LogAdd(LOG_EVENT, "[Swamp Event] Khong Can load monster base file!");
	}
...
#if(GAMESERVER_TYPE == 0)
						//LogAdd(LOG_EVENT, "[Swamp Section: 0] ROOM:%d [%s] has %d groups",							RoomID + 1, pRoom[RoomID].Territory, pRoom[RoomID].Groups							);
#endif
					}
				}
				else if (type == 1)
...
#if(GAMESERVER_TYPE == 0)
							//LogAdd(LOG_EVENT, "[Swamp Section: 1] ROOM:%d[%d] Monster:%d Count:%d", m_MonsterInfo.Room, m_MonsterInfo.Group,								m_MonsterInfo.Number, m_MonsterInfo.Count);
#endif
						}
						LeaveCriticalSection(&this->m_critEventData);
					}
...
#if(GAMESERVER_TYPE == 0)
					//LogAdd(LOG_EVENT, "[Swamp Event] GROUP:%d set to %d minutes",						GroupID + 1, GroupTimer[GroupID]);
#endif
				}
				else
					break;
	...
	if (this->Enabled == 1)
	{
		if (Gate >= this->SwampGateRangeStart && Gate <= this->SwampGateRangeEnd)
		{
			if (this->OnlyMastersEnter == 1)
			{
				...
				{
					gNotice.NewNoticeSend(aIndex, 0, 0, 0, 0, 0, "Máp Này Chỉ Dành Cho Masters");
					return false;
				}
			}
			...
			if (this->SwampWarpGate != Gate)
			{
				if (this->Start == 0)
				{
					...
					{
						gNotice.NewNoticeSend(aIndex, 0, 0, 0, 0, 0, "[SwampEvent] Sự Kiện Chưa Bắt Đầu");
						return false;
					}
				}
			}
...
void cSwampEvent::SetMonsters(int iAssultType, int iGroup)
{
	...
	for (std::vector<SWAMP_EVENTMOB_DATA>::iterator it2 = it->second.begin(); it2 != it->second.end(); it2++)
	{
		...
		//LogAdd(LOG_BLACK, "[Swamp Event][AssultType:%d,Group:%d] - Adding Monsters ID: %d / QTY: %d",			iAssultType, iGroup, pMonsterInfo->Number, pMonsterInfo->Count);
		...
		for (int i = 0; i < pMonsterInfo->Count; i++)
		{
			...
			if (gObjGetRandomFreeLocation(MAP_SWAMP_OF_CALMNESS, &cX, &cY, (pMonsterInfo->XF - pMonsterInfo->YF), (pMonsterInfo->YT - pMonsterInfo->XT), 100) == TRUE)
			{
				...
				if (OBJECT_RANGE(iMobIndex >= 0))
				{
					...
					{
						//LogAdd(LOG_BLACK, "[Swamp Event][ERROR][AssultType:%d,Group:%d] Monster ammount is over",							iAssultType, iGroup							);
						return;
					}
				}
				else {
					//LogAdd(LOG_BLACK, "[Swamp Event][ERROR][AssultType:%d,Group:%d] gObjAddMonster returned: %d",						iAssultType, iGroup,						iMobIndex						);
				}
			}
			else {
				//LogAdd(LOG_BLACK, "[Swamp Event][ERROR][AssultType:%d,Group:%d][%d] - Fail Getting Location for: %d [%d,%d,%d,%d]",					iAssultType, iGroup, i, pMonsterInfo->Number, pMonsterInfo->XF, pMonsterInfo->YF, pMonsterInfo->XT, pMonsterInfo->YT);
			}
		}
	}
...
void cSwampEvent__InsideTrigger(void *  lpParam)
{
	...
	for (int i = 6; i > 0; i--)
	{
		if (i == 6)
		{
			//LogAdd(LOG_BLACK, "[SwampEvent] Chuan Bi Event");
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Medusas Đang Chuẩn Bị Tấn Công");
		}
		else
		{
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Bắt Đầu Sau %d Phút", i);
			//LogAdd(LOG_BLACK, "[SwampEvent] Bat Dau Sau %d Phut", i);
		}
		Sleep(60000);
	}

	//LogAdd(LOG_BLACK, "[SwampEvent] Chuan Bi Tran Chien Tiep Theo", 1);

	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Chuẩn Bị Cho Trận Chiến Tiếp Theo!", 1);
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Medusa Tấn Công: %s", gSwampEvent.pRoom[gSwampEvent.CurrentRoom - 1].Territory);

	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "Hãy Tìm Medusa Tiêu Diệt!");
	...
	while (Activate)
	{
		...
		{
			LogAdd(LOG_BLACK, "[SwampEvent] State -> That Bai: Thoi Gian Het");
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Thất Bại Đã Để Cho Medusa Chạy Thoát!", 0);
			break;
		}
		...
		if (gSwampEvent.MobsKilled >= gSwampEvent.MobsStageGroupCount)
		{
			...
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Chuẩn Bị Cho Trận Chiến Tiếp Theo!", 0);
				Seconds = 0;
				...
				gSwampEvent.SetMonsters(gSwampEvent.CurrentRoom, gSwampEvent.CurrentGroup);
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Đội Quân Đang Tấn Công Địa Bàn!", 0);

			}
			else {
				Success = 1;

				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Thành Công Đã Chén Được Medusa!", 0);
				Sleep(5000);
				break;
			}
		}
	...
	if (gSwampEvent.SwampMapEnterOnlyWhenStarted == 1)
	{
		if (Success == 1 && gSwampEvent.SwampIfWonKeepOpenMapMinutes > 0)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Đã Được Chiến Thắng Vẫn Mở  %d Phút", gSwampEvent.SwampIfWonKeepOpenMapMinutes);
			...
			while (secondsopen > 0)
			{
				...
				{
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Sẽ Đóng Sau %d Phút", secondsopen / 60);
				}
		...
		if (Activate == TRUE)
		{
			gNotice.GCNoticeSendToAll(1, 0, 0, 0, 0, 0, "Bạn Sẽ Bị Đẩy Ra Ngoài Sau %d Giây", SWAMP_STANBYSECONDS);
			Sleep(SWAMP_STANBYSECONDS * 1000);
		}
		gSwampEvent.WarpOutside();
	}

	LogAdd(LOG_BLACK, "[SwampEvent] State -> Da Dong Cua");
