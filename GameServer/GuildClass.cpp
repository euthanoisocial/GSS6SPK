...
GUILD_INFO_STRUCT * CGuildClass::AddGuild(int number, char* guildname, GUILDMARK mark, char * mastername, int score)
{
	...
	if ( pNewNode != NULL )
	{
		...
		pNewNode->LevelDevote = 0;
		pNewNode->TotalDevote = 0;
#endif
...
#if(CUSTOM_GUILD_UPGRADE)
GUILD_INFO_STRUCT* CGuildClass::AddMember(char* guild_name, char* player_name, int aIndex, int totalcount, int iGuildStatus, int pServer, int Devote)
{
...
struct GU_GUILD_RANKING_SORT_DATA
{
	int Number;
	int Devote;
	GU_GUILD_RANKING_SORT_DATA(int a1, int a2)
	{
		Number = a1;
		Devote = a2;
	}
...
bool compareGuildRanking(GU_GUILD_RANKING_SORT_DATA a1, GU_GUILD_RANKING_SORT_DATA a2)
{
	return (a1.Devote > a2.Devote);
}

void CGuildClass::SendListGuild(int aIndex)
{
	...
	if (this->m_GuildMap.size()) {
		BYTE send[2048];
		PSWMSG_GUILD_UPGRADE_COUNT_SEND pMsg = { 0 };
		pMsg.header.set(0xA8, 0x1, 0);
		int size = sizeof(pMsg);
		pMsg.count = 0;

		std::vector<GU_GUILD_RANKING_SORT_DATA> GuildSort;
		GuildSort.clear();

		for (std::map<std::string, GUILD_INFO_STRUCT*>::iterator it = this->m_GuildMap.begin(); it != this->m_GuildMap.end(); ++it)
		{
			GuildSort.push_back(GU_GUILD_RANKING_SORT_DATA(it->second->Number, it->second->TotalDevote));
		}

		sort(GuildSort.begin(), GuildSort.end(), compareGuildRanking);

		for (int i = 0; i < min(50, GuildSort.size()); i++) //hien thi top 50 guild
		{
			GUILD_INFO_STRUCT* G = this->SearchGuild_Number(GuildSort[i].Number);
			if (G != NULL) {
				GUGuildInfo info;
				memcpy(info.Name, G->Name, sizeof(G->Name));
				memcpy(info.Master, G->Names[0], sizeof(G->Names[0]));
				info.Devote = G->TotalDevote;
				info.Level = G->LevelDevote;
