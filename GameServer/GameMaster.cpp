...
void CGameMaster::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			strcpy_s(info.CheckIP,lpMemScript->GetAsString());
...
void CGameMaster::SetGameMasterLevel(LPOBJ lpObj,int level) // OK
{
	...
	memcpy(info.CheckIP, lpObj->IpAddr, sizeof(info.CheckIP));
...
bool CGameMaster::CheckGameMasterLevel(LPOBJ lpObj,int level) // OK
{
	for(int n=0;n < this->m_count;n++)
	{
		if(strcmp(this->m_GameMasterInfo[n].Account,lpObj->Account) == 0 && strcmp(this->m_GameMasterInfo[n].Name,lpObj->Name) == 0 && this->m_GameMasterInfo[n].Level >= level && strcmp(this->m_GameMasterInfo[n].CheckIP, lpObj->IpAddr) == 0)
		{
			return 1;
		}
	}
