...
CMuunSystem::CMuunSystem() // OK
{
	...
	this->m_MuunCustomPetInfo.clear();
	...
	try
	{
		while(true)
		{
			...
			while(true)
			{
				...
				else if (section == 2) //Custom option new
				{
					if (strcmp("end", lpMemScript->GetAsString()) == 0)
					{
						break;
					...
					this->m_MuunCustomPetInfo.insert(std::pair<int, MUUN_SYSTEM_CUSTOM_INFO>(info.Index, info));
				}
				else
				{
					break;
...
//#if(GAMESERVER_UPDATE>=803)
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		for (int n = 0; n < MUUN_INVENTORY_WEAR_SIZE; n++)
		{
			...
			if (n == 0)
			{
				this->GCMuunItemChangeSend(lpObj->Index, n);
			}
...
bool CMuunSystem::CheckSpecialOption(LPOBJ lpObj,MUUN_SYSTEM_OPTION_INFO* lpInfo) // OK
{
	...
	if(lpInfo->PlayTime != -1 && ((DWORD)(lpInfo->PlayTime*10)) > (GetTickCount()-lpObj->ServerTickCount))
	{
		return 0;
	}
...
void CMuunSystem::CalcMuunOption(LPOBJ lpObj,bool flag) // OK
{
	...
	for(int n=0;n < MUUN_INVENTORY_WEAR_SIZE;n++)
	{
		...
		this->CalcMuunNewPetOption(lpObj, MuunSystemInfo.OptionIndex, flag); //Set New Option
	}
	#endif
}
void CMuunSystem::CalcMuunNewPetOption(LPOBJ lpObj,int OptionIndex, bool flag)
{
#if SLOTPET2 == 0
	if (flag != 0)
	{
	...
	std::map<int, MUUN_SYSTEM_CUSTOM_INFO>::iterator it = this->m_MuunCustomPetInfo.find(OptionIndex);

	if (it != this->m_MuunCustomPetInfo.end())
	{
		if (pEquipet->m_Durability > 0)
		{
			...
			//LogAdd(LOG_RED, "Add Dame Muun Pet NEw");
		}
	}
#endif
}
...
void CMuunSystem::CGMuunDropItemRecv(PMSG_MUUN_ITEM_DROP_SEND* lpMsg, int aIndex) // OK
{
#if SLOTPET2 == 0
	LPOBJ lpObj = &gObj[aIndex];
	...
	//LogAdd(LOG_RED, "MuunDrop : %d, x %d y %d", lpMsg->slot, lpMsg->x, lpMsg->y);
	CItem* lpItem = &lpObj->MuunInventory[lpMsg->slot];
	gMap[lpObj->Map].ItemDrop(lpItem->m_Index, lpItem->m_Level, lpItem->m_Durability, lpMsg->x, lpMsg->y, lpItem->m_Option1, lpItem->m_Option2, lpItem->m_Option3, lpItem->m_NewOption, lpItem->m_SetOption, lpItem->m_Serial, aIndex, lpItem->m_PetItemLevel, lpItem->m_PetItemExp, lpItem->m_JewelOfHarmonyOption, lpItem->m_ItemOptionEx, lpItem->m_SocketOption, lpItem->m_SocketOptionBonus, lpItem->m_PeriodicItemTime);
...
void CMuunSystem::BMuunPickUp(int aIndex, int ItemIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
void CMuunSystem::CGMuunItemSellRecv(PMSG_MUUN_ITEM_SELL_RECV* lpMsg,int aIndex) // OK
{
	...
	this->GCMuunItemDeleteSend(lpObj->Index, lpMsg->slot, 1);
...
void CMuunSystem::GDMuunInventorySaveSend(int aIndex) // OK
{
	...
	//LogAdd(LOG_RED, "GDMuunInventorySaveSend");
	#endif
}
