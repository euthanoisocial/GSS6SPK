...
void JGConnectAccountRecv(SDHP_CONNECT_ACCOUNT_RECV* lpMsg) // OK
{
	...
	{
		if(lpMsg->result != 1) LogAdd(LOG_RED,"[FakeOnline] Connect %s result (%d)",gObj[lpMsg->index].Name,lpMsg->result);
		GDCharacterInfoSend(lpMsg->index,info->Name);
		LogAdd(LOG_RED, "[FakeOnline]  [TK: %s NV: %s] Da Online Vao Server", info->Account, info->Name);
		goto FakeOKNext;
	}
#endif
	if(lpMsg->result != 1)
...
FakeOKNext:	
	...
	LogAddConnect(LOG_BLUE,"Tài khoản [%s] đã đăng nhâp!",gObj[lpMsg->index].Account);
}
...
void JGMapServerMoveAuthRecv(SDHP_MAP_SERVER_MOVE_AUTH_RECV* lpMsg) // OK
{
	...
	LogAddConnect(LOG_GREEN,"Tài khoản [%s] đã đăng nhâp!",gObj[lpMsg->index].Account);
}
