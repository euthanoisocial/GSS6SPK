...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	#if(SPK_DUNGLUYEN)
	if (gBDungLuyenItem.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
	#endif
	#if(SPK_ITEMTRADER)
	if (gGC_ItemTrader.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
	#endif
	...
	//tue add
#if(CHIEN_TRUONG_CO)	
		if (gChienTruongCo.Dialog(lpObj->Index, lpNpc->Index) != 0)
		{
			return 1;
		}
#endif
	...
	#if(CTCMINI)
	if (gCTCMini.Dialog(lpObj->Index, lpNpc->Index) != 0)
	{
		return 1;
	}
	#endif
	...
	#if (CUSTOM_HARMONY)
	if (gCustomHarmony.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
	#endif
	#if (CongHuongV2)
	if (gCBCongHuong.mCH_Enabled)
	{
		if (gCBCongHuong.TalkNPC(lpObj->Index, lpNpc->Index) != 0)
		{
			return 1;
		}
	}
	#endif
	#if(CB_EventArenaPvP)
	if (gCBArenaPVP.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
	#endif
	...
	#if(THANMA)
	if (gThanMaChien.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
	#endif
	#if(EVENT_PK)
	if (gBsVEvent.Dialog(lpObj,lpNpc))
	{
		return 1;
	}
	#endif
	switch(lpNpc->Class)
	{
		case 205: this->NpcCrywolfAltar(lpNpc,lpObj);				break;
		case 206: this->NpcCrywolfAltar(lpNpc,lpObj);				break;
		case 207: this->NpcCrywolfAltar(lpNpc,lpObj);				break;
		case 208: this->NpcCrywolfAltar(lpNpc,lpObj);				break;
		case 209: this->NpcCrywolfAltar(lpNpc,lpObj);				break;
		case 216: this->NpcCastleSiegeCrown(lpNpc,lpObj);			break;
		case 217: this->NpcCastleSiegeCrownSwitch(lpNpc,lpObj);		break;
		case 218: this->NpcCastleSiegeCrownSwitch(lpNpc,lpObj);		break;
		case 219: this->NpcCastleSiegeGateSwitch(lpNpc,lpObj);		break;
		case 220: this->NpcCastleSiegeGuard(lpNpc,lpObj);			break;
		case 221: this->NpcCastleSiegeWeaponOffense(lpNpc,lpObj);	break;
		case 222: this->NpcCastleSiegeWeaponDefense(lpNpc,lpObj);	break;
		case 223: this->NpcCastleSiegeSenior(lpNpc,lpObj);			break;
		case 224: this->NpcCastleSiegeGuardsman(lpNpc,lpObj);		break;
		case 226: this->NpcTrainer(lpNpc,lpObj);					break;
		case 232: this->NpcAngelKing(lpNpc,lpObj);					break;
		case 233: this->NpcAngelMessenger(lpNpc,lpObj);				break;
		case 234: this->NpcServerTransfer(lpNpc,lpObj);				break;
		case 236: gCustomNpcCollector.NpcTalk(lpNpc,lpObj);			break;
		case 237: this->NpcCharon(lpNpc,lpObj);						break;
		case 238: this->NpcChaosGoblin(lpNpc,lpObj);				break;
		case 240: this->NpcWarehouse(lpNpc,lpObj);					break;
		case 241: this->NpcGuildMaster(lpNpc,lpObj);				break;
		case 249: this->NpcGuardTalk(lpNpc,lpObj);					break;
		case 256: this->NpcLehap(lpNpc,lpObj);						break;
		case 257: this->NpcElfBuffer(lpNpc,lpObj);					break;
		case 367: this->NpcKanturuGate(lpNpc,lpObj);				break;
		case 368: this->NpcElpis(lpNpc,lpObj);						break;
		case 369: this->NpcOsbourne(lpNpc,lpObj);					break;
		case 370: this->NpcJerridon(lpNpc,lpObj);					break;
		case 371: this->NpcLeoTheHelper(lpNpc,lpObj);				break;
		case 375: this->NpcChaosCardMaster(lpNpc,lpObj);			break;
		case 380: this->NpcStoneStatue(lpNpc,lpObj);				break;
		case 383: this->NpcYellowStorage(lpNpc,lpObj);				break;
		case 384: this->NpcBlueStorage(lpNpc,lpObj);				break;
		case 385: this->NpcMirageTheMummy(lpNpc,lpObj);				break;
		case 450:
		#if(SKY_EVENT == 1)
			if (gSkyEvent.NPCEnabled == 1 && lpObj->Map == MAP_DEVIAS)
			{
				gSkyEvent.NpcTalk(lpNpc, lpObj);
			}
			else
			{
				if (gSkyEvent.NPCEnabled == 0 && lpObj->Map == MAP_DEVIAS)
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "[SkyEvent] the event has not started");
				}
				else
				{
			...
			#endif
		break;
		case 452:			this->NpcSeedMaster(lpNpc,lpObj);			break;
		case 453:			this->NpcSeedResearcher(lpNpc,lpObj);		break;
		case 465:
		...
		break;
		case 467:			this->NpcSnowman(lpNpc,lpObj);				break;
		case 468:			this->NpcChristmas(lpNpc,lpObj);			break;
		case 469:			this->NpcChristmas(lpNpc,lpObj);			break;
		case 470:			this->NpcChristmas(lpNpc,lpObj);			break;
		case 471:			this->NpcChristmas(lpNpc,lpObj);			break;
		case 472:			this->NpcChristmas(lpNpc,lpObj);			break;
		case 473:			this->NpcChristmas(lpNpc,lpObj);			break;
		case 474:			this->NpcChristmas(lpNpc,lpObj);			break;
		case 475:			this->NpcChristmas(lpNpc,lpObj);			break;
		case 478:			this->NpcDelgado(lpNpc,lpObj);				break;
		case 479:			this->NpcTitus(lpNpc,lpObj);				break;
		case 522:			this->NpcJerryTheAdviser(lpNpc,lpObj);		break;
		case 540:			this->NpcSartiganTheAngel(lpNpc,lpObj);		break;
		case 541:			this->NpcSilverRewardChest(lpNpc,lpObj);	break;
		case 542:			this->NpcGoldenRewardChest(lpNpc,lpObj);	break;
		case 543:			this->NpcGensDuprianSteward(lpNpc,lpObj);	break;
		case 544:			this->NpcGensVarnertSteward(lpNpc,lpObj);	break;
		case 546:			this->NpcRaul(lpNpc,lpObj);					break;
		case 547:			this->NpcJulia(lpNpc,lpObj);				break;
		case 566:			this->NpcTercia(lpNpc, lpObj);				break;
		case 567:			this->NpcVeina(lpNpc, lpObj);				break;
		case 568:			this->NpcZyro(lpNpc, lpObj);				break;
		case 581:			this->NpcDeruvish(lpNpc, lpObj);			break;
		case 579:			this->NpcDavid(lpNpc,lpObj);				break;
		case 602:			this->NpcShopRuud(lpNpc,lpObj);				break;
		default:
		if(lpNpc->Class == gServerInfo.m_CustomNpcTalkBaul)
		{
			this->NpcWarehouse(lpNpc,lpObj);
		}
		else if(lpNpc->Class == gServerInfo.m_CustomNpcTalkQuest)
		{
			this->NpcLesnar(lpNpc,lpObj);
		}
		else if(lpNpc->Class == gServerInfo.m_CustomNpcTalkGoblin)
		{
			this->NpcChaosGoblin(lpNpc,lpObj);
		}
		else if(lpNpc->Class == gServerInfo.m_CustomNpcTalkTrainer)
		{
			this->NpcTrainer(lpNpc,lpObj);
		}
		else if(lpNpc->Class == gServerInfo.m_CustomNpcTalkGuard)
		{
			this->NpcGuardTalk(lpNpc,lpObj);
		}
		else if(lpNpc->Class == gServerInfo.m_CustomNpcTalkNixie)
		{
			this->SpiritNixie(lpNpc,lpObj);
		}
		return 0;
	}
	return 1;
...
#	endif
	...
	if (lpObj->WarehouseTimeCount > 0)
	{
		...
		lpObj->Interface.state = 0;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(136), lpObj->WarehouseTimeCount);
		return;
	}
...
void CNpcTalk::NpcGuildMaster(LPOBJ lpNpc, LPOBJ lpObj) // OK
{
	if (lpObj->GuildNumber > 0)
	{
		if (gServerInfo.m_GuildWarehouseEnable == 1)
		{
			...
			{
				gWarehouse.GDWarehouseGuildConsult(lpObj->Index, lpObj->GuildName);
			}
		}
		else
		{
...
void CNpcTalk::NpcElfBuffer(LPOBJ lpNpc, LPOBJ lpObj) // OK
{
	...
	if (gEffectManager.CheckEffect(lpObj, EFFECT_ELF_BUFFER) != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn Đã Nhận Buff Rồi");
		return;
	}
#	endif
	...
	else
	{
		...
		gEffectManager.AddEffect(lpObj, 0, EFFECT_ELF_BUFFER, gServerInfo.m_ElfBufferTime[lpObj->AccountLevel], (gServerInfo.m_ElfBufferDamageConstA + (lpObj->Level / gServerInfo.m_ElfBufferDamageConstB)), (gServerInfo.m_ElfBufferDefenseConstA + (lpObj->Level / gServerInfo.m_ElfBufferDefenseConstB)), 0, 0);
	}
}
...
void CNpcTalk::NpcChristmas(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	switch(lpNpc->Class)
	{
		case 468: gEffectManager.AddEffect(lpObj,0,EFFECT_CHRISTMAS4,1800,30,0,0,0);			break;
		case 469: gEffectManager.AddEffect(lpObj,0,EFFECT_CHRISTMAS5,1800,100,0,0,0);			break;
		case 472: gEffectManager.AddEffect(lpObj,0,EFFECT_CHRISTMAS2,1800,500,0,0,0);			break;
		case 473: gEffectManager.AddEffect(lpObj,0,EFFECT_CHRISTMAS3,1800,500,0,0,0);			break;
		case 474: gEffectManager.AddEffect(lpObj,0,EFFECT_CHRISTMAS6,1800,15,0,0,0);			break;
		case 475: gEffectManager.AddEffect(lpObj,0,EFFECT_CHRISTMAS7,1800,10,0,0,0);			break;
	}
}
...
void CNpcTalk::NpcShopRuud(LPOBJ lpNpc,LPOBJ lpObj)
{
	...
	PMSG_NPC_TALK_SEND pMsg;
	pMsg.header.setE(0xEE,sizeof(pMsg));
	pMsg.result = 100;
	DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
...
void CNpcTalk::CGNpcTalkCloseRecv(int aIndex) // OK
{
	...
	switch(lpObj->Interface.type)
	{
		case INTERFACE_SHOP:
			...
			break;
		case INTERFACE_WAREHOUSE:			gWarehouse.CGWarehouseClose(aIndex);			break;
		case INTERFACE_TRAINER:
			gChaosBox.ChaosBoxInit(lpObj);
			gObjInventoryCommit(aIndex);
			break;
		case INTERFACE_QUEST_WORLD:			lpObj->QuestWorldMonsterClass = -1;			break;
	}
