...
void CObjectManager::ObjectSetStateProc() // OK
{
	for(int n=0;n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Type == OBJECT_MONSTER)
		{
			...
			#if SO_LUONG_BOSS
			gInvasionManager.CheckBossCount(lpObj->Class, lpObj->Map);
			#endif
		}
		...
		if(lpObj->Type == OBJECT_USER)
		{
			...
			#if(SAU_ANTIHACK)
			lpObj->BProtectMove_Time = GetTickCount();
			#endif
			...
			if (lpObj->Guild != NULL)
			{
				GuildUpgrade.AddDevoteEff(lpObj);
			}
			#endif
		}
	}
...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	{
		lpObj->CloseCount	= 1;
		gObjMoveGate(lpObj->Index, 22); // Gate Davias
	}
	...
	#if(SPK_DUNGLUYEN)
	if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_DUNGLUYENITEM)
	{
		gBDungLuyenItem.SetStateInterface(lpObj->Index);
	}
	#endif
	...
	gGC_ItemTrader.RollBackItem(lpObj->Index, 0);

	#if (CUSTOM_HARMONY)
	if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_DATAOHOA)
	{
		gCustomHarmony.SetStateInterface(lpObj->Index);
	}
	#endif
	#if (CongHuongV2)
	gCBCongHuong.CH_ClearDB(lpObj->Index, 0);
	#endif
	//gReconnect.SetReconnectInfo(lpObj);
	#if(CB_EventArenaPvP)
	if (lpObj->ArenaEventStatus == 1)
	{
		gCBArenaPVP.DelUser(lpObj->Index);
		gObjMoveGate(aIndex, 17);
	}
#endif

#if(PARTYSETPASS)
	gPartySetPass.PartySetPassSearchDelFromList(lpObj->Name);
#endif
	...
	#endif
	#if(PARTYSEARCH==1)
		if (gPartySearch.DelAfterRelog == 1){
	...
	LogAddConnect(LOG_ALERT,"[%s] đã đăng xuất!",lpObj->Name);
...
void CObjectManager::CharacterGameCloseSet(int aIndex,int type) // OK
{
	...
	lpObj->CloseCount = gServerInfo.m_TimeOutGame;
...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj) // OK
{
	...
	#if(THANMA)
	else if (gThanMaChien.CheckPlayerTarget(lpObj))
	{
		result = gThanMaChien.GetUserRespawnLocation(lpObj, &gate, &map, &x, &y, &dir, &level);
	}
	#endif
	#if(EVENT_PK)
	else if(gBsVEvent.CheckPlayerTarget(lpObj))
	{
		result = gBsVEvent.GetUserRespawnLocation(lpObj,&gate,&map,&x,&y,&dir,&level);
	}
	#endif
	...
	else if (gRespawnLocation.IsMap(lpObj))
	{
		result = gRespawnLocation.GetUserRespawnLocation(lpObj,&gate,&map,&x,&y,&dir,&level);
	}
	#if(CTCMINI)
	else if (lpObj->Map == MapCTC)
	{
		result = gCTCMini.GetUserRespawnLocation(lpObj, &gate, &map, &x, &y, &dir, &level);
	}
	#endif
	#if(CB_EventArenaPvP)
	if (gCBArenaPVP.CheckEnteredUser(lpObj->Index))
	{
		result = gCBArenaPVP.GetUserRespawnLocation(lpObj, &gate, &map, &x, &y, &dir, &level);
	}
	#endif
	if(result != 0)
	{
...
void CObjectManager::CharacterCalcExperienceSplit(LPOBJ lpObj,LPOBJ lpMonster,float damage,int flag) // OK
{
...
void CObjectManager::CharacterCalcExperienceAlone(LPOBJ lpObj,LPOBJ lpMonster,float damage,int flag,bool type,float AttackDamage) // OK
{
	if(lpMonster->Type != OBJECT_MONSTER)
	{
		return;
	...
	if(gMasterSkillTree.CheckMasterLevel(lpObj) == 0)
	{
			...
			else if (lpObj->AttackCustomOffline == 1) 
			{
				expserver = gServerInfo.m_OffAttackExperienceRate[lpObj->AccountLevel];
			}
			else if (lpObj->AutoResetEnable == 1) 
			{
				lpObj->AutoResetEnable =  ( gServerInfo.m_CommandResetAutoEnable[lpObj->AccountLevel] == 0 ) ? 0 : lpObj->AutoResetEnable;
				expserver = gServerInfo.m_AutoResetExperienceRate[lpObj->AccountLevel];
			}
			#if(RESET_VIP_AUTO_ON)
			else if (lpObj->AutoResetVipEnable == 1) 
			{
				expserver = gServerInfo.m_AutoResetExperienceRate[lpObj->AccountLevel];
			}
			#endif
	...
	if (gZenDrop.IsZenDropActive(lpMonster->Map) == true)
	{
		DWORD dwZenDropMoney = gZenDrop.GetZenAmount(lpMonster->Map, lpMonster->Level);

		if (dwZenDropMoney > 0)
		{
			lpMonster->Money = dwZenDropMoney;
		}
	}
...
void CObjectManager::CharacterCalcExperienceParty(LPOBJ lpObj,LPOBJ lpMonster,float damage,int flag) // OK
{
	if(OBJECT_RANGE(lpObj->PartyNumber) == 0)
	{
		return;
	...
	for(int n=0;n < MAX_PARTY_USER;n++)
	{
		...
		if(gMasterSkillTree.CheckMasterLevel(lpTarget) == 0)
		{
			...
			else if (lpTarget->AttackCustomOffline == 1) 
			{
...
bool CObjectManager::CharacterLevelUpPointAdd(LPOBJ lpObj,int type,int amount) // OK
{
	...
	if(stat == &lpObj->Leadership && ((*stat)+amount) > gServerInfo.m_MaxStatPointCMD[lpObj->AccountLevel]){
		...
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(268),gServerInfo.m_MaxStatPointCMD[lpObj->AccountLevel]);
		return 0;
	}
	else if(((*stat)+amount) > gServerInfo.m_MaxStatPoint[lpObj->AccountLevel])
	{
...
{
	if(INVENTORY_FULL_RANGE(SourceSlot) == 0 || INVENTORY_FULL_RANGE(TargetSlot) == 0 || lpObj->Inventory[SourceSlot].IsItem() == 0 || lpObj->Inventory[TargetSlot].IsItem() == 0 || lpObj->Inventory[TargetSlot].m_IsPeriodicItem != 0)
	{
		return 0;
	}
	...
	if(gCustomPet.PetIsInmortal(lpItem->m_Index)) // Fenrir
	{
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	while(change != 0)
	{
		...
		#if SLOTPET2
		gCustomPet.CalcCustomPet2Option(lpObj, 1);
		#endif
		gCustomWing.CalcItemCustomOption(lpObj, 1);

		#if(HT_THUE_FLAG)
		ThueFlag.CharacterCalc(lpObj->Index, 1);
		#endif
		#if(CUSTOM_GUILD_UPGRADE)
		GuildUpgrade.CaclCharacter(lpObj, 1);
		#endif
		#if (BUFF_VUKHI_ICON)
		gCustomStartItemDame.CaclCharacter(lpObj, 1);
		#endif
		#if (BUFF_SET_ICON)
		gCustomStartSetItemDame.CalcCustomSetItemOption(lpObj, 1);
		#endif
		#if(DANH_HIEU_NEW)
		gSPKDanhHieu.GetOptionDanhHieu(lpObj, 1);
		#endif
		#if(QUAN_HAM_NEW)
		gSPKQuanHam.GetOptionQuanHam(lpObj, 1);
		#endif
		#if(TU_LUYEN_NEW)
		gSPKTuLuyen.GetOptionTuLuyen(lpObj, 1);
		#endif
		#if(NEW_VIP_NEW)
		gSPKNewVip.GetOptionNewVip(lpObj, 1);
		#endif
		#if(B_HON_HOAN)
		gBHonHoan.GetOptionHonHoan(lpObj, 1);
		#endif
		#if SLOTPET2 == 0
		...
		#endif
		#if(SLOTPET2)
	     g_pNewMyInventory.CalcItemCommonOption(lpObj, 1);
		#endif
		#if ACTIVE_VKR
		 GITEM00.Option(lpObj, 1);
		 GITEM06.Option(lpObj, 1);
		 GITEM07.Option(lpObj, 1);
		 GITEM08.Option(lpObj, 1);
		 GITEM09.Option(lpObj, 1);
		 GITEM10.Option(lpObj, 1);
		 GITEM11.Option(lpObj, 1);
		 GITEM12.Option(lpObj, 1);
		 GITEM13.Option(lpObj, 1);
		#endif
		for(int n=0;n < INVENTORY_WEAR_SIZE;n++)
		{
	...
	if(lpObj->Class == CLASS_DW)
	{
		...
		lpObj->DWDamageMultiplierRate = Energy/100;
		lpObj->DWDamageMultiplierRate = ((lpObj->DWDamageMultiplierRate > gServerInfo.m_DWDamageMultiplierMaxRate) ? gServerInfo.m_DWDamageMultiplierMaxRate : lpObj->DWDamageMultiplierRate);
	}
	...
	else if(lpObj->Class == CLASS_FE)
	{
		...
		lpObj->ELFDamageMultiplierRate = Dexterity/100;
		lpObj->ELFDamageMultiplierRate = ((lpObj->ELFDamageMultiplierRate > gServerInfo.m_ELFDamageMultiplierMaxRate) ? gServerInfo.m_ELFDamageMultiplierMaxRate : lpObj->ELFDamageMultiplierRate);
	...
	else if(lpObj->Class == CLASS_MG)
	{
		...
		lpObj->MGDamageMultiplierRate = 200 + (Energy / gServerInfo.m_MGDamageMultiplierConstA);//gServerInfo.m_MGDamageMultiplierRatePhysic
		lpObj->MGDamageMultiplierRatePhysic = gServerInfo.m_MGDamageMultiplierRatePhysic;
		lpObj->MGDamageMultiplierRate = ((lpObj->MGDamageMultiplierRate>gServerInfo.m_MGDamageMultiplierMaxRate) ? gServerInfo.m_MGDamageMultiplierMaxRate : lpObj->MGDamageMultiplierRate);
	}
	...
	else if(lpObj->Class == CLASS_SU)
	{
		...
		lpObj->SUDamageMultiplierRate = Energy/100;
		lpObj->SUDamageMultiplierRate = ((lpObj->SUDamageMultiplierRate > gServerInfo.m_SUDamageMultiplierMaxRate) ? gServerInfo.m_SUDamageMultiplierMaxRate : lpObj->SUDamageMultiplierRate);
	}
	...
	#if SLOTPET2
	gCustomPet.CalcCustomPet2Option(lpObj, 0);
	#endif
	gCustomWing.CalcItemCustomOption(lpObj, 0);
	#if(HT_THUE_FLAG)
	ThueFlag.CharacterCalc(lpObj->Index, 0);
	#endif
	#if (BUFF_VUKHI_ICON)
	gCustomStartItemDame.CaclCharacter(lpObj, 0);
	#endif

	#if(CUSTOM_GUILD_UPGRADE)
	GuildUpgrade.CaclCharacter(lpObj, 0);
	#endif

	#if (BUFF_SET_ICON)
	gCustomStartSetItemDame.CalcCustomSetItemOption(lpObj, 0);
	#endif

	#if(DANH_HIEU_NEW)
	gSPKDanhHieu.GetOptionDanhHieu(lpObj, 0);
	#endif
	#if(QUAN_HAM_NEW)
	gSPKQuanHam.GetOptionQuanHam(lpObj, 0);
	#endif
	#if(TU_LUYEN_NEW)
	gSPKTuLuyen.GetOptionTuLuyen(lpObj, 0);
	#endif
	#if(NEW_VIP_NEW)
	gSPKNewVip.GetOptionNewVip(lpObj, 0);
	#endif
	#if(B_HON_HOAN)
	gBHonHoan.GetOptionHonHoan(lpObj, 0);
	#endif
	#if SLOTPET2 == 0
	#if(GAMESERVER_UPDATE>=603)
	...
	#endif
	#if(SLOTPET2)
	g_pNewMyInventory.CalcItemCommonOption(lpObj, 0);
	#endif

	#if ACTIVE_VKR
	GITEM00.Option(lpObj, 0);
	GITEM06.Option(lpObj, 0);
	GITEM07.Option(lpObj, 0);
	GITEM08.Option(lpObj, 0);
	GITEM09.Option(lpObj, 0);
	GITEM10.Option(lpObj, 0);
	GITEM11.Option(lpObj, 0);
	GITEM12.Option(lpObj, 0);
	GITEM13.Option(lpObj, 0);
	#endif
	...
	#if LUC_CHIEN
	gCustomLucChien.CalcLucChien(lpObj->Index);
	#endif
}
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	#if(DANH_HIEU_NEW)
	lpObj->rDanhHieu = lpMsg->rDanhHieu;
	#endif
	#if(QUAN_HAM_NEW)
	lpObj->rQuanHam = lpMsg->rQuanHam;
	#endif
	#if(TU_LUYEN_NEW)
	lpObj->rTuLuyen = lpMsg->rTuLuyen;
	#endif
	#if(NEW_VIP_NEW)
	lpObj->rNewVip = lpMsg->rNewVip;
	#endif
	#if(B_HON_HOAN)
	lpObj->m_RankTitle5 = lpMsg->m_RankTitle5;
	#endif

	#if RELIFE
	lpObj->rResetLife = lpMsg->rResetLife;
	#endif
...
#if(HT_THUE_FLAG)
	lpObj->m_RankTitle6 = lpMsg->m_RankTitle6;
	lpObj->CheckFlag = 0;
#endif
	...
	lpObj->SDAutoRecuperationTime = GetTickCount();
	lpObj->CharSaveTime = GetTickCount();
	memcpy(lpObj->DayReg,lpMsg->DayReg,sizeof(lpObj->DayReg));
	if(gServerInfo.m_CheckDaySub == 1) // Sub 3
	{
		if(string2timestamp(lpObj->DayReg) < string2timestamp(gServerInfo.DayConfig))
		{
			LogAdd(LOG_RED,"%s đăng ký ngày %s đã vào Sub ngày %s",lpObj->Name, lpObj->DayReg,gServerInfo.DayConfig);
			lpObj->CloseCount	= 6;
			lpObj->CloseType	= 3;
			return 1;
		}
	...
	else // Sub 1 & 2
	{
		if(string2timestamp(lpObj->DayReg) >= string2timestamp(gServerInfo.DayConfig))
		{
			LogAdd(LOG_RED,"%s đăng ký ngày %s đã vào Sub ngày %s",lpObj->Name,gServerInfo.DayConfig, lpObj->DayReg);
			lpObj->CloseCount	= 6;
			lpObj->CloseType	= 3;
			return 1;
		}
	...
	lpObj->NhanMocNap = lpMsg->NhanMocNap;
	//LogAdd(LOG_RED, "[TongNap] %d MocNhan (%d)", lpObj->TongNap, lpObj->NhanMocNap); // Login Mess
#endif
	return 1;
}

void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,float damage,int DamageType,int flag,int type,int skill,float ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			{
				#if BAG_EVENT_DROP
				gObjMonsterDieGiveItem(lpTarget, lpObj);
				#else
				gObjAddMsgSendDelay(lpTarget,1,SummonIndex,500,0);
				#endif
				//#if BAG_EVENT_DROP
				//if (!gItemBagManager.DropItemByMonsterClass(lpObj->Class, lpTarget, lpObj->Map, lpObj->X, lpObj->Y))
				//{
				//	gObjAddMsgSendDelay(lpTarget, 1, SummonIndex, 500, 0);
				//}
				//#endif
