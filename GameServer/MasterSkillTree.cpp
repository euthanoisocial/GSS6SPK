...
int CMasterSkillTree::GetMasterSkillDamageMin(LPOBJ lpObj,int index) // OK
{
	...
	switch(index)
	{
		...
		case SKILL_ICE_STORM:
			...
			//damage += this->GetMasterSkillValue(lpObj,MASTER_SKILL_ADD_ICE_STORM_STUN); // ADD STUN
			break;
		case SKILL_NOVA:
...
void CMasterSkillTree::SetMasterLevelExperienceTable() // OK
{
	...
	{
		#if(MAX_LEVEL)
		var1 = n + MAX_CHARACTER_LEVEL;	//code exp mt lvl 2k		
		var3 = ((((var1 + 9)*var1)*var1) * 2);		
		var2 = var1 - 255;		
		var3 += ((((var2 + 9)*var2)*var2) * 5);		
		var3 = (var3 - 3892250000) / 2;
		#else
		var1 = n+MAX_CHARACTER_LEVEL; //mac dinh		
...
void CMasterSkillTree::InsertOption(LPOBJ lpObj,int index,int value,bool flag) // OK
{
	...
	switch(index)
	{
		...
		case MASTER_SKILL_ADD_BLOOD_STORM:
			if((value=gSkillManager.AddSkill(lpObj,SKILL_FIRE_SLASH,0)) >= 0)
			//if((value=gSkillManager.AddSkill(lpObj,SKILL_BLOOD_STORM,0)) >= 0)
			{
				gSkillManager.GCSkillAddSend(lpObj->Index,value,SKILL_FIRE_SLASH,0,0);
				//gSkillManager.GCSkillAddSend(lpObj->Index,value,SKILL_BLOOD_STORM,0,0);
			}
			break;
				case MASTER_SKILL_ADD_BLOOD_STORM_IMPROVED:
			if((value=gSkillManager.AddSkill(lpObj,SKILL_SWORD_SLASH,0)) >= 0)
			//if((value=gSkillManager.AddSkill(lpObj,SKILL_BLOOD_STORM,0)) >= 0)
			{
				gSkillManager.GCSkillAddSend(lpObj->Index,value,SKILL_SWORD_SLASH,0,0);
				//gSkillManager.GCSkillAddSend(lpObj->Index,value,SKILL_BLOOD_STORM,0,0);
			}
			break;
		...
		*/
		...
		case MASTER_SKILL_ADD_SUMMON_SATYROS:
			if((value=gSkillManager.AddSkill(lpObj,SKILL_SUMMON7,0)) >= 0)
			{
				gSkillManager.GCSkillAddSend(lpObj->Index,value,SKILL_SUMMON7,0,0);
			}
			break;
		...
		case MASTER_SKILL_ADD_EARTH_PRISON:
			if((value=gSkillManager.AddSkill(lpObj,SKILL_SWORD_POWER,0)) >= 0)
			//if((value=gSkillManager.AddSkill(lpObj,SKILL_EARTH_PRISON,0)) >= 0)
			{
				gSkillManager.GCSkillAddSend(lpObj->Index,value,SKILL_SWORD_POWER,0,0);
				//gSkillManager.GCSkillAddSend(lpObj->Index,value,SKILL_EARTH_PRISON,0,0);
			}
			break;
			case MASTER_SKILL_ADD_EARTH_PRISON_IMPROVED:
			if((value=gSkillManager.AddSkill(lpObj,SKILL_SLEEP,0)) >= 0)
			//if((value=gSkillManager.AddSkill(lpObj,SKILL_EARTH_PRISON,0)) >= 0)
			{
				gSkillManager.GCSkillAddSend(lpObj->Index,value,SKILL_SLEEP,0,0);
				//gSkillManager.GCSkillAddSend(lpObj->Index,value,SKILL_EARTH_PRISON,0,0);
			}
			break;
		...
		case MASTER_SKILL_ADD_WEAPON_FURY:
			if(lpObj->Inventory[0].m_Index >= GET_ITEM(0,0) && lpObj->Inventory[0].m_Index < GET_ITEM(1,0) && lpObj->Inventory[0].m_TwoHand == 0 &&
			lpObj->Inventory[0].m_Index >= GET_ITEM(0,0) && lpObj->Inventory[0].m_Index < GET_ITEM(1,0) && lpObj->Inventory[0].m_TwoHand != 0 &&
			lpObj->Inventory[0].m_Index >= GET_ITEM(2,0) && lpObj->Inventory[0].m_Index < GET_ITEM(3,0) &&
			lpObj->Inventory[0].m_Index >= GET_ITEM(3,0) && lpObj->Inventory[0].m_Index < GET_ITEM(4,0))

			{
				lpObj->PhysiDamageMin += value;
				lpObj->PhysiDamageMax += value;
				lpObj->PhysiSpeed += value;
				lpObj->AttackSuccessRate += value;
			}
			break;
		//---
		case MASTER_SKILL_ADD_SHIELD_BLOCK:
			if(lpObj->Inventory[1].m_Index >= GET_ITEM(6,0) && lpObj->Inventory[1].m_Index < GET_ITEM(7,0))
			...
			break;
		//---
		case MASTER_SKILL_ADD_SHIELD_BLOCK_UPGRADE:
			if(lpObj->Inventory[1].m_Index >= GET_ITEM(6,0) && lpObj->Inventory[1].m_Index < GET_ITEM(7,0))
			{
				lpObj->ResistDoubleDamageRate += value;
				lpObj->ResistExcellentDamageRate += value;
				lpObj->ResistCriticalDamageRate += value;
			}
			break;
		//---
		case MASTER_SKILL_ADD_STRONG_MIND:
			lpObj->ResistStunRate += value;
			break;
		//---
		case MASTER_SKILL_ADD_STEEL_ARMOR:
			lpObj->ResistIgnoreDefenseRate += value;
			break;
		//--
		case MASTER_SKILL_ADD_RECOVERY_HP:
			lpObj->OffensiveFullHPRestoreRate += value;
			break;
		//---
		case MASTER_SKILL_ADD_RECOVERY_SHIELD:
			lpObj->OffensiveFullSDRestoreRate += value;
			break;
		//---
		#if(TIDUS_FIX_MT)
		case MASTER_SKILL_ADD_DARK_SPIRIT_CRITICAL_DAMAGE_RATE:
			lpObj->CriticalDamageRate += value;
			break;
		case MASTER_SKILL_ADD_DARK_SPIRIT_EXCELLENT_DAMAGE_RATE:
			lpObj->ExcellentDamageRate += value;
			break;
		case MASTER_SKILL_ADD_DARK_SPIRIT_ATTACK_SPEED:
			lpObj->AttackSpeed += value;
			break;
		case MASTER_SKILL_ADD_DARK_SPIRIT_DOUBLE_DAMAGE_RATE:
			lpObj->DoubleDamageRate += value;
			break;
		case MASTER_SKILL_ADD_DARK_SPIRIT_IGNORE_DEFENSE_RATE:
			lpObj->IgnoreDefenseRate += value;
			break;
			
		case MASTER_SKILL_ADD_DARK_SPIRIT_DAMAGE:
			if (lpObj->Inventory[1].m_Index == GET_ITEM(13, 5))
			{
				lpObj->MagicDamageMin += value;
				lpObj->MagicDamageMax += value;
				lpObj->PhysiDamageMinLeft += value;
...
void CMasterSkillTree::GCMasterSkillList2Send(int aIndex) // OK
{
	...
	PMSG_MASTER_SKILL_LIST_SEND_ pMsg2;

	pMsg2.header.set(0xF3, 0xF4, 0);

	int size2 = sizeof(pMsg2);

	pMsg2.count = 0;

	PMSG_MASTER_SKILL_LIST_ info2;
...
//gLog.Output(LOG_CHAOS_MIX,"[%s] [%d] --> [%d]",lpObj->Name, info.skill, info.RelatedSkill);
	...
	pMsg2.header.size[0] = SET_NUMBERHB(size2);
	pMsg2.header.size[1] = SET_NUMBERLB(size2);

	memcpy(send2,&pMsg2,sizeof(pMsg2));

	DataSend(lpObj->Index, send2, size2);
	#endif
}
...
void CMasterSkillTree::GDMasterSkillTreeSaveSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_MASTER_SKILL_LIST; n++)
	{
		gSkillManager.SkillByteConvert(pMsg.MasterSkill[n], &lpObj->MasterSkill[n]);
	}
