...
void CThanMaChien::SetState_CLEAN() // OK
{
	...
	if(this->ScoreBlue > this->ScoreRed)
	{
		...
		this->NoticeSendToAll(0,"Đội Thiên Thần Thắng");
	}
	else if(this->ScoreRed > this->ScoreBlue)
	{
		TeamReward = TM_TEAMACQUY;
		this->NoticeSendToAll(0,"Đội Ác Quỷ Thắng");
	}
	for(int n=0;n < MAX_THANMACHIEN_USER;n++)
	{
		if(OBJECT_RANGE(this->User[n].Index) != 0)
		{
			...
			if(this->User[n].Team == TeamReward)
			{
				...
				{	
					GDSetCoinSend(lpObj->Index, this->Coin1, this->Coin2, this->Coin3,0,0,"Than Ma");
				}				
				GDRankingThanMaChienSaveSend(lpObj->Index, this->User[n].Kills, this->User[n].Deaths); 			
...
void CThanMaChien::CGOpenWindowNPC(XULY_CGPACKET1* aRecv, int aIndex)
{
...
bool CThanMaChien::CheckReqItems(LPOBJ lpObj)
{
	...
	{
		GDSetCoinSend(lpObj->Index, -(this->ReqWC), -(this->ReqWP), -(this->ReqGP), 0, 0, "Than Ma Chien");
	}
...
void CThanMaChien::DivisionTeam()
{
	...
	for(int n=0;n<MAX_THANMACHIEN_USER;n++)
	{
		if (this->User[n].Team == 1)
		{
			...
			gNotice.GCNoticeSend(this->User[n].Index,0,0,0,0,0,0,"%d - Thiên Thần Vs Ác Quỷ - %d",this->ScoreBlue,this->ScoreRed);
		}
		else if (this->User[n].Team == 2)
		{
			...
			gObjViewportListProtocolCreate(&gObj[this->User[n].Index]);
			gObjectManager.CharacterUpdateMapEffect(&gObj[this->User[n].Index]);
			gNotice.GCNoticeSend(this->User[n].Index,0,0,0,0,0,0,"%d - Ác Quỷ Vs Thiên Thần - %d",this->ScoreRed,this->ScoreBlue);
		}	
	}
}
...
void CThanMaChien::UserDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		GDSetCoinSend(lpUserB->Index, this->BonusKill[0], this->BonusKill[1], this->BonusKill[2], 0, 0, "Thưởng Coin Thần Ma");
	}
	//== Tinh Toan Rank
	this->CalcUserRank();
