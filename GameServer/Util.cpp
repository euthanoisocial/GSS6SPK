...
long int string2timestamp(char* date)
{
	int dd, mm, yyyy;
	struct tm when = { 0 };

	sscanf_s(date, "%d/%d/%d", &dd, &mm, &yyyy);
...
void PostMessagePK(char* name,char* message,char* text) // OK
{
	...
	memcpy(pMsg.name,gServerInfo.m_MessagePK,sizeof(pMsg.name));
...
void PostMessageUserON(char* name,char* message) // OK
{
	...
	memcpy(pMsg.name,gServerInfo.m_MemberOnline,sizeof(pMsg.name));
