...
void GDGuildCreateRecv(SDHP_GUILDCREATE_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result == 1 )
	{
		...
		#if(CUSTOM_GUILD_UPGRADE == 1)
		gObj[aIndex].Devote = 0; //fix
		#endif
...
void GDGuildMemberAddResult(SDHP_GUILDMEMBERADD_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result == 1 )
	{
		...
		if ( HereUserIndex >= 0 && lpNode != NULL )
		{
			...
			GuildUpgrade.AddDevoteEff(&gObj[HereUserIndex]);
#endif
		}
	}
}
				...
				if ( gObj[iMember].GuildNumber > 0 )
				{
					if ( gObj[iMember].Name[0] == szMember[0] && gObj[iMember].Name[1] == szMember[1] )
					{
						if ( strcmp(gObj[iMember].Name, szMember) == 0 )
						{
							...
							#if(CUSTOM_GUILD_UPGRADE == TRUE)
							GuildUpgrade.RemoveDevoteEff(&gObj[iMember]);
#endif
							break;
						}
					}
			...
			if ( gObj[n].Name[0] == lpMsg->MemberID[0] )
			{
				if ( strcmp(lpMsg->MemberID, gObj[n].Name ) == 0 )
				{
					...
					if ( gObj[n].Guild != NULL )
					{
						...
						#if(CUSTOM_GUILD_UPGRADE == TRUE)
						gObj[n].Devote = lpMsg->Devote;
						GuildUpgrade.AddDevoteEff(&gObj[n]);
#endif
					}
					else
					{
...
void DGGuildMasterListRecv(LPBYTE lpData)
{
	...
	{
		#if(CUSTOM_GUILD_UPGRADE == TRUE)
		lpGuild->TotalDevote = lpCount->TotalDevote;
		lpGuild->LevelDevote = lpCount->LevelDevote;
#endif
...
#if(CUSTOM_GUILD_UPGRADE == TRUE)
		if (gGuildClass.AddMember(szGuildName, szMasterName, -1, lpCount->Count, lpList->btGuildStatus, lpList->pServer, lpList->Devote) == FALSE)
		{
			break;
		}
#else
