...
#if(PARTYSETPASS)
			gPartySetPass.PartySetPassSearchDelFromList(gObj[this->m_PartyInfo[index].Index[n]].Name);
#endif
...
bool CParty::DelMember(int index,int aIndex) // OK
{
	...
	for(int n=0;n < MAX_PARTY_USER;n++)
	{
		if(this->m_PartyInfo[index].Index[n] == aIndex)
		{
			...
			this->m_PartyInfo[index].Index[n] = -1;
	#if(PARTYSETPASS)
			gPartySetPass.PartySetPassSearchDelFromList(gObj[aIndex].Name);
	#endif
...
bool CParty::SetLeader(int OldLeader, int NewLeader)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Your character is not in party");
		return false;
	}
	// ----
	if( lpObj->PartyNumber != lpTargetObj->PartyNumber )
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Target is not party member");
		return false;
	}
	// ----
	if( !this->IsLeader(PartyNumber, lpObj->Index) )
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Your character is not party leader");
		return false;
	}
	// ----
...
void CParty::CGPartyRequestRecv(PMSG_PARTY_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->Interface.type == INTERFACE_CHAOS_BOX || lpTarget->Interface.type == INTERFACE_CHAOS_BOX)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Khong The Tiep Tuc");
		return;
	}
	if (lpTarget->DieRegen != 0 || lpObj->DieRegen != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Wait...");
		return;
	}
	...
	//-- PatyLevel
	if (gServerInfo.m_PartyRestrict == 1 && this->IsParty(lpTarget->PartyNumber) == 0) 
	{
...
void CParty::GCPartyLifeSend2(int index) // OK
{
	...
	PMSG_PARTY_LIFE_SEND_ pMsg;

	pMsg.header.set(0x79, 0);

	int size = sizeof(pMsg);

	pMsg.count = 0;

	PMSG_PARTY_LIFE_ info;
...
void CParty::CGPartyListRecv2(int aIndex) // OK
{
	...
	PMSG_PARTY_LIST_SEND_ pMsg;

	pMsg.header.set(0x7A, 0);
