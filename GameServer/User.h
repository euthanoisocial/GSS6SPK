...
//cambio de level a max 1000
#if(MAX_LEVEL)
#define MAX_CHARACTER_LEVEL 2000
#else
...
#define MAX_CHAT_MESSAGE_SIZE 100	//chinh dai ki tu chat
...
#define ITEMGET(x,y) ( (x)*MAX_SUBTYPE_ITEMS + (y)) // éo p nó
...
{
	float AddPhysiDamage;
	float AddMagicDamage;
	float AddCurseDamage;
	float AddMinPhysiDamage;
	float AddMaxPhysiDamage;
	float AddMinMagicDamage;
	float AddMaxMagicDamage;
	float AddMinCurseDamage;
	float AddMaxCurseDamage;
...
#if(SAU_ANTIHACK)
struct BAntihackDataSkill
{
	DWORD BPLastAttackTime[6];
	DWORD BPDetectCount[6];
	int   mSkillFloatHit[6];
	int   xDmgCache[6];
	void Clear()
	{
		memset(&mSkillFloatHit, 0, sizeof(mSkillFloatHit));
		memset(&BPDetectCount, 0, sizeof(BPDetectCount));
		memset(&xDmgCache, 0, sizeof(xDmgCache));
	}
};
#endif
...
struct OBJECTSTRUCT
{
	...
	float PhysiDamageMin;
	float PhysiDamageMax;
	float MagicDamageMin;
	float MagicDamageMax;
	float CurseDamageMin;
	float CurseDamageMax;
	float PhysiDamageMaxLeft;
	float PhysiDamageMinLeft;
	float PhysiDamageMaxRight;
	float PhysiDamageMinRight;

	int DWDamageMultiplierRate;
	int ELFDamageMultiplierRate;
	int SUDamageMultiplierRate;
	int DKDamageMultiplierRate;
	int MGDamageMultiplierRate;
	int MGDamageMultiplierRatePhysic;
...
//----------
...
#if(SLOTPET2)
	int LoadEquipItem;
	int EquipInventoryStatus[16];
	CItem* EquipInventory;
	CItem* EquipInventory1;
	CItem* EquipInventory2;
	BYTE* EquipInventoryMap;
	BYTE* EquipInventoryMap1;
	BYTE* EquipInventoryMap2;
#else
	...
	int rBossPoint;
	#if (CB_EventArenaPvP)
	bool	ArenaEventStatus; //trang thai antihack khong check, dung de set neu co chuc nang event hoaac trang thai dac biet
#endif	
	...
	int TrashCodeItemSet3;
	int TrashCodeItemSet4;
	int TrashCodeItemSet5;
	int TrashCodeItemSet6;
	int TrashCodeItemWing;
	int TrashCodeItemKhien;
...
#endif
	#if(SAU_ANTIHACK)
	DWORD BHackDetectCount[5];
	DWORD BHackTimeClearDetectCount;
	DWORD BHackBlockSkillTime;
	int	  BProtectBlockAttack;
	DWORD BHackTimeSendClient;
	DWORD BHackWaitRecv;
	WORD BTargetIndex[2];
	DWORD BTargetTime[2];
	BYTE BProtect_X;
	...
	WORD BProtect_MAP;
	float BProtect_PV;
	DWORD BProtectMove_Time;
	float GroupSkill[256];
	BAntihackDataSkill* BUserAttack;
	DWORD	UseSkillTime;
	float	UseFameSpeed;
	WORD	UseSkillSkillId;
	#endif
	#if(TIDUS_FIX_MT)
	short AttackSpeed;
	#endif
...
#if(SLOTPET2)
bool gObjFixEquipInventoryPointer(int aIndex);
void gObjSetEquipInventory1Pointer(LPOBJ lpObj);
void gObjSetEquipInventory2Pointer(LPOBJ lpObj);
#endif
//**************************************************************************//
// RAW FUNCTIONS ***********************************************************//
//**************************************************************************//
