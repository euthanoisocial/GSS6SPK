...
bool CSkillManager::SkillGreaterDefense(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	int count = gServerInfo.m_GreaterDefenseTimeConstA+((lpObj->Energy+lpObj->AddEnergy)/gServerInfo.m_GreaterDefenseTimeConstB); //Skill tăng phòng thử Elf
...
bool CSkillManager::SkillGreaterDamage(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	int count = gServerInfo.m_GreaterDamageTimeConstA+((lpObj->Energy+lpObj->AddEnergy)/gServerInfo.m_GreaterDamageTimeConstB);
...
bool CSkillManager::SkillDeathStab(int aIndex,int bIndex,CSkill* lpSkill,bool combo) // OK
{
	...
	if (gServerInfo.SkillDamGioLan == 0 && this->CheckSkillRange(lpSkill->m_index, lpObj->X, lpObj->Y, lpTarget->X, lpTarget->Y) == 0) 
	{
		return 0;
	}
...
bool CSkillManager::SkillMonsterSummon(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	switch(lpObj->Class)
	{
		...
		case 275:
			...
			break;
		case 892:
			MonsterClass = 890+GetLargeRand()%2;
			break;
		default:
			return 0;
	}
...
bool CSkillManager::SkillSwordPowerGetPhysiDamage(int aIndex,int* DamageMin,int* DamageMax) // OK
{
	...
	float value = lpObj->EffectOption.AddSwordPowerDamageRate;
...
bool CSkillManager::SkillGreaterIgnoreDefenseRate(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	value = ((value > gServerInfo.m_GreaterIgnoreDefenseMaxRate) ? gServerInfo.m_GreaterIgnoreDefenseMaxRate : value);
