...
void CTrade::ExchangeTradeItemLog(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	for(int n=0;n < TRADE_SIZE;n++)
	{
		if(lpObj->Trade[n].IsItem() != 0)
		{
			gLog.Output(LOG_TRADE,"Giao Dịch từ %s | %s đến %s | %s, Item %s+%d, Skill: %d, Luck: %d, Tính năng: %d, Hoàn hảo: %d, Thần: %d, Hamorny: %03d, Item 380: %03d, Socket: %03d, %03d, %03d, %03d, %03d, Serial: %08X)"
			,lpObj->Account
			,lpObj->Name
			,lpTarget->Account
			,lpTarget->Name
...
void CTrade::CGTradeRequestRecv(PMSG_TRADE_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(970));
		return;
	}
	...
	if (BotAlchemist.TradeOpen(aIndex, bIndex) == 1)
	{
		return;
	}
	...
	{
		this->GCTradeResponseSend(aIndex,0,lpTarget->Name,0,0);
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Chi Giao Dich trong khu vuc an toan!");																	   
		return;
	}
...
#if(BOTHUMUADORAC==1)
	{
		if (BotThuMua.TradeOpen(aIndex, bIndex) == 1)
			return;
	}
#endif								   
#if(BOTCONGHUONG==1)
...
void CTrade::CGTradeOkButtonRecv(PMSG_TRADE_OK_BUTTON_RECV* lpMsg,int aIndex) // OK
{
...
#if(BOTHUMUADORAC==1)
	{
		if (BotThuMua.IsInTrade(aIndex) == 1 && lpMsg->flag == 1)
		{
			BotThuMua.TradeOk(aIndex);
			return;
		}
	}
#endif
	...
	if (lpObj->Map != lpTarget->Map)
	{
		...
		if ((GetTickCount() - lpObj->TradeOkTime) < 5000)
		{
			return;
		}
...
#if(BOTHUMUADORAC==1)
	{
		if (BotThuMua.IsInTrade(aIndex) == 1)
		{
			BotThuMua.TradeCancel(aIndex);
			return;
		}
	}
#endif
