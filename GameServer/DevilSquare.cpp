...
void CDevilSquare::GiveUserRewardMoney(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_DS_USER;n++)
	{
		...
		if (lpLevel->User[n].Rank == 0)
		{
			...
			{
				//Reward
				GDSetCoinSend(lpLevel->User[n].Index, this->m_DevilSquareRewardValue[lpLevel->Level][0], this->m_DevilSquareRewardValue[lpLevel->Level][1], this->m_DevilSquareRewardValue[lpLevel->Level][2], 0, 0,"DS");
...
void CDevilSquare::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (gServerInfo.m_BatTatDevilVIP == 1)
	{
		if(lpObj->Map == MAP_DEVIL_SQUARE2)
		{
		
			lpTarget->ScoreKillDevil++;
			gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(957), lpTarget->ScoreKillDevil,gServerInfo.m_MocKillQuaiDevilVIP);
			if (lpTarget->ScoreKillDevil == gServerInfo.m_MocKillQuaiDevilVIP)
			{
				/*int Devil_Index = 0;
				unsigned char ItemsIndex[5] = { 13, 14, 16, 22, 41 };
				Devil_Index = ItemsIndex[rand() % 5];*/
				//GDCreateItemSend(lpTarget->Index, lpObj->Map, lpObj->X, lpObj->Y, GET_ITEM(14, Devil_Index), 0, 0, 0, 0, 0, lpTarget->Index, 0, 0, 0, 0, 0, 0xFF, 0);
				//gItemBagManager.DropItemBySpecialValue(ITEM_BAG_DEVIL_VIP,&gObj[lpUser->Index],0, 142, 128);
				gItemBagManager.DropItemBySpecialValue(ITEM_BAG_DEVIL_VIP,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
				this->NoticeSendToAll(&this->m_DevilSquareLevel[level], 0, gMessage.GetMessage(959), lpTarget->Name, lpTarget->ScoreKillDevil);
				return;
			}
			if(lpTarget->ScoreKillDevil > gServerInfo.m_MocKillQuaiDevilVIP)
			{
				lpTarget->ScoreKillDevil = 0; Sleep(500);
			}
		}
	}
	//======Devil VIP danh quai roi ngoc
