...
void CPartySetPass::RequestParty(PMSG_PARTY_SETPASS_REQ_REQ_SETPASS lpMsg, int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1019));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1020));
		return;
	}

	if (!strcmp(lpObj->Name, lpTarget->Name))
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1021));
		return;
	}
